...
The commands introduced by this statistics feature includes: `task-stats` and `member-stats`. These commands are faciliatated by the class 'Statistics'
...
*Step 1.* The user launches the application for the first time. The 'Statistics' object stored by ProjectDashboard will be initialised based on the data previously saved.
...
Figure 11. Operational flow of UI when 'GetStatisticsCommand' is called

The following activity diagram summarizes what happens when a user the `task-stats` command:
...
** Pros: Will ensure faster performance, and more detailed statistics can be included
** Cons: Design of statistics object has to be very well-implemented, might not have enough time to implement it by v1.4

Alternative 1 was chosen given the time constraint in implementing the features in time for +Work Version 1.4.
Alternative 2 will be taken into consideration for future expansions of +Work.
...
This makes +Work a more comprehensive application because project tasks and members are able to be kept track of together.
...
Apart from the typical commands ('add-member', 'delete-member', 'find-member') involved in such a central class, the member features also introduce a 'set-image' command. The set-image command allows users to set an image in their computer as the profile picture of a member in +Work.
To accommodate the set-image command, the 'Member' class has an alternative constructor that takes in the image url as a parameter to save it as an attribute to the member object, when 'set-image' command is called. Additionally, to support the 'set-image' command, the 'Member' class implements the following operation:

* `Member#getImagePath()` -- Retrieves the file path of the image stored in the user's computer
* `Member#getImage()` -- Retrieves the member's image using the image file path
...
A new member object is formed, with all the same parameters as the specified member object, and a new Image URL parameter.
`Model#setMember` to replace the old member object with the new one in +Work.
...
The following sequence diagram shows how the image is called up and subsequently displayed in the UI for for an individual member.

image::DisplayImageSequenceDiagram.png[]
...
===== Aspect: Storing of image under member (final or changeable) (basically repalcing the whole member or not)
This section describes the pros and cons of the current and other alternative implementations storing a profile picture under
each member object.

* **Alternative 1:** Storing the image file path as an
** Pros: Editing the member's profile picture involves
** Cons: Less details of individual taks and members exposed in the statistiscs page.
* **Alternative 2 (current choice):** Storing the image file path as a final attribute
...
This section describes the pros and cons of the current and other alternative implementations of displaying the members in +Work.
...
** Cons: Display of members is cramped and needs more
