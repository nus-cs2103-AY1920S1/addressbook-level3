...
After each of the above operations, the state of `loans` is saved in the file `loans.json`, which is stored on the local hard disk.
...
In general, the rest of the operations work using the same sequence of steps.
The only exceptions are the `LoansManager#updateLoanStatus` and `LoansManager#deleteLoan` operations,
which do not create a new `Loan` object.
