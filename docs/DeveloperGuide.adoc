...
* `AlgoBase#copyTask()` - copy a task from one plan to another.
...
* `AlgoBase#setPlan()` - set a task as the current plan in main display.
...
.Activity Diagram for the Execution of `find` Command
...
.Sequence Diagram for the Execution of `find` Command
...
Training plan feature allows user to create customized training plans with specific starting date and end date that consist of selected problems in AlgoBase. Each problem is wrapped up as a task in the plan. Users can record their progress by marking problems in plans as done.

==== Current Implementation

The training plan mechanism is faciliated by `AlgoBase`. It supports the following operations:

* `Algobase#addPlan()` -- Adds a new empty training plan to AlgoBase.
* `AlgoBase#setPlan()` -- Edits an existing training plan by specifying its number.
* `AlgoBase#removePlan()` -- Deletes a training plan by specifying its number.
* `AlgoBase#getPlanList()` -- Returns a list of plans in AlgoBase.

Given below is an example usage scenario and how the training plan behaves at each step.
...
Step 3. The user executes `listplan` to list all plans. The `listplan` command also calls `Model#updateFilteredPlanList()`. The plan `CS2040` is numbered `1` in the displayed list.

Step 4. The user finds out that the exam date of CS2040 has changed, and decides to change the end date by executing the `editplan 1 end/2019-05-05` command. The `editplan` command will call `Model#setPlan()`, which will replace the original plan with the modified plan in the `PlanList`.

[NOTE]
The user can also add value for an empty field by executing `editplan` command if the field was not specified when adding the plan.

Step 5. The user then decides to execute the command `findplan start/2019-03-01 end/2019-03-31`. The `findplan` command constructs a `FindPlanDescriptor` and executes `Model#updateFilteredPlanList(FindPlanDescriptor)`. A list of plans in AlgoBase that has overlapping time range with specified starting date and end date will be displayed on the plan list panel.

[NOTE]
If the user wants to find plans with overlapping time range, both the starting date and the end date should be specified and valid, or an error message will be displayed to inform the user that both fields should be present.
...
[discrete]
=== Use Case 8: Copy Tasks between Plans
...
[discrete]
=== Use Case 9: Delete Tasks from Plan
...
[discrete]
=== Use Case 10: Edit Due Dates of Tasks
...
[discrete]
=== Use Case 11: Mark Tasks as Done in Plan
...
[discrete]
=== Use Case 12: Mark Tasks as Undone in Plan
...
[discrete]
=== Use Case 13: Move Tasks between Plans
...
[discrete]
=== Use Case 14: Set Current Plan in Main Display
...
[discrete]
=== Use Case 15: Switch between View of Items
...
[discrete]
=== Use Case 17: Export AlgoBase data
...
[discrete]
=== Use Case 18: Import AlgoBase data
