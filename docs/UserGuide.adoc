...
* Words in `UPPER_CASE` are the parameters to be supplied by the user e.g. in `add_claim n/NAME d/DESCRIPTION c/CASH AMOUNT date/DATE(dd-MM-yyyy) p/PHONE NUMBER`,
...
This feature sorts the various lists according to the contact's name in lexicographical order. The command
...
This feature sorts the various lists according to the date from the oldest to newest entry.
...
This feature sorts the various lists according to the contact's name in reverse lexicographical order. This command
...
This feature sorts the various lists according to the date from the newest to oldest entry. This command is not
