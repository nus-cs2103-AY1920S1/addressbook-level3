...
[NOTE]
If you do not specify any tasks, Gradlew will run the default tasks `clean` and `test`.
...
* *`shadowJar`* +
Creates `nustudy.jar` in the `build/jar` folder, _if the current file is outdated_. +
