...
If the user requests to remove the entire annotation instead, the `an:Annotation` is simply dereferenced.
If the user requests to remove only the annotation note, the `AnnotationNote` of `Annotation` is dereferenced.
Alternatively, if the user requests to remove annotations from all paragraphs, the entire `:OfflineDocument` is dereferenced and a new, unannotated copy is created.
