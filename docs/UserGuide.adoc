...
Adds a new class to the list of existing classes.
...
[NOTE]
The name of the new class must not be empty,
and it must be different from all current class names.


==== Deleting: deleteclass

Allows a user to delete the selected class. +
...
Allows a user to select a different class. The image below illustrates the change in
...
States will only be made if there is a modification to the notebook.
Hence, commands like `help` and `liststudents` that do not change the internal structure of the notebook will not lead to a saved state. +
