...
image::interactive_parsing_2.png[width='520']

.Typee's response to the above input.
image::interactive_parsing_3.png[width='520']
...
image::interactive_parsing_4.png[width='520']

.Typee's response to the invalid input.
image::interactive_parsing_5.png[width='520']
...
=== Locating appointments by attendee, description, location and priority: `find`
...
|Description
|`d/`
|Descriptions containing description keyword will be returned. e.g. `find d/Singapore based` will return
...
|Location
|`l/`
|Only exact full words will be matched e.g. `find Com Basement` will not match `Com Basement Room 6`

|Priority
|`p/`
|`find p/High`, `find p/Medium`, `find p/Low`, `find p/None`
|===

****
* All search are case insensitive. e.g `find a/hans` is equivalent to `find a/Hans`
* For non-interactive search, the order of the prefixes does not matter. e.g. `find a/Hans Bo p/high` is equivalent to `find p/high a/Hans`
* For non-interactive search, at least one search parameter should be specified.
* For interactive search, all search parameters must be specified sequentially according to the prompt.
****
...
* `find a/Betsy d/Project Meeting l/NUS p/None` +
...
.Sample game play
image::GameWindow.png[width="400" float="left"]

.Sample game play when game is over
image::GameOver.png[width="400" float="left]
