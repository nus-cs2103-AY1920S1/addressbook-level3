...
Trying to open up the _itinerary page_? Use this command!

*Format:* +
``goto PAGE TYPE`` +
{empty} + 

WARNING: The only available ``PAGE TYPE``s are ``calendar``, ``itinerary``, ``address_book``, ``financial_tracker``, ``diary`` and ``achievements``.
