...
Other than "add", ordering of fields is flexible.
...
If more than one are present and the command only accepts one `dt/`, the last argument is taken. Otherwise, see below in `event` command for more details.
    ...
    - `td/TIME_DURATION` The format of time duration is: `hh:mm` and it should not be blank. e.g.
    ...
    - `ymw/YEAR_MONTH_DAY` The format of year month day is: `yyyy-mm-dd` and the number should be valid(same as year month day). If more than one are present, the last argument is taken.
...
For example, `event d/appointment dt/2019-12-20 14:00 dt/2019-12-20 15:00 td/01:30` adds an appointment event on 2019 Dec 20 from 14:00 to 15:00 to the calendar and a reminder for it at
12:30 on that day.
...
Average daily `BLOODSUGAR` levels are clearly within the required range of 4.0 and 7.8 mmol/L (inclusive).
...
3. For more information about GI, please refer to this link:++https://en.wikipedia.org/wiki/Glycemic_index++[link].#
====
*Format:* `recmfmix`
...
==== Recovering food data: _recoverf_ `[coming in v2.0]`
...
==== Editing a food: _editf_ `[coming in v2.0]`
...
Records the user's diets on specified dates and provides daily, weekly, and monthly summaries about nutrition intakes.
This would allow the user to have an overview of his food consumption statistics.
Bases on such statistics, the user can get more specific and personalized suggestions to balance the his nutrition intake.
//end::recmf[]
...
. *Delete*: `delete INDEX`
E.g. `delete 2`
. *Delete Food*: `deletef fn/FOOD_NAME`
...
. *Get User Guide URL*: `help`
. *List*: `list`
E.g. `list`
. *Recommend Food*: `recmf [-nsv] [-sv] [-f] [-p] [-s] [-m] [fn/FOOD_NAME...] [±sort/SORT_ORDER_TYPE]`
E.g. `recmf -p -nsv +sort/gi`
E.g. `recmf fn/chicken rice`
. *Recommend Mixed Food*: `recmfmix`
