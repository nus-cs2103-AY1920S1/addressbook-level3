...
Moreover, the below sequence diagram summarizes the interactions between different components when user enter `DeleteReminderCommand`.
...
1. The date value of the `deadline` attribute is limited to before the end of next year and must be in the future. This prevent users from entering reminder of overdue tasks as inputs.
...
|Store reminders together with other information in `moneygowhere.json`.  +
 +
*Pros:* +
All user's data stored in one place. +
Allows re-use of existing code. +
Lesser number of files to manage. +
 +
*Cons:* +
Could lead to larger file size. +
Higher data loss in cases of file corruption.

|Store reminders in a json file separated from other information. +
 +
*Pros:* +
Improve performance while importing and exporting reminders. +
Lesser data loss in case of file corruption. +
 +
*Cons:* +
Could complicate future implementation of importing and exporting reminders.
`import` and `export` commands now need to manges two file at the same time.

|===

Alternative 1 from above table, is currently implemented in MoneyGoWhere (v1.4) since current version of `Import` and `Export` Commands has not be integrated with reminders yet.
