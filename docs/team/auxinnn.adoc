...
interface for use input.We decided to morph it in to a study management application called NUStudy which is targeted to
university students. NUStudy is used to manage students' lecture notes, test questions and revision tasks. It also
allows students to do mock quiz and view the statistics for better revision.
...
****
* A grey highlight `monospace` implies that this is a command that can be inputted and executed by the application.
...
We have updated the User Guide of Address Book to fit the commands of our project. The following part shows my contribution
to the modification of *NUStudy User Guide* for quiz feature.
...
We also updated the Developer Guide so that the style and architecture fits our project. The following parts shows my
contribution to the modification of *NUStudy Developer Guide* for quiz feature.

NOTE: In this sections, `monospace` indicates both user command and class objects and methods.
