...
User can simply type in the `help` command in the input text area to gain additional information of the usage of our application. +
Once `help` command is executed, system will pop out a dialog window that includes our development web page and some examples of the commands.
...
[.left]
.Calendar view (December 2019) after executing the `nextmonth` command
...
image::calendar-view.png[width="450", float="left"]
[.left]
.Calendar view (October 2019) after executing the `previousmonth` command
