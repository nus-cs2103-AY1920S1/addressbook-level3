...
to generate. It will get the `EventPayload` from the `UndoableCommand` and pass it into the constructor of the
...
behaves similarly to step 4, except that the next `Event` is taken from the top of the redo stack instead
of the undo stack.
[NOTE]
For step 4 and 5, if any of the respective stack is empty when it is needed, a `CommandException` will be thrown
and an error message will be displayed to inform of the user that there are no commands to undo or redo
at the moment.
...
The first diagram (Figure 9) describes the process of storing an `EditEvent` to `EventHistory` during the execution of the
...
The second diagram (Figure 10) here describes the process of undoing the `EditCommand` executed above using the `UndoCommand`.
...
Given below is a Class Diagram (Figure 11) to show the associations between Event, Command and Model. It is specifically designed
...
The following Activity Diagram (Figure 12) summarizes what happens when a user enters undoable commands,
...
** Pros:
*** Uses less memory to store Event objects as compared to entire copes of the Model.
...
*** Uses less memory
** Cons:
*** Violates Single Responsibility Principle as Commands need to know implementation for how it
can be undone or redone and also stores data such as the exercise being deleted in a local field.
...
*** The EventFactory relies on the Factory pattern and helps to reduce coupling between
EventHistory and each individual Event.
** Cons:
*** The Singleton pattern may have a chance of break if multiple threads initializes the singleton class
at the same time, creating multiple versions of EventHistory.
...
image::ScheduleResolveClassDiagram.png[]
...
be too much overhead and we can never be certain that we have met all of the users' needs. As such, we decide to allow
users to define their own properties should they require them. These custom properties can then be applied to all of the
exercises, providing more flexibility in terms of tracking the exercises' properties.

Furthermore, as noted by one of our user stories, there are users that love customisations too. This can also come in handy
when they want to define their own properties for the exercises.
...
Method 3 is implemented to ensure that the name of a default property is not removed by mistake if the user chooses to
remove a predefined custom property.
...
The following sequence diagram (Figure 17) will illustrate how the custom operation works when a custom
...
(Figure 18) shows the workflow when a new custom property is defined.

.Activity diagram of the workflow when a new custom property is added
image::CustomAddActivityDiagram.png[]
...
`custom rm/FULL_NAME`. Alternatively, if the user wishes to remove a custom property just from a single exercise, he/she
can choose to enter `custom rm/FULL_NAME i/INDEX` instead.

The following sequence diagram (Figure 19) illustrates how a custom property is removed from all of the exercises
when a user wishes to remove, say, the `Rating` property from the `PropertyBook`.

.Sequence diagram of a successful removal of a custom property from all exercises
image::EditedCustomRemoveSequenceDiagram.png[]

Similar to the addition of custom properties, the removal of a custom property involves a sequence of checks to
ensure that the correct custom property is removed from the correct exercise(s). These checks are summarised in the
next activity diagram (Figure 20).

.Activity diagram of the workflow when a custom property is to be removed
image::CustomRemoveActivityDiagram.png[]

A custom property that has been removed from the ``PropertyBook`` can be re-added back if the user chooses to.

===== Viewing Custom Properties
To allow user to view the custom properties which they have defined, they can enter ``viewcustom`` which will produce
a table that lists out the name, prefix and parameter type of each custom property they have defined.
...
** Pros: This prevents situations where there could be 2 instances of `PropertyBook` objects and the addition
of a custom property is only carried out to one instance and not in the other. Furthermore, having a singleton
like this helps to provide more utility for methods that rely on the `CustomProperty` objects that have been
created.
...
Choice 2 seems like an ideal selection at the start as coupling is reduced and it reduces the likelihood
of untestable code. However, certain issues arise:
...
for the custom properties in the add/edit commands are valid. However, as the `ExerciseBookParser` in the original https://xxx[code] base only takes in a
...
is just to ensure that a predefined command is entered and is passed to the correct command parser.
Thus, it does not require knowledge of any custom properties. So, a slightly better choice in this case
is to make the data structure holding the `CustomProperty` objects a static variable and parsers that require it can access
it directly.

2. If the data structure holding the `CustomProperty` object is to be made static, it means that this information is
shared among all of the `PropertyBook` instances if Choice 2 was implemented instead. Thus, `PropertyBook`
is acting similarly to a singleton since the information carried by any instance of `PropertyBook` is the same.

As such, after much consideration, Choice 1 was implemented. To ensure that testing can be carried out better,
the data inside `PropertyBook` is cleared before each test.
//end::customfeature[]
...
==== Rationale
Beginners now have a plethora of choices and may be overwhelmed when deciding on what exercises to do.
Thus, we decided to provide users with sample exercise routines to reduce the inertia of starting this lifestyle change.
On other hand, regular gym goers may face a repetitive and mundane exercise routine or may want to experiment with different exercises.
As such, put it briefly, we decided to give users the ability to discover exercises based on the characteristics they are interested in.

This feature presents a cohesive function that all users can benefit from.
This also makes our application well-rounded so that users can better achieve their fitness goals.
...
.Activity diagram showing the workflow of a suggest possible command
image::SuggestActivityDiagram.png[]

In detail, when a `SuggestPossibleCommand` is entered, the `Logic` component is responsible for parsing the inputs into a `Predicate`.
The `Predicate` is then used to instantiate a `SuggestPossibleCommand`, and later used to filter a list of `Exercise` when the command is executed.
...
.Sequence diagram of a suggest possible command
image::SuggestSequenceDiagram.png[]
...
image::SuggestPredicateSequenceDiagram.png[]
...
image::SuggestCommandClassDiagram.png[]

Creating classes such as `ExerciseCustomPropertyPredicate` and `ExerciseMusclePredicate`
allow us to conduct better testing because we can compare the `Collection` of `Muscle`/`CustomProperty` being considered.
...
The figure (Figure 24) below shows the class diagram of statistics:
