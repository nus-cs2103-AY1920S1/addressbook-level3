...
You can use the `find` command to find repeaters whose description matches all the (case-insensitive) given keyword(s). If no keyword is specified, then the search filter is cleared.
...
* *Loan Add new* : `add a/Amount [d/DATE] [n/NAME] `
* *Loan Pay* : `paid i/INDEX [n/NAME] [a/AMOUNT]`
* *Loan Edit* : `edit i/INDEX [a/AMOUNT] [s/DATE] [n/NAME] [d/DESCRIPTION]`
