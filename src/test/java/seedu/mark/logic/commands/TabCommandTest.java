...
class TabCommandTest {
    ...
    public void execute_tab_success() {
        CommandResult expectedCommandResult = new TabCommandResult(String.format(MESSAGE_SWITCH_ACKNOWLEDGEMENT, TabCommand.Tab.DASHBOARD.toString()), true, false, false);
        assertCommandSuccess(new TabCommand(TabCommand.Tab.DASHBOARD), model, expectedCommandResult, expectedModel);

        expectedCommandResult = new TabCommandResult(String.format(MESSAGE_SWITCH_ACKNOWLEDGEMENT, TabCommand.Tab.OFFLINE.toString()), false, false, true);
        assertCommandSuccess(new TabCommand(TabCommand.Tab.OFFLINE), model, expectedCommandResult, expectedModel);

        expectedCommandResult = new TabCommandResult(String.format(MESSAGE_SWITCH_ACKNOWLEDGEMENT, TabCommand.Tab.ONLINE.toString()), false, true, false);
