...
This command allows users of the AlgoBase application to add optional remarks to people in their description book and edit it if required.
...
/**
 * Changes the remark of an existing problem in the description book.
 */
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `AlgoBaseParser#parseCommand()` to recognize the `remark` keyword.
Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.

You can refer to the changes in this webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/7d04e49e364dad661cd88f462f01923fba972d2c#diff-5338391f3f6fbb4022c44add6590b74f[diff].
...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the problem identified "
            + "by the index number used in the last problem listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the problem in the filtered problem list to edit the remark
     * @param remark of the problem to be updated to
     */
...
Your code should look something like webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/83dd9e6b03d6b83199ceb6f3b66166483155abed#diff-34ace715a8a8d2e5a66e71289f017b47[this] after you are done.

=== Parse user input

Now let's move on to writing a parser that will extract the index and remark from the input provided by the user.

Create a `RemarkCommandParser` class in the `seedu.description.logic.parser` package.
...
NOTE: Don't forget to update `AlgoBaseParser` to use our new `RemarkCommandParser`!

If you are stuck, check out the sample webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/efdcdf0e80cec9489f7b47e3f65824f4688ad8f7#diff-fc19ecee89c3732a62fbc8c840250508[here].

== Add `Remark` to the model

Now that we have all the information that we need, let's lay the groundwork for some _persistent_ changes.
We achieve that by working with the `Problem` model.
Each field in a Problem is implemented as a separate class (e.g. a `Name` object represents the problem's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a problem.

=== Add a new `Remark` class

Create a new `Remark` in `seedu.description.model.Problem`. Since a `Remark` is a field that is similar to `Description`, we can reuse a significant bit of code.

A copy-paste and search-replace later, you should have something like webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/b7a47c50c8e5f0430d343a23d2863446b6ce9298#diff-af2f075d24dfcd333876f0fbce321f25[this].
...
Without getting too deep into `fxml`, let's go on a 5 minute adventure to get some placeholder text to show up for each problem.
...
to webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.description.ui.ProblemCard`].
...
```
into webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-12580431f55d7880578aa4c16f249e71[`main/resources/view/ProblemListCard.fxml`].
...
== Modify `Problem` to support a `Remark` field

Since `ProblemCard` displays data from a `Problem`, we need to update `Problem` to get our `Remark` displayed!

=== Modify `Problem`

We change the constructor of `Problem` to take a `Remark`.
We will also need to define new fields and accessors accordingly to store our new addition.

=== Update other usages of `Problem`

Unfortunately, a change to `Problem` will cause other commands to break, you will have to modify these commands to use the updated `Problem`!

TIP: Use the `Find Usages` feature in IntelliJ IDEA on the `Problem` class to find these commands.

Refer to webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/b241ef8526bd4e8a6ad94b8118b7fc3441ec8fa3#diff-3462fc1118b9872387a01b41157b0402[this commit] and check that you have got everything in order!

== Updating Storage

AlgoBase stores data by serializing `JsonAdaptedProblem` into `json` with the help of an external library -- Jackson.
Let's update `JsonAdaptedProblem` to work with our new `Problem`!

While the changes to code may be minimal, the test data will have to be updated as well.

WARNING: You must delete AlgoBase's storage file located at `/data/algobase.json` before running it!
Not doing so will cause AlgoBase to default to an empty description book!

Check out webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/ce4f9b70f524d2395948861d80d57fda9ae6e82e#diff-07708562699e2436c717f3330bafae1e[this commit] to see what the changes entail.

== Finalizing the UI

Now that we have finalized the `Problem` class and its dependencies, we can now bind the `Remark` field to the UI.

Just add webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/56d5cb662c31dd38b02f6a5301ba6ab3c667d6a3#diff-0c6b6abcfac8c205e075294f25e851fe[this one line of code!]
[source, java]
.ProblemCard.java
----
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Problem`.
Since all fields in a `Problem` are immutable, we create a new instance of a `Problem` with the values that we want and save it with `Model#setProblem()`.

[source, java]
.RemarkCommand.java
----
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Problem: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Problem: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Problem editedProblem = new Problem(problemToEdit.getName(), problemToEdit.getAuthor(), problemToEdit.getWeblink(),
                problemToEdit.getDescription(), remark, problemToEdit.getTags());

        model.setProblem(problemToEdit, editedProblem);
        model.updateFilteredProblemList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedProblem));
    }
     ...
     * {@code problemToEdit}.
     */
    private String generateSuccessMessage(Problem problemToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, problemToEdit);
    }
----

Make one last webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/b3307d7a42acb613a8f1d36979a7f547abcf6623#diff-34ace715a8a8d2e5a66e71289f017b47[check] to ensure that everything is working properly...
...
webLink:https://github.com/nus-cs2103-AY1920S1/algobase-level3/commit/b3307d7a42acb613a8f1d36979a7f547abcf6623#diff-d749de38392f7ea504da7824641ba8d9[this].

== Conclusion

This concludes the tutorial for adding a new `Command` to AlgoBase.
