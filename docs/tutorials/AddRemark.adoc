...
Each `Command` returns an instance of `CommandResult` upon success and `CommandResult#feedbackToUser` is printed to the `FeedbackDisplay`.
...
While we have successfully printed a message to `FeedbackDisplay`, the command does not do what it is supposed to do.
...
We achieve that by working with the `Contact` model.
Each field in a Contact is implemented as a separate class (e.g. a `Name` object represents the contact's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a contact.

=== Add a new `Remark` class

Create a new `Remark` in `seedu.planner.model.contacts`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.planner.ui.ContactCard`].
...
```
into link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-12580431f55d7880578aa4c16f249e71[`main/resources/view/ContactListCard.fxml`].
...
We change the constructor of `Contact` to take a `Remark`.
...
TIP: Use the `Find Usages` feature in IntelliJ IDEA on the `Contact` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedContact` into `json` with the help of an external library -- Jackson.
Let's update `JsonAdaptedContact` to work with our new `Contact`!
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Contact`.
Since all fields in a `Contact` are immutable, we create a new instance of a `Contact` with the values that we want and save it with `Model#setContact()`.
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Contact editedAccommodation = new Contact(contactToEdit.getName(), contactToEdit.getPhone(), contactToEdit.getEmail(),
                contactToEdit.getAddress(), remark, contactToEdit.getTags());

        model.setContact(contactToEdit, editedAccommodation);
        model.updateFilteredContactList(PREDICATE_SHOW_ALL_ContactS);

        return new CommandResult(generateSuccessMessage(editedAccommodation));
    }
