...
Note how the diagram shows only how the execution flows _between_ the main components. That is, it does not show detail of
the execution path _inside_ each component. By hiding those detail, the diagram succeeds in informing the reader
about the overall execution path of a command without overwhelming the reader with too much detail.
In this tutorial, you aim to find those omitted detail so that you get a more in-depth understanding of the code base.
...
+
The sequence diagram below shows the detail of the execution path through the Logic component.
