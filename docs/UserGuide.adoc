...
[WARNING]
There is a limitation to the amount that you may enter which would be -999999.99 to 999999.
====
...
to show up in the Reimbursements Tab.
...
==== To Delete Transaction(s) in the Home Tab
...
[NOTE]
The fields above can vary in their order. It is not compulsory to include all of them but you need to
...
All other commands require you to input `back` to return to the full list of all transactions.
...
[WARNING]
Due to size limitation, you will not be allowed to add any items into the table if the total amount exceeds $9999. You
...
** [blue]`add c/stationary d/pancake q/3` - Adds 3 items which has the description "pancake"
** [blue]`add d/pancake q/3` - Adds 3 items which has the description "pancake"
...
If both are specified but description is invalid, *only suggestions* will be shown in the response box according to the mistyped description.
...
** [blue]`edit d/book q/3` - Updates the quantity of an existing item of description "book" to 3
...
is not empty. This disables any modification to the inventory except addition of items.].
