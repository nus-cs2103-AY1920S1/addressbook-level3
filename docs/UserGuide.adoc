...
[IMPORTANT]
 Note that indices are always based off the latest view of persons/policies i.e. if the user keys in a `find`
command, indices to subsequent person-related commands will be based off the resulting list of persons from the
`find` command. The same applies for policies.
