...
In addition, various Exception classes are added to handle possible runtime errors while adding new Deadlines.
...
The bad flashcard feature allows the user to rate specific FlashCards as 'bad' if they did not perform well on them.
These rated FlashCards will then we stored as a Deadline for 3 days later, where the user will then be reminded to
re-test these FlashCards.
...
The user executes `bad 2` to rate the second FlashCard as a "Bad" FlashCard
