...
|**Pros**
a|* Retrieving athlete's individual records is fast - O(1) time
a|* Checking of records can be done with a simple for-loop

|**Cons**
a|* Checking requires using an iterator or a lambda operation that requires only final variables
a|* Adding of records is susceptible to errors duplicate persons can be added
...
* HashMap supports fast retrieval, which fulfils one of our non-functional requirements of being able
to support a database of 1000 athletes
* Concept of key-value is very similar to our athlete-records relationship so the existing methods
that are in the HashMap API are very useful
