...
* stores a `UniqueElementList` which contains the list of rooms.
* does not depend on any of the other three components.

.Structure of the QueueManager Component
image:QueueManagerClassDiagram.png[]
...
1.  Doctor in room 1 takes a break
2.  User requests to avoid directing patients to the doctor in room 1
3.  System sets the doctor to be on break
...
1.  Doctor in room 1 resumes his/her duty
2.  User requests to start directing patients to the doctor in room 1
3.  System sets the doctor to be on duty
...
2.  User checks whether the current appointment can be postponed.
...
[discrete]
=== Use case: Set next appointment (UC7)
...
[discrete]
=== Use case: Indicate that a doctor is on-duty and able to tend to patients (UC8)
...
[discrete]
=== Use case: Editing Information in GUI (UC9)
