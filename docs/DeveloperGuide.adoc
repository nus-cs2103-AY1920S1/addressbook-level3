...
* `GetStatisticsCommand` -- Returns a panel containing a break down of the questions by its results.
...
The `stats` command creates a new `commandResult` setting the type of command to `STATS`.
...
Step 3. When the `commandResult` is executed from the `MainWindow`, it will call a `showStats` method to filter
the `quizResults` list by CS2103T and create the relevant outputs.
...
Step 4. The user now wants to stop viewing statistics and decides to view all notes and tasks again using the `listnote` command.
...
** Pros: The questions done are sorted by subjects, is more informative.
...
=== Add revision task
==== Implementation
`AddTaskForNoteCommand` is implemented to add a task for lecture notes revision with the command `rn`. It extends `Command` class and adds a
`Task` for lecture notes revision to the `TaskList` kept in `AppData` model, which wraps all application data. Similarly,
`AddTaskForQuestionCommand` is implemented to add a task for question notes.
...
The following process illustrates the mechanism of adding a revision task to the task list.

Step 1. The user enters command `rn t/UML diagram dt/01/01/2019 tm/1200` for adding a task for note. The command will be parsed by `AddTaskForNoteCommandParser`.
...
* **Alternative 1 (current implementation):** implement `Task` as a concrete class with two subclasses to support task for
