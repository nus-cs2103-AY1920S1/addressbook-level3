...
** Justification: Nurses or new users may not be familiar with command line inputs and the different types commands. Thus, having a suggestion panel at the side will greatly enhance user experience. Having a guided system will aid nurses in familiarising with the application commands and understanding of the function of the system in general.
** Highlights: The implementation of word suggestion is rather tedious since many of the full commands have different formats. For example, "pat-list" do not require any patient index and prefixes but "pat-edit 1 n/ ..." requires both. Thus the implementation was changed a few times to fit the objective of providing a better user experience. Logic for checking already matched words in order to change a new set of suggestion words is challenging as well mainly due to that many commands have different formats as mentioned earlier.
