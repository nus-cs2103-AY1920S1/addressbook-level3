...
*Value proposition*: Many people visit the neighbourhood library to borrow books and also donate their books. There is
...
|`* * *` |librarian |add a book brought/donated by people to the library |keep track of the addition of the book

|`* * *` |librarian |delete books that are no longer available |keep track of the deletion of the book

|`* * *` |helpful librarian |search for certain book by the title/author/genre |help borrowers check if it is
available

|`* * *` |forgetful librarian |mark a book as loaned out |tell borrowers if the book is available

|`* * *` |forgetful librarian |mark a book as returned |let borrowers know that the book will now be available
...
|`* * *` |helpful librarian |assist a borrower to registering in the system |help new borrowers start borrowing books
...
|`* *` |librarian |be able to see an image of the book cover |be reminded of the commands available
...
(For all use cases below, the *System* is the `Liberry` and the *Actor* is the `user`, unless specified otherwise)
