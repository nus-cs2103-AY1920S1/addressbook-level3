...
otherwise provided, `EditNoteCommandParser` calls the static method `NoteImageSelector#selectImage()`,
...
Unlike `Title` and `Content`, which are wrappers around `String` objects, both the use of a JavaFX
built-in object and the system-dependent implementation of graphics-related objects required me to
make certain critical decisions on the implementation of image support.
...
* _Alternative 2:_ Images are copied when the image is selected, i.e. `NoteImageSelector#selectImage()`
