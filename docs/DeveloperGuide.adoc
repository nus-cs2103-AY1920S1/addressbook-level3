...
Step 1. The user launches the application for the first time. If `recordList.json` is not found in `/data`, a `recordList.json` will be produced from `SampleDataUtil#getSampleRecords()`. Otherwise, the recordList will be loaded from `recordList.json` using `UniqueRecordList#setRecord(<List>)` and checked for inconsistencies e.g. missing fields, invalid fields. If invalid fields or missing consequential fields that must be used (e.g. `BloodSugar`’s `Concentration` field is consequential while its `Height` and `Weight` fields are not) are detected, an Exception is thrown and `recordList.json` start from scratch. Otherwise, inconsequential fields will be automatically repopulated on saving to the `recordList.json` in any command.
...
* Other personalisation features such as font colour, `background` and achievements are currently represented by independent classes `Colour`, `Background` and `Achievement` respectively on their own, representing the model as their name describes.
...
* For both font colour and `background` commands, the StyleManager class of `Ui` is used to set the user’s intention of colour and `background` (if parsing is successful). The way StyleManager sets the `background` is by making a copy of the existing StyleSheet used, modifying the required fields and setting it to the StyleSheets of the scene, internally.
