...
The gradle configuration for this project is defined in the _build script_ webLink:../build.gradle[`build.gradle`].
...
* *`shadowJar`* +
Creates the `algobase.jar` file in the `build/jar` folder, _if the current file is outdated_. +
