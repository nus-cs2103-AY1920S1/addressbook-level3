...
Shows an overview of the state of expenses and incomes. Log entries can be grouped by a specified attribute (`<gb>`) with either the frequency or total amount of each group summarised (`<sum>`) in a graphical summary. These attributes include log entries of the same month, type, transaction method, category or place.

Both `<gb>` and `<sum>` have to be present. Any combination of `ATTRIBUTE` and `STAT` should work.
...
E.g. `stats <sum> freq <gb> entrytype`
...
Budgets for a specific place, category or transaction method can also be set (optional). Only at most one field out of these 3 `<met>`, `<cat>`, `<place>` can be included to make the budget more specific. If a spend entry matches the specified field and fall within the budget duration, they will be included in the budget. If none of these fields are indicated, the budget will sum up the amounts of *all* `Spend` entries which fall between the budget duration.

In essence, there are 3 optional fields (only at one can be used at a time):
...
Format: `*budget* INDEX [<day> TRANSACTION_DATE]` +
e.g. `budget 3`

==== List out all budgets: `listb`

List out all budgets in recorded down. Budgets will
