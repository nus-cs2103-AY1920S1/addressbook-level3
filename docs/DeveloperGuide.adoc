...
(Question: should I list out all the methods which the controller uses to access the model? ie getPolicyPopularityBreakdown, getGenderBreakdown etc)
The controller is facilitated by an abstract class `DisplayController`, which extends `UIPart<Region>`.
Every supported visual format extends this abstract class.

(TODO: show class diagram)

The view is facilitated by the associated FXML. These views share a common
CSS, and also have their own CSS file.

[NOTE]
In the event a controller cannot display a particular indicator, it will throw a `parseException` error,
which provides suggestions of which visual controllers are supported by the particular indicator.
...
popularity breakdown indicator in piechart format. The execution of a `display` command determines
what will be shown (the `displayIndicator`) and how it will be shown `displayFormat`.

Step 2. `displayFormat` specifies that the controller `BarChartVisual` will be instantiated.
This controller will be instantiated with the arguments `logic` and `displayIndicator`.

Step 3. The barchart controller sets the variables of its associated FXML in its construction. This is done by
calling the `Visual#setAttributes()` method. Within this method, the controller utilises the display indicator
`policy-popularity-breakdown` to call the method `Logic#getPolicyPopularityBreakdown()`,
which returns the data in the model it needs. The controller then calls `Visual#castData`, which will cast the model’s data
type to the data type supported by bar charts. `Visual#setAttributes` also sets the chart title and axis titles.

Step 4. Finally, the `MainWindow` calls `Visual#getRoot()` and displays the view.
...
* **Alternative 1 (current choice):** Visual controllers to extend from abstract class `visual`
** Pros: Allows visual controller subclasses to share code.
* **Alternative 2:** Visual controllers to implement interface `visual`
** Pros: Satisfies the can-do relationship of an interface.
** Cons: These methods are used internally in the construction of the controller; they have private access modifiers.
Using an interface entails that these methods have access modifiers other than private instead,
