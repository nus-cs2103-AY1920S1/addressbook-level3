...
Create a `RemarkCommandParser` class in the `seedu.address.person.logic.parser` package.
...
Taking a quick peek at existing an `Command`...
...
Create a new `Remark` in `seedu.address.person.model.person`.
Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.address.person.ui.PersonCard`].
...
That's it!
Fire up the application again and you should see something like this:
...
Tests are crucial to ensuring that bugs don't slip into the codebase unnoticed.
This is especially true for large code bases where a change might lead to unintended behavior.
...
The functions ensure that commands produce the expected `CommandResult` and output the correct message.
