...
Undo the most recent command by
...
This section contains the commands for mananging your team member's calendars and for scheduling a project meeting time.


****
Before you get started using +Work's calendar features, make sure you have completed the following steps

====

1. Ensure your team members have exported their calendars as an `.ics` file (See below on how to export a calendar)

2. Collect the calendar files from your team members

3. Take note of the file path where the files are stored, it will be used later
...
Tip: How to export a calendars as an `.ics` file?
...
You can add a calendar by specifying the file path where the team member's calendar is stored as well as the name of the team member.
...
You can schedule your next meeting by specifying the time period between which you wish to hold your meeting as well as the duration of the meeting in hour(s).

+Work will help you find the most suitable meeting time between `START_DATE` and `END_DATE`.
...
* Let's say you want to schedule a __2__ hour meeting in the upcoming week, between __11th Nov 8 a.m__ and __15th Nov 5 p.m__. After entering the details in the corresponding fields, as such
...
After using the command `find-meeting-time`, you can schedule a meeting from the list of possible timings by referring to the `INDEX` of the meeting.
...
* After looking through the suitable timings, you find meeting **#5** is your preferred timing

image::preferred-meeting-time.PNG[]

* Using the index of the meeting **#5**, enter the command `add-meeting meeting/5` and hit kbd:[Enter]

* +Work saves the meeting and allows you to view them at the `home` page
