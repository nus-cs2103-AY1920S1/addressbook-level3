...
Imports the spending from a Csv file specified by a file path. The new spending
Existing spending entries are not replaced. +
...
The figure above shows a valid Csv file that you could have or might want to create. The Csv file must have a header row with the following values:
...
Exports spending data to a `moneygowhere.csv` file specified by a folder path. +
...
Data from the spending list is exported to the specified folder with a file name `moneygowhere.csv`.
