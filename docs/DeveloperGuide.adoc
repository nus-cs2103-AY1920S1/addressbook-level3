...
* Also contains a `Person` object representing the 'Operator' who filed the incident and a `District` which represents the location of the incident.
...
Prior to login, the user is only granted access to the `Login`, `Add`, `Help`, and `Exit` commands. This is achieved via a guard statement in the `LogicManager` checking whether the user is logged in or the command created is an approved command that doesn't require login.
...
Uniqueness of a username is ensured by preventing duplicates during the account creation [`AddCommand`] and account update [`UpdateCommand`] processes. The respective commands will check the list of accounts in the model and throw an exception if a duplicate is found.
...
The checks described above all function in the command execution stage. The `AddCommand`, `UpdateCommand`, and `DeleteCommand` retrieves the logged in `Person` from the `Model` via utilisation of the `Session` object.
...
[NOTE]
The swap command also restricts users from accessing commands affecting objects not on display.
...
** Pros: Easy to implement.
** Cons: User might get confused between list and find command usage.
* **Alternative:** Utilising find command to search for tags
** Pros: Centralise all account search operations in one command.
** Cons: Requires the addition of argument prefixes, hard to implement.
