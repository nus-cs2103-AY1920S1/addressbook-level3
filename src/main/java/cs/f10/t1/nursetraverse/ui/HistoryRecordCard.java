...
public class HistoryRecordCard extends UiPart<Region> {
    ...
    public HistoryRecordCard(HistoryRecord record, Index index) {
        super(FXML);
        ...
        if (commandTextOptional.isPresent()) {
            commandText.setText(String.format("%d. %s", index.getOneBased(), commandTextOptional.get()));
        } else {
            // This should not happen, but defensively display the debug string
            commandText.setText(String.format("%d. %s", index.getOneBased(), record.toString()));
        }
    }
