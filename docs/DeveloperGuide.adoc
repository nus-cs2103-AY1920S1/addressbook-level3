...
=== Undo/Redo Feature
The undo/redo feature mechanism is facilitated by `HistoryManager`. `HistoryManager` remembers commands that mutate the
state of the `Model` in its execution which we define as *invertible commands*. These commands are stored internally in
...
All commands that mutate the state of `Model` are considered *invertible commands*. There are only two *exceptions*, which
are undo and redo commands. Therefore undo and redo commands are not remembered by `HistoryManager`.
...
Commands support the given operations:
...
Step 1. The user launches the application for the first time. The `HistoryManager` is initialized. `HistoryManager#executedCommands` and `HistoryManager#inverselyExecutedCommands`
are empty.
