...
**List command:** `-list_orders` & `-summary_orders`

*add words here*

**Sort command:** `-sort_orders`
...
=== [Proposed] Deliverymen salary adjustment feature
...
This feature allows editing of a Restaurant's details and Food menu under 1 mode, through different commands.

==== Implementation
* **Model**:
ModelManager now contains a editingRestaurantList in addition to the filteredRestaurantList.
access to the Restaurant currently under EditMode.

* **Logic**:

A different Parser will parse the userInput based on the current context.
Current commands for EditMode can only be accessed if under Editing Context.

* **Ui**:
Changes to the Ui for edit mode is facilitated by `Context` enum type.
Changes to the Ui will be made accordingly,
An extra StackPane showing the restaurant under EditMode currently will be displayed,
while the StackPane displaying the list of Restaurants originally will be filled with the restaurants Food menu instead.
