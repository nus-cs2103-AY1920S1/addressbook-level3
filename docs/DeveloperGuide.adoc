...
* **Alternative 2:** Use a different parser for each command word.

* **Alternative 3:** Use a different parser for each use case.
...
The edit function for manipulating annotations also has multiple responsibilities: editing existing
annotations and moving annotations from paragraph to paragraph.

This means each command has multiple cases to handle.
There are hence a few alternatives in which we can segregate the responsibility of parsing and executing:

* **Alternative 1:** Use a single parser to parse all the annotation-related commands and for each command create
** Pros:
** Cons:

* **Alternative 2:** For each of `annotate`, `annotate-delete` and `annotate-edit`, use a separate parser.
** Pros:
** Cons:

* **Alternative 3:** For each small function, create
