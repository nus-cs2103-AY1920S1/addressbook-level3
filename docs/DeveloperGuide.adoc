...
_Due to this fact that this component has to work closely with the `UI` and `AppManager` (without being
coupled directly), it is separated from the `Logic` or `Game` components._ Its roles include:


* Dealing with the timer that runs during a game session.
* Periodically trigger events that will update either the `UI` or `AppManager` components.
component.
