...
****
1. The `FIELD` are as follows: `NAME`, `DESCRIPTION`, `PRICE`, `CATEGORY`, `LOCATION`, `OPENING_HOURS`, `RESTRICTIONS`.
2. The `DIRECTION` are as follows: `ASCENDING` or `DESCENDING`.
****
...
The fields are stored in the SavingsAccount in the CustomSorter object, which stores the necessary
as a list.
...
The Savings feature also allows the user to withdraw from his savings account into his $aveNUS wallet using the `withdraw AMOUNT` command.
On top of this, a `show TO_DISPLAY` command is implemented so that users can view their savings and withdrawals exclusively, so that the user can decide for himself/herself
if he has been saving enough into or withdrawing too much from his savings account. `TO_DISPLAY` will either be `savings`/
`withdrawals`/`both`, whereby to the `TO_DISPLAY` input is case-insensitive.

==== Classes for savings feature in Model

The Savings feature was implemented with the following set of classes introduced to the Model:
...
****
`CurrentSavings` is a class containing the user's current savings value, i.e. how much the user has saved.
****
...
===== ReadOnlySavingsHistory

===== ReadOnlySavingsAccount
...
On top of the implemented classes above, to support the Savings feature, we have introduced new exceptions to handle cases when
the user enters erroneous inputs which could result in inaccurate tracking of the user's savings behaviour. Therefore,
reducing the effectiveness of $aveNUS as a savings tracking application.
The exception classes include the following:

===== InsufficientSavingsException

===== SavingsOutOfBoundException

The `SavingsAccount` object is stored in the $aveNUS application persistently.
It exposes a read-only interface `ReadOnlySavingsAccount` to allow only an unmodifiable version of the `SavingsAccount`
to be exposed to the users, maintaining data integrity. Within the `SavingsAccount`, the class `CurrentSavings` is instantiated,
which is a sum of the total of savings added to and withdrawals taken from the saving's account.

On top of this, the class of `SavingsHistory` is also implemented into the Model. It contains a list of the past savings and withdrawals made by the user,
Like the `SavingsAccount`, it implements a read-only interface, named `ReadOnlySavingsHistory`, to expose an unmodifiable version
of the `SavingsHistory` to the user, so as to maintain data integrity.

Within the `SavingsHistory` class, the class `SavingsHistoryList` is implemented,
to introduce better abstraction of the list which stores the savings and withdrawal history data. The collection
of savings and withdrawals data is returned as an unmodifiable list with methods within the class `SavingsHistoryList`.
...
.Design decisions for `RecommendationSystem`
****
`RecommendationSystem` was chosen to be a singleton because of the fact that it exists as a global class, encapsulating
many seemingly unrelated classes such as `Budget`, `PurchaseHistory` and `UserRecommendations` in order to produce the
required recommendation value.

Furthermore, there should only be one unique `RecommendationSystem` and `UserRecommendations` in the application.
To ensure the above requirements are met, we have decided to implement `RecommendationSystem` as a singleton class.
****
...
Detailed below are the design considerations taken into account when engineering the
 `RecommendationSystem#calculateRecommendation()` function.
