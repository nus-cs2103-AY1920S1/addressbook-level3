...
image::expense/developerguide/ExpenseModelClassDiagram.png[width="60%" title=Class diagram showing the expense model]
...
image::expense/developerguide/ExpenseUIClassDiagram.png[width="95%" title=Class diagram showing the user interface of expense]
...
image::expense/developerguide/DoneEditExpenseCommandSequenceDiagram.png[width="95%" align="left" title=Sequence diagram showing the execution of DoneEditCommand]
...
Deletion of expense is similar to the _Delete Trip/Day/Event_ feature (see <<Delete_Trip>>).
...
image::expense/developerguide/DeleteExpenseDiagram.png[width="80%" title=Activity diagram showing the execution of deleting an expense]
...
image::expense/developerguide/CurrencyModelClassDiagram.png[width="60%" align="left" title=Class diagram showing the currency model]
...
image::expense/developerguide/CurrencyUIClassDiagram.png[width="90%" align="left" title=Class diagram showing the user interface of currency]
...
if an instance of `DiaryPhoto` needed to be instantiated somewhere else other than the `AddPhotoParser` for future use.

However, since the storage model for `DiaryPhoto` (which is `JsonAdaptedDiaryPhoto`), initializes the model
through deserializing the saved file path, this would have led to needing to a separate constructor for `DiaryPhoto`
...
However, `Page` implementation (see <<page_api>>) is still followed, and all components are thus contained within,
in one of `DiaryPage's` placeholders.
