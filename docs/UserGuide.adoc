...
. A list of books will be displayed at the left column but not a list of borrowers as a list of borrowers
    will not be very useful. The borrowers will need to provide their `borrowerId`
    when they borrower books. Thus, to borrow books or check a borrower's information,
    simply enter the serve mode with the borrower's `borrowerId`.
// end::quickStart[]
