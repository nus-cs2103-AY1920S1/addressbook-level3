...
* Assume you are planing to expand the functionality of the AlgoBase (but keep it as a CLI application).
What other user stories do you think AlgoBase should support? Add those user stories to the `DeveloperGuide.adoc`.
...
e.g. rename the tag `friends` to `buddies` (i.e. all problems who had the `friends` tag will now have
a `buddies` tag instead)
Assume that AlgoBase confirms the change with the user before carrying out the operation.
...
* Currently, AlgoBase data are saved to the file after every command.
...
Note how the `AlgoBase` class implements the `ReadOnlyAlgoBase` interface so that clients who don't need write access to the `AlgoBase` can access the `AlgoBase` through the `ReadOnlyAlgoBase` interface instead.

image::ReadOnlyAlgoBaseUsage.png[width=500]
...
* `Override` the `getPrintableString` in classes `Name`, `Author`, `Weblink`, and `Description` so that each produces a printable string representation of the object. e.g. `Name: John Smith`, `Author: 12349862`
...
----
+
The above method can be used to get a printable string representing a bunch of problem details.
For example, you should be able to call that method like this:
+
[source,java]
----
// p is a Problem object
return getPrintableString(p.getAuthor(), p.getWeblink(), p.getDescription());
----
...
* Do some enhancements to the AlgoBase GUI. e.g. add an application icon, change font size/style
...
* Consider adding a new command to the Description Book. e.g. an `edit` command. Notice how little you need to change in the `LogicManager` class to extend its behavior so that it can execute the new command.
...
* Is it possible to make the `AlgoBaseParser` class more OCP-compliant in terms of extending it to handle more
command types?
...
=== Exercise: Enhance AlgoBase

* Enhance AlgoBase in some way. e.g. add a new command
