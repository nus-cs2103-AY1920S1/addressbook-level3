...
choices, either a script or an expression. Therefore, their concrete classes are `*PredicateExpression*` and `*ActionExpression*`
...
name of the script it represents.
...
Shown above is a reference frame sequence diagram which takes place during the execution of the `*TransactionAddCommand*`,
...
****
For example, one of the classes implementing `*TestableExpression*`, `*ContainsExpression*`, is created when a `*PredicateExpression*`
has the *Operator:* `contains`. The corresponding attribute and value for the expression will be provided into the
`*ContainsExpression*`.
****
