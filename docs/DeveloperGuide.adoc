...
The current `ExpenseChart` observes the list of expenses that are contained in the `MainWindow`. When a command which updates the expenses is executed,
that command updates the list of expenses in the model. That update propagates to the list of expenses in `MainWindow`, which notifies
the current chart of its changes.

Using a `StatisticsGenerator`, the `ExpenseChart` then generates the statistics to be displayed from the new expenses. In this manner,
