...
all donors via the `MatchCommand#match` operation. A `MatchedPatient` object will be created
for each patient, which stores the patient's attributes and the number of potential donors the patient has.
//add object diagram.

Step 3. The user types in the command `match ic/S7016432I`. Existing `MatchedPatient` or `MatchedDonor` objects
in the `listOfMatches` are removed. For each potential donors of the patient with the NRIC S7016432I, a `MatchedDonor`
object is created, accompanied with the compatibility rate.
