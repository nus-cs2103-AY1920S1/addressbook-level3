= CS2103/T Revision Tool - Testing
...
e.g. `seedu.revision.commons.StringUtilTest`
. _Integration tests_ that are checking the integration of multiple code units (those code units are assumed to be working). +
e.g. `seedu.revision.storage.StorageManagerTest`
...
e.g. `seedu.revision.logic.LogicManagerTest`
