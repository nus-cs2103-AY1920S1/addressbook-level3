...
Create a new `Remark` in `seedu.description.model.Problem`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Problem editedProblem = new Problem(problemToEdit.getName(), problemToEdit.getPhone(), problemToEdit.getEmail(),
                problemToEdit.getAddress(), remark, problemToEdit.getTags());
