...
5) The sort methods will be explained in detail below with some examples:

* `sortFilteredClaimListByName` is implemented with the help of a comparator that compares the descriptions of each claim
with `claim.getDescription()` method. Below is a code snippet that illustrates the comparator.
...
with `income.getDate().getLocalDate()` method. Below is a code snippet that illustrates the comparator.
