...
|=======
| `notif_on` | Red text in the User Guide indicates that a term is a command that can be executed by the program if input through the command line.
| `NotificationManager`| Red text in the Developer Guide indicates that a term is a class or an object used in the source code of the application.
|=======
