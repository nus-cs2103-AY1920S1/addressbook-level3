...
The redo command allows user to `return to the original state before the latest undo is called`.
...
`CommandHistory` is a `static` class that contains the undoEventStack and redoEventStack, each `containing Events`.
...
*Step 2.* `EventFactory` will parse the `UndoableCommand` to generate an Event. +
...
*Step 3.* `Event` is added to `undoEventStack` stored in CommandHistory. The `redoEventStack` in
CommandHistory is also `cleared` upon generating a new Event.

*Step 4.* The `UndoableCommand` is executed, generating the `CommandResult`.

*Step 5.* To undo the previous UndoableCommand, the user executes `undo` command. An `UndoCommand` is generated.

*Step 6.* `UndoCommand is not an UndoableCommand`. Executing the UndoCommand calls the undo method of `Event`
stored in the undoEventStack of CommandHistory.

[NOTE]
Both UndoCommand and RedoCommand are an UndoableCommands, no Events are generated.

*Step 7.* The `Event` is popped from the undoEventStack and pushed to redoEventStack in CommandHistory. `The Event is undone`.

*Step 8.* To redo the command that has been undone, the user executes `redo` command. A `RedoCommand` is generated.
This execution is `similar to steps 6 and 7`, except Event is popped from redo stack instead and pushed to undo stack.

The following two sequence diagram shows how the user's input is handled for `'delete activity 1'`.

The first diagram below shows the execution of `delete activity 1` command. It shows `Event` generated and how the
...
[NOTE]
When DeleteActivityEvent is executed, it generates the `activityToDelete` by extracting the `Activity` to be deleted from
...
[NOTE]
A Command that is not a UndoableCommand, UndoCommand or RedoCommand will not affect the stack of Events in CommandHistory. +
...
*** Convenient for future extensions for new Commands added. Just need to ensure for every UndoableCommand, there must a Command
that is able to revert its effects (undo).
...
** Cons:
*** Every UndoableCommand requires a Command to reverse its effects. Hence some Commands created are not utilized
by the user, but only used within application.
