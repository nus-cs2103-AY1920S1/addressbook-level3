...
e.g. rename the tag `friends` to `buddies` (i.e. all students who had the `friends` tag will now have
...
command types that mutate the data. e.g. `AddStudentCommand`
...
----
+
The above method can be used to get a printable string representing a bunch of students details.
For example, you should be able to call that method like this:
+
[source,java]
----
// p is a Student object
...
* Add a `throws Exception` clause to the `AddStudentCommand::execute` method. Notice how Java compiler will not allow it,
