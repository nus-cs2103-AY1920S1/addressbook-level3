...
The `ackAppointment` command will mark the patient's the most upcoming appointment as Acknowledge when the patient is reaching and it will update `UniqueEventList` to display the rest appointments that are belong to the patient.
...
The `missAppointment` command displays the missed appointments. The missed appointments are found in `ModelManager`. The list is instantiated by filtering the `UniqueEventList` using `EventsMissedPredicate` which checks all `APPROVED`-Status appointments ending times with current time.

==== `settleappt` Command

The `settleappt` command removes a missed appointment. It updates `UniqueEventList` to display the rest of the missing appointments.
