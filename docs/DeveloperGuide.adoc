...
A `CommandResult` object now has an added boolean "unknown" instance variable to indicate if the command entered is
an unknown command. Along with this, TutorAid shifts between "learner" mode and normal mode by the following implementation:
[source,java]
----
public class MainWindow extends UiPart<Stage> {
    ...
    private CommandResult executeCommand(String commandText) throws CommandException, ParseException {
        try {
            ...
            //currently in "learner" mode and a known command is entered. TutorAid goes back to normal mode.
            if (this.unknown) {
                if (!commandResult.isUnknown()) {
                    //unknown instance variable set to false
                    setUnknownFalse();
                }
            }
            //unknown command entered, TutorAid goes into "learner" mode
            if (commandResult.isUnknown()) {
                //unknown instance variable set to true
                setUnknownTrue();
                }
            //..other logic
...
The whole mechanism is largely facilitated by `TutorAidParser` as well which has been modified to suit our needs. A TreeMap
is now being used in the process of parsing commands.

New command classes such as `UnknownCommand`, `NewCommand` and `CancelCommand` were also created.
...
Step 3. The `UnknownCommand` is executed by `Logic` and "plus" is saved on `savedCommand` in the `Model` component. Subsequently, the `CommandResult` is passed to `MainWindow` to display the corresponding text on the GUI. The `CommandResult` triggers `MainWindow#setUnknownTrue()` and sets TutorAid into "learner" mode via the earlier explained implementation.

image::customStarting.png[width="800"]
...
Step 4. TutorAid prompts the user to type in a valid command to map the unknown command to. The user types in another wrong command that is not recognised. This time, he enters "ad" instead of "add". Since `isUnknown` is now passed into `Logic#execute(String userInput, boolean isUnknown)` with a value of `true`, the method `TutorAidParser#checkCommand(String userInput, String prevUnknownCommand)` is now called instead of `TutorAidParser#parseCommand(String userInput)`.
...
* **Alternative 1 (current choice):** Use a list to store the history of tutor aid states.
** Pros: Easy for new Computer Science student undergraduates to understand, who are likely to be the new incoming developers of our project.
** Cons: Logic is duplicated twice. For example, when a new command is executed, we must remember to update both `HistoryManager` and `VersionedTutorAid`.
