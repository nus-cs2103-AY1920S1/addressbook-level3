...
The visual representation of a schedule is created in the `ScheduleView` class. The model below illustrates the purpose of the `ScheduleView` class.

image::./yijie/ScheduleViewDomainModel.png[]

In order to control the schedule created, an abstract class `ScheduleViewManager` is needed to control the creation of `ScheduleView` objects.
...
Step 1. The user wants to view a group called "Three musketeers" consisting of 3 members in TimeBook and executes the command
...
executes the `lookat` command to inspect some of his group member's schedules. The state of `ScheduleViewManager` is once again modified to only show the specified group members' schedules.
//IMAGE3
