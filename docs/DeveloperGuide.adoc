...
It is a living document that evolves throughout the design
and implementation for each release. Each release will have an edition of the document, and the
current edition of the document is for the first public release of the application.
...
In particular, the intended audience of this document is the students taking the roles of the developers, designers, and
software testers of _PalPay_ from CS2103T - Software Engineering.

=== Description
_PalPay_ is a CLI application targeted for users who have poor financial management skills.
It allows the users to keep track of daily financial transactions, as well as set a budget for a time duration
...
* exposes an unmodifiable `ObservableList<BankOperation>` and `ObservableList<Budget>`
    ...
    so that the UI automatically updates when the data in the list change due to user command.
...
// tag::transaction[]
=== [Proposed] Transaction: `in`/`out`

This feature allows user to input income or expense statements into the PalPay.
...
`Transaction` stores an *amount*, *date* and a set of *categories* which describes the variables a `Transaction` with the `BankAccount` will typically hold.
...
`inTransaction` and `outTransaction` can be referenced by making a call to `BankAccountOperation`, this `BankAccountOperation` is then passed as an argument to `Logic` components and multiple CRUD commands. This allows multiple transaction calls within the same method.

// end::transaction[]

// tag::sorttransactions[]
=== Sort Transactions Feature: `sort`
...
The `sort` command is facilitated by the _Logic_ and _Model_ components of the application, _PalPay_.
...
Given a list of *shares* and *people*, each person is assigned an *amount* based on the corresponding positional share and
the total amount given to `split` command. +
A `LendMoney` instance is created for each person and executed.

// TODO: add PUML figure here
...
The user is allowed to set multiple budgets.
...
Upon setting a new budget, a `BudgetCard` is created and displayed in a list in the application window
till the date set by the user. +
...
This feature allows users to _project_ their balance status based on past income and outflows
as manifest in their _TransactionHistory_ by using the command `project [DATE]`.
...
In future updates, the _user's_ balance projection will be displayed as a linear graph plotted against time, up until
the date specified by the user in the command.

===== Projection by Category
In future updates, the _user_ will be able to customise projections beyond specifying a single date.
Some possible customizations include solely projection _income_ / _outflow_ amounts, and filtered
projections based on _categories_. For instance, `project d/22112019 c/Food -o` will project the
_user's_ spending (or outflow, as denoted by the `-o` flag) on Food (specified by the `c/Food` tag)
up until the 22nd of November, 2019 (specified by the `d/22112019` tag).
...
_Projected Balance = Current Balance - [(Past Income - Past Outflow) / Total Days Elapsed] *
Days Until Date of Projection_

While this algorithm allows for simple and fast computation, it does not hedge against one-time big purchases.
For example, if the user were to buy a computer, his outflow projection is likely to be proportionally
...
algorithm in finding a best fit projection line for the user's balance. However, this will increase the
computational cost of the `project` function significantly, causing _PalPay_ to run slower overall.
