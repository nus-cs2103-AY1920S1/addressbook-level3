...
* For parameters requiring `index`, `index` refers to the index number shown in the displayed eatery or review list. The index *must be a positive integer* 1, 2, 3, ... .
====
...
Imports eateries from a json file. +
Format: `import \u [filename]`

****
* Imports eatery data from the specified json file.
* The user can type in the filename without the ".json" extension, however, the file *must* still be a json file.
****
...
Closes an eatery if the eatery no longer exists. The eatery will still be listed, but will be highlighted in red to inform you that the eatery no longer exists. In the case that you accidentally closed the wrong eatery or the eatery reopens, typing `reopen` does the opposite of `close`. +
Format: `close [index]`
...
* The index refers to the index number shown in the displayed eatery list.
...
****
* Deletes the eatery at the specified index.
****
...
If you are not connected to the Internet, you can still add the feed but feed posts will only be fetched when the app is restarted with Internet connectivity.
****
