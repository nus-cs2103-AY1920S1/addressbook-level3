...
The Remove Deadline command `Remove` works in a similar way, and hence the Sequence and Activity diagrams
will be omitted for redundancy.
...
In addition, two Exception classes are added to handle possible runtime errors while adding new Deadlines.
This is done to ensure that the user specified Deadline is valid for storage and processing.
...
The user executes `bad 2` to rate the second `FlashCard` as a "Bad".

The following activity diagram summarizes what happens when a user executes a `bad` command:

image::DeadlineActivityDiagram.png[width=320,height=480]

==== Design Considerations
===== Aspect: How to implement Bad FlashCard feature

* **Alternative 1(current choice):** Use command `bad` together with an `Index` value at the end
** Pros: Easier to implement
** Cons: User might be unclear which `FlashCard` was chosen without a clear indication
* **Alternative 2:** Make use of the GUI. After user execute `bad` command,
a **bad FlashCard window** would pop up for the user to select specific `FlashCard`
** Pros: More user friendly
** Cons: Difficult to implement. Might be slower for the user to key in deadlines as compared to using a one liner with prefixes.
...
==== Proposed Enhancement for v2.0
===== Improvement on Spaced Repetition
