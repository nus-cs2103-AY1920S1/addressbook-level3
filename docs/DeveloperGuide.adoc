...
represented as `Tags` and `BookmarkPredicates` respectively. The class diagram for the Autotag package illustrates this:
...
These `Model` operations allow autotags to be added, edited, or removed by commands in the `Logic` component.
...
.The state of Mark after an autotag has been added
...
.The state of Mark after a new bookmark has been added and automatically tagged
...
Since it does not, the `tagger` is added and all taggers are applied to the bookmarks in `model`.
...
As seen above, the `Model` calls `VersionedMark`, which then obtains its `list` of bookmarks
and passes the list to `AutotagController`.
`AutotagController` iterates through the bookmarks and taggers, applying tags to bookmarks by using
