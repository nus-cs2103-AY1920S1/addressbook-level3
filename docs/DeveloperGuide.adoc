...
three different ways - by date modified, date added or number of access. Below is a
...
More than one of the conditions can be used to sort the notes in the note book at one time, with the leftmost condition
having the greatest precedence. This is handled by the `MultipleSortByCond#buildComparator()`, which takes in the string
of conditions specified by the user and returns a `MultipleCondNoteComparator` object that is used to sort the list of notes
in NoteBook.
