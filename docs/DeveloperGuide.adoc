...
==== Implementation
The Appointments feature comprises of Reminders and Follow-Ups. Both mechanisms are facilitated by the `AppointmentTable` class, with individual Appointment data stored in the `Appointment` class.

It implements the following operations:

* `AppointmentTable#addAppointment(appointment)` -- Adds a new appointment.
* `AppointmentTable#outputAppointments()` -- Retrieves the appointments for output.

These operations are exposed in the `Model` interface as `Model#addAppointment(reminder)` and `Model#outputAppointments()` respectively.
