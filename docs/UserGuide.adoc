...
Nobody likes to read long documents, especially when you have something as exciting as _TravEzy_ to use. So, we have organised this document such that you can easily look for what *you* need. 
...
Trying to open up the achievements page? Use this command!

*Format:* +
``goto PAGE TYPE`` +
{empty} + 
Note that the only available ``PAGE TYPE``s are ``calendar``, ``itinerary``, ``address_book``, ``financial_tracker``, ``diary`` and ``achievements``.
...
Step 1. Type ``goto itinerary`` in the _command box_ and click _Enter_. +
...
Step 1. Type ``help`` in the _command box_ and click _Enter_. + 
...
Step 1. Type ``exit`` in the _command box_ and click _Enter_. + 
