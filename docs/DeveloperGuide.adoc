...
For the components UI, Logic, Model and Game:
...
_Due to the fact that this component has to work closely with the `UI` and `AppManager` (without being
coupled directly), it is separated from the `Logic`, `Model` and `Game` components._ Its roles include:


* Dealing with the countdown timer that runs during a game session.
* Periodically triggering _callbacks_ that will notify either the `UI` or `AppManager` components.
...
// tag::undoredo[]
=== AutoComplete feature for command execution
==== Implementation
...
The following activity diagram summarizes what happens when a user types a keystroke:
