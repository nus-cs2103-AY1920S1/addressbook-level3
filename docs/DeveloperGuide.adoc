...
=== EditMode
A Restaurant object contains many attributes, including a List of Food items as its menu,
which makes it difficult to edit an entire Restaurant object using 1 command line.
This feature allows editing of a specific Restaurant's details and Food menu under 1 mode, through different commands.

==== Implementation
* **Model**:
ModelManager now contains a editingRestaurantList in addition to the filteredRestaurantList, which stores
and allows access to the Restaurant currently under EditMode. Upon calling of edit command, the restaurant
referenced by the index will be placed in the editingRestaurantList.

* **Logic**:
The Logic for EditMode is facilitated by `Context` enum type.
A different Parser will parse the userInput based on the current context.

UserInput will first be parsed by the universal parser first. If command word in userinput matches
...
* **UI**:
Current context will be passed to UI by the command result returned by `editmode` command.
...
|`* * *` |Administrator |add or remove menu items from restaurant menus |restaurants can adjust their menus whenever without any delay

|`* *` |Administrator |sort restaurant by popularity/ food type/ availability |recommend restaurants to customers
