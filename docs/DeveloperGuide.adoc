...
shifted to the replenish list. The item shift is demonstrated in the sequence diagram above.
...
* **Alternative 1 (current choice): Automatically shifts item to the replenish list when item quantity reaches 0**
...
** Pros: User will not have any unwanted items in the replenish list.
** Cons: User has to go through the trouble key in the details of an item that has been in the main list once again should the user want the item in the replenish list.
...
An item with `name` Jelly and `expiryDate` 24/10/2019 (which has passed) is now added to the json file for your testing.
...
.. Prerequisites: Current view is the main list. The list contains at least one expired item.
.. Test case `check|0` +
