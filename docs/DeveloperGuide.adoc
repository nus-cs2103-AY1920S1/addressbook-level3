...
=== Budget Feature
The `Budget` component has two core feature:

1. Allowing the user to set the monthly budget
2. Calculating the remanding budget after all the spending in the same month.

The budget component allows the users to modify the current monthly budget `budget AMOUNT` command.
...
Upon initialization MoneyGoWhere will check the current date. If the current month is different from the current month
...
|===
The second option is chosen to prevent circular dependency.
