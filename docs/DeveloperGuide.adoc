...
1. User first enters the command `stats d/01/01/2019 d/07/01/2019`.
...
4. Finally, the `statsData` is returned to the `MainWindow`. The data will be displayed on the Ui through `StatsWindow#loadData(statsData)` call.
...
During the preparation phase, the program parses the command for Graph and prepares to open a new Graph Window.
...
1. User first enters the command `graph d/01/01/2019 d/07/01/2019`.
...
.. Else, a `ParseException` will be thrown and an error message will be shown to the user.
4. `GraphCommand` is returned to `LogicManager` which then calls `GraphCommand#execute()`, which updates the `graphPredicate` in `Model`.
...
image::GraphDataSequenceDiagram.png[]
...
4. Finally, the `graphData` is returned to the `MainWindow`. The data will be displayed on the Ui through `GraphWindow#loadData(graphData)` call.
...
|How to handle graph data and parameters
|Data and each parameter is handled as separate objects +
Pros: Easy to implement +
Cons: Need to call multiple methods to get parameters
|Create Graph object which contains data and all the parameters +
Pros: More scalable. Less method calls to get parameters +
Cons: None.
