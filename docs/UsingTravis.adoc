...
* Runs the `./gradlew clean test coverage coveralls -i` command (see <<UsingGradle#, UsingGradle>> for more detail on what this command means).
...
.  Go to https://travis-ci.org/ and click `Sign in with GitHub`, then enter your GitHub account detail if needed.
+
...
+
.  If the build is successful, you should be able to check the coverage detail of the tests at http://coveralls.io/[Coveralls]
