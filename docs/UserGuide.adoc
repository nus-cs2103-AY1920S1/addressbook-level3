...
- `TaskPlace` Where the task took place. Prefix: `<place>`. +
...
Format: `add <title>TITLE <day>DAY <desc>DESCRIPTION <deadline>DEADLINE <place>PLACE [<tag>TAG]` +
e.g. `add <title>CS2109 Lab 10 <day>tuesday <desc>Submit code through LumiNUS <deadline>21-10-2019
<place>NUS <tag>IMPORTANT`

==== Edit existing task: `edit`
Edit one or multiple fields of an existing task. `INDEX` should be a positive integer.

Format: `edit  INDEX [<title>TITLE] [<day>TIME] [<desc>DESCRIPTION] [<deadline>DEADLINE] [<place>PLACE] [<tag>TAG]` +
