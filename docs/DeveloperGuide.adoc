...
Current, the itinerary feature supports the basic commands of add, delete and summary.
...
Inputs given by the user are channeled from the text UI and parsed by Command Parser before different commands are formed which generates
the model and updates the FinancialTracker object which contains the expense list.

The text UI, log centre and storage all stem from the common package of the main TravEzy application.
However, in the parser package of itinerary, it contains various parser objects for the different command.
