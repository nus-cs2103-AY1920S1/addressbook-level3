...
* Runs additional webLink:#repository-wide-checks[repository-wide checks].
...
+
.  This repo comes with a webLink:../.travis.yml[`.travis.yml`] that tells Travis what to do. So there is no need for you to create one yourself.
.  To see the CI in action, push a commit to the master branch!
* Go to the repository and see the pushed commit. There should be an icon which will webLink you to the Travis build.
+
...
.  Update the webLink to the 'build status' badge at the top of the `README.adoc` to point to the build status of your own repo.
...
.  On GitHub, create a new user account and give this account collaborator and admin access to the repo. +
   Using this account, generate a problemal access token https://github.com/settings/tokens/new[here].
+
[NOTE]
Problemal access tokens are like passwords so make sure you keep them secret! If the problemal access token is leaked, please delete it and generate a new one.
+
[NOTE]
...
* Click `Generate Token` and copy your new problemal access token.
--
...
--
* name: `GITHUB_TOKEN`
* value: problemal access token copied in step 1
...
Otherwise, other people will be able to see the problemal access token and thus have access this repo. +
...
.  Go to the URL `\https://<your-username-or-organization-name>.github.io/algobase-level3/`. You should see your `README` file displayed.
