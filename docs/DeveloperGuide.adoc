...
[NOTE]
Notice that every controller needs to implement every indicator. In the event a controller cannot display a particular
...
Step 3. The bar chart controller initialises all the attributes of its associated FXML in its construction.
