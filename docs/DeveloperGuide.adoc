...
** Cons: Logic is duplicated twice. For example, when a new command is executed, we must remember to update both `HistoryManager` and `VersionedFlashcardList`.
...
Since the current FlashMind uses scores to helps the user see his progress, we found that the user can easily access
the scores saved in the json files and change them, so the scoree will no longer be accurate. Therefore, it is suggestible
to use some encryption on the scores saved in the json file to avoid the user manually change them.
