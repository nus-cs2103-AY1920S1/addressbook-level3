...
==== 2. Add a person to your contacts: `add`
...
==== 3.1 Add a new person to your contacts and project: `addMember` [Checkout]

This command is similar to adding a person to your contacts shown previously, except that you have to
...
==== 3.2 Add a person from contacts to a project: `addFromContacts` [Checkout]
...
`INDEX` is the number which the person is listed at, and it should be a postive integer eg. 1, 2, 3, ....

Example:

* `addFromContacts 1`

==== 4. Add a timetable for a person: `addTimeTable`
...
==== 5. Add a task to your project: `addTask` [Checkout]
...
==== 1. Deleting a person from contacts : `delete`
...
==== 2. Removing a member from a project: `removeMember` [Checkout]
...
==== 3. Deleting a task from a project: `deleteTask` [Checkout]
...
==== 1. Finding a person by name: `find`
...
`INDEX` in this case refers to which type of sorting you want to do. (Refer to the highlighted section above)

Example:

* `sortTask 1` (Sorts the tasks by alphabetical order)
