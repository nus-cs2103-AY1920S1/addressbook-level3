...
import static seedu.tarence.logic.commands.event.DeleteEventCommand.MESSAGE_DELETE_EVENT_SUCCESS;
...
public class DeleteEventCommandTest {
    ...
    @Test
    void execute_validEvent_eventDeleted() throws CommandException {
        DeleteEventCommand deleteEventCommand = new DeleteEventCommand(new ModCode(VALID_MOD_CODE),
                new TutName(VALID_TUT_NAME), null, Index.fromOneBased(1), null, null, null);

        String expectedMessage = String.format(MESSAGE_DELETE_EVENT_SUCCESS, VALID_EVENT_NAME);

        assertEquals(new CommandResult(expectedMessage), deleteEventCommand.execute(model));
    }

    @Test
    void execute_invalidTutorial_throwsCommandException() {
        DeleteEventCommand deleteEventCommand = new DeleteEventCommand(new ModCode(OTHER_MOD_CODE),
                new TutName(VALID_TUT_NAME), null, Index.fromOneBased(1), null, null, null);

        assertThrows(CommandException.class,
                MESSAGE_INVALID_TUTORIAL_IN_MODULE, () -> deleteEventCommand.execute(model));
    }

    @Test
    void execute_similarTutorial_suggestSimilarCommands() throws CommandException {
        DeleteEventCommand deleteEventCommand = new DeleteEventCommand(new ModCode(SIMILAR_MOD_CODE),
                new TutName(VALID_TUT_NAME), null, Index.fromOneBased(1), null, null, null);
