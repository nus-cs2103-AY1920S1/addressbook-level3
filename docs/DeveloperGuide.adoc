...
. There exists 2 decks: Deck1 and Deck 2.

. The user wishes to delete Deck 1, but accidentally deletes Deck 2.

. In order to recover Deck 2, the user types in the command `undo` into the CLI.
...
4a) The user decides that they prefer Deck2 to be deleted.
    4ai) The user types 'redo' into the CLI.
    4aii) Deck2 is deleted again.
