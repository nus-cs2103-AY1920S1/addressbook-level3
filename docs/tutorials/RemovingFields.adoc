...
The `address` field in `Person` is actually an instance of the `seedu.address.model.book.Address` class.
...
{
  "books": [ {
    "title": "Person with invalid title field: Ha!ns Mu@ster",
    "serialNumber": "9482424",
    "author": "hans@example.com",
    "address": "4th street"
  } ]
}
```
