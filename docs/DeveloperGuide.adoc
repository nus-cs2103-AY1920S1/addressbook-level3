...
. 1b. Ineterval between notificaitons larger than period.
...
. 1b. User tries to set frequency/ period for a General Reminder
  . 1b1. GuiltTrip notifies user that reminder can only be created for events after today.
. 1c. Ineterval between notificaitons larger than period.
  . 1c1. GuiltTrip notifies user that frequency must be smaller than period.
