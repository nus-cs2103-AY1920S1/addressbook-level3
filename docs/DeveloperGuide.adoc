...
_Activity diagram of the actions performed_
...
The Statistics feature is facilitated by the Statistics class.
It takes a HashMap<String, HashMap<String, Double>> type data that matches the students name to their respective subject-score mappings and generates all relevant data available in the report. It implements the following methods.

* `Statistics#initializeData()` -- Sets all relevant counters to their initial values (e.g. zero for score ranges).
* `Statistics#processData()` -- Assigns all relevant statistics summary, such as number of data entries, min, max, mean, median and standard deviation.
* `Statistics#allocateDistribution ()` -- Counts the number of people of who got a particular score.
* `Statistics#sortIntoGrade ()` -- Counts the number of people of who got a particular range of scores.

These data are encapsulated in the StatisticsRecord, which is carried by the Model.

Given below is an example usage scenario and how the statistics are generated at each step.

Step 1. The user launches the application for the first time. Model currently carries sample statistics data from the SampleStatisticsUtil class. The user types in a command requesting for generation of statistics report. (statistics file/{filePath})

image::UndoRedoState0.png[]

Step 2. The parser checks for syntax errors and validity of file format. If the file is in expected format (specified in the user guide), a StatisticsCommand is created with unprocessed data passed as arguments into Statistics object, and the Statistics object as arguments of a StatisticsAddCommand object.

image::UndoRedoState1.png[]

Step 3. The Statistics object performs data processing to be used later in the report. `StatisticsAddCommand#execute()` is invoked, which replaces the sample statistics data in the model to the one generated by the input file. Success message is returned for rendering in the User Interface. Model has updated, processed statistics.

image::UndoRedoState2.png[]

Step 4. If the parsing of input file was successful, ‘MainWindow#handleStats()’ opens a new Stage object to render the generated report.

image::UndoRedoState3.png[]

[NOTE]
If the input file was not found or in invalid format, an error message is shown in the result box and no new window is opened.
...
// end::Generating Statistics Feature[]

=== Event Mangagement Feature
==== Implementation
...
* Creating an event
** Events can recur daily, weekly or none
** Events can be grouped by their own color
* Deleting an event
* Editing an event
* Switching between daily and weekly view in the calendar
* Change calendar range
* All event details are stored and persisted
