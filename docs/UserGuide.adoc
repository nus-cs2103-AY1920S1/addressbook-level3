...
* **`assign ti/1 mi/AS`** : Assigns the project member "Adam Smith" (With member id "AS"), to the task "Finish up milestone setting".
...
enter the command `set-image mi/NM im/C:\Users\Lynn\Desktop\Y2S1\CS2103T\tP\NewUserImage.png` into the command prompt box.
...
[IMPORTANT]
If you shift the image's location in your computer, +Work will be unable to find the image to display. It
is recommended that you store all the images in a central folder to prevent this from happening.
...
as well as the time taken for each inputted tasks in +Work.
