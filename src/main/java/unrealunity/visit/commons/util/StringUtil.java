...
import static unrealunity.visit.commons.util.AppUtil.checkArgument;
...
public class StringUtil {
     ...
     * @param sentence cannot be null
     * @param word cannot be null, cannot be empty, must be a single word
     * @return boolean for if the sentence contains the word query
     */
    public static boolean containsPartialWordIgnoreCase(String sentence, String word) {
        requireNonNull(sentence);
        requireNonNull(word);

        String preppedWord = word.trim(); // Eliminate leading and trailing spaces
        checkArgument(!preppedWord.isEmpty(), "Word parameter cannot be empty");
