...
* contains an underlying unmodifiable `ObservableList<Card>` and `ObservableList<WordBank` structure that can be
'observed'. The `UI` is bounded to this two list so that the UI automatically updates when the data in the list change.
...
Let me first introduce you how these commands are structured in `Logic`. +
...
To segregate cards according to their function, I have the following:
