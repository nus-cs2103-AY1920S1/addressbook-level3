...
The following diagram illustrates the class diagram of the most important part of the calendar model. Specifically, it focuses on the ``Event`` package which can be found https://github.com/AY1920S1-CS2103T-T17-2/main/tree/master/src/main/java/seedu/address/calendar/model/event[here].
...
* ``IntervalSearchTree`` makes certain user-requested operations more efficent computationally. However, to facilliate the use of ``IntervalSearchTree``, ``Event`` and ``EventType`` need to inherit from ``Interval``. 

To find out how ``Calendar``, ``EventManager`` and ``IntervalSearchTree`` described above interact, consider the following sequence diagram. It descibes how these classes work together when the user tries to add a holiday, which is represented by ``event`` (a ``Holiday`` instance) in the diagram below. 
...
a|The usual date format like dd/mm/yy (for example, 12/05/2019 to specify 12 May 2019)

* Pros: Easy to implement and is similar to the required date format in most online form
...
* Pros: More flexible. Furthermore, it can be implementd in a way that allows users to leave out the month and year information if they are referring to the current month and year. 
