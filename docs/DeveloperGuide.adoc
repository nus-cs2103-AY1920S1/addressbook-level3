...
.. Test case: `delete_em 1` +
   Expected: Employee of index '1' is deleted from the list.
...
=== Auto-Allocation of employees to an event
. Allocating employees to a displayed Event.

.. Prerequisites:
1. User is in `Main` or `Schedule` Tab.
2. There are at least two Events in the displayed list.
3. There are sufficient available employees for the Event.
4. Chosen Event still requires manpower.
.. Test case: `allocate 1` +
    Expected: Employees with non-conflicting schedules are allocated to the
1st Event displayed in the list. Number of employees allocated is based on demand of event.
.. Test case: `allocate 1 t/male` +
    Expected: Employees, tagged [male] with non-conflicting schedules are allocated to the
1st Event displayed in the list. Number of employees allocated is based on demand of event.
.. Test case: `allocate 2 n/1` +
    Expected: 1 Employee with non-conflicting schedule is allocated to the
2nd Event displayed in the list.
.. Test case: `allocate 2 n/1 t/male` +
    Expected: 1 Employee, tagged [male], with non-conflicting schedules are allocated to the
2nd Event displayed in the list.

.. Test case: `allocate 0` +
    Expected: No employee is allocated to event.
Error details shown in the status message.
.. Other incorrect auto-allocate commands to try: `allocate`, `allocate x` (where x is larger than the list size) +
   Expected: Similar to previous.
