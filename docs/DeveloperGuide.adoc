...
**Step 5**. The command checks if supply (generated in *step 4*) exceeds
demand (generated in *step 3*) of the event.
[NOTE]
...
**Step 6**. The command calls `Event#createEventAfterManpowerAllocation()` to create a new event with an updated
manpower list.
[NOTE]
...
specific events that are happening on those dates listed. Do note that the `generate_schedule` command will only display dates and events that have a set date and time allocated to it.
...
process through these events to create specific `DistinctDate` Objects which stores a list of events that occurs on the date
...
** makes more logical sense and will be more efficient as compared to Alternative 2. Alternative 1 requires less intermediate processing and storage units to support the feature.
...
** is a cleaner and more user-friendly approach compared to Alternative 1. Alternative 2 helps to prevent the Schedule Tab from being filled with too many lists and information.
