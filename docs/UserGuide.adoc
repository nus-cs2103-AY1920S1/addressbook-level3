...
****
Note the double `/` near the prefix `p/`.
This is because the examples given in this guide use MacOS absolute file paths, which start with a `/`.
Therefore, the first `/` is part of the argument prefix, whereas the second `/` is part of the file path.
If you are using MacOS or *nix systems, both `/` s must be present for the command to succeed.
...
To import a directory of images, first load the directory into the the import staging area.
You can then edit or delete memes in the staging area before actually importing them into Weme.

==== Loading memes into the import staging area: `load`

Loads memes from a specified directory into the import staging area. +
Format: `load p/PATH`
...
==== Editing a meme in the import staging area: `edit`

Edits a meme from the import staging area. This allows the user
to make changes before actually importing the meme. +
Format: `edit INDEX [d/DESCRIPTION] [t/TAG]...`

==== Deleting a meme: `delete`

Deletes an unwanted meme from the import staging area. +
Format: `delete INDEX`

==== Importing memes: `import`

Imports all the memes from the import staging area into Weme. +
Format: `import`

==== Clearing loaded memes: `clear`

Clears all the memes in the import staging area. +
Format: `clear`
