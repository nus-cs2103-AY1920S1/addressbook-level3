...
The `missappt` command searches for appointments that the patient did not attend. The filtered appointments are found in ModelManager. The list is instantiated by filtering the UniqueAppointmentList using EventsMissedPredicate which checks all APPROVED-Status appointments' ending times with current time.
...
In future implementations, i.e. v2.0, the valid timing slot will be given after checking with doctor's duty shift and Model#UniqueAppointmentList. This ensures users can easily make appointments for patients.
