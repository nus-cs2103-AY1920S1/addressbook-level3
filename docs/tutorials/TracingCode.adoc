...
Recall from the User Guide that the `edit` command has the format: `edit INDEX [n/NAME] [p/PRICE] [e/EMAIL]
...
command and store any possible changes in an `EditFoodDescriptor`. Recall that we can verify the contents of
`editfoodDesciptor` through the `Variable` tool window.
+
...
----
. It appears that a `JsonAdaptedfood` is created for each `food` and then added to the
