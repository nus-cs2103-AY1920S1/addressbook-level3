...
===== Implementation
As shown in  <<Logic component,Section 2.3>>, both `XpireParser` and `ReplenishParser` implement the interface `Parser`.
...
that both implement `SortedUniqueItemList`, in a relationship summarised in  <<Model component,Section 2.4>>.
...
On the other hand, the figure below presents what happens when a user executes a command with invalid arguments.
...
The sequence diagram titled `find similar words` here expands on the process omitted above.
