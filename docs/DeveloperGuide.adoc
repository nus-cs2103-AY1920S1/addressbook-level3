...
that implements the https://en.wikipedia.org/wiki/Levenshtein_distance[Levenshtein distance formula] with a threshold of +++<u>40 percent</u>+++ (See <<Feature-Question-Design-Similarity>>).
...
*Step *. A new `CommandResult` will be returned with a message stating the search term and the number of results returned.
