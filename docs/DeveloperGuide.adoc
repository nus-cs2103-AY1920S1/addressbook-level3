...
`annotate`, `annotate-delete` and `annotate-edit` make up the _family of annotation commands_.
...
Subsequently, when necessary the appropriate `Paragraph`(s) is retrieved, and the annotation is handled according to the command.
...
.Sequence diagram showing how annotation commands generally work.
//TODO: sd - general for annotation; when done make this line newline
...
When adding an annotati

image::AddAnnotationExecuteSequenceDiagram.png[]
