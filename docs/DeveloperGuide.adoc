...
These actions is facilitated by the `ArchiveWrapper` and `Archive` classes:
...
Given below is an example usage scenario of the add expense to archive function, and how the archive model behaves at each step: +
...
Next, the user executes the command `archive add 3 arc/archiveName` to archive an expense. The command is first parsed by `BillboardParser` to determine what kind of command it is,
`archive` indicates it is an archive command, so the remaining command is parsed through `ArchiveCommandParser` to determine which archive command it is.
`add` indicates it is an add command, so it is parsed for the final time by `AddArchiveCommandParser` to extract out the arguments entered for the operation.
In this case, the arguments are `3` and `archiveName` +

`AddArchiveCommandParser` then returns a `AddArchiveCommand` object to be executed. The `AddArchiveCommand` performs two checks before executing any changes:
...
so the model is left unmodified.
