...
* Note the <<Constraints>> for *NAME*, *MANPOWER_NEEDED*, and *START/END_DATE*.
* Additionally, take note that the `START_DATE` must be before the `END_DATE`, and that the maximum date range of the event is *90 days*.
* An events's identity is uniquely identified by its `EVENT_NAME`, `START_DATE`, and `END_DATE`. You cannot edit an event to become one that is identical to another existing event inside the Event List.
* If there is manpower currently allocated to the event, the updated `MANPOWER_NEEDED` cannot be lesser than the current number of employees allocated.
* An attempt to modify the `START_DATE` and/or `END_DATE` requires the deallocation of all manpower first before edit.
...
complete employee list to an event. *AddMin+ prevents conflicting schedules by ensuring that
employees cannot be allocated to events with overlapping periods.*
