...
automatically upon the addition of a new transaction. This consists of allowing the user to add, list, edit, and delete rules
from the Rule Engine. The main part of my implementation however, comes from the parsing and processing of expressions that the
user has created, whenever a transaction is added or edited in Budget Buddy. This requires the extraction of relevant components
from the target transaction which will be used to carry out specific operations.

The following sections will elaborate on this feature, which includes the additions I have made to both the user and developer guides.
...
adding/editing a transaction. A rule may carry out an action depending on whether the transaction satisfies the rule's requirement,
or "predicate".
...
* *Code contributed*: [https://nus-cs2103-ay1920s1.github.io/tp-dashboard/#=undefined&search=tcgj[RepoSense Contribution]]
