...
The format for the `addnote` command is as follows:
 ...
 addnote c/<MODULE_CODE> type/<CLASS_TYPE> note/<CONTENT>
...
** `code` would use `ParserUtil#parseClassId()` to ensure that the module code entered by the user is in the correct format of
CSXXXX.

** `classtype` would use `ParserUtil#parseClassType()` to ensure that the type of class input by the user is in the correct format of
tut, lab, c, s.
...
image::AddNotesActivityDiagram.png[]
...
`AddNotesCommandParser#parse()`. In this case, it would use `ParserUtil#parseClassId()`, `ParserUtil#parseClassType()` and `ParserUtil#parseContent()`.
