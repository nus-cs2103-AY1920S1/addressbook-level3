...
// tag::budget[]
=== Budget Feature
The budget feature allows users to: (i) *add, delete and modify* budgets; (ii) monitor spending by the budget's
...
and all expenses from all budgets must add up to the total number in the general expense list.
Each expense keeps track of its own budget through budget name (of Description type) - this is made possible by
disallowing duplicate names in UniqueBudgetList. On the other hand, each budget keeps track of a list of expenses
added to this budget.

image::ExpenseAndBudgetRelationshipClassDiagram.png[]
...
As shown in the figure above, when user inputs "addbudget ...", LogicManager executes the string, MooLahParser creates the
corresponding CommandParser which parses the input into an AddBudgetCommand. AddBudgetCommand then validates itself by
checking if there is an identical budget already existing in MooLah. If no duplicate budgets are found, it executes the
command, adding the specified new budget to MooLah. After that, AddBudgetCommand creates a CommandResult and passes it
back to LogicManager. LogicManager then saves the updated MooLah as json file, and returns the CommandResult to GUI to
be displayed to user.

The implementation of other commands has similar flow as AddBudgetCommand, differing only in the parameters and corresponding
methods in ModelManager, MooLah and UniqueBudgetList. A noteworthy implementation in EditBudgetCommand is that it
has a inner class EditBudgetDescriptor to record the updated attributes of the budget, which is then used in the method
EditBudgetCommand#createEditedBudget(Budget, EditBudgetDescriptor) to create an edited budget, maintaining non-updated
attributes the same as the original budget.

When a budget is edited, the original budget's expenses will be transferred to the updated budget through
Budget#transferExpensesTo(Budget other). The transfer process does two things: (i) set the expenses's budget name
to the new budget name; (ii) add the expenses to the new budget's expense list. Similarly, when a budget is deleted,
...
If the user wants to track expenses under a different budget,
he switches to the target budget first, before adding the expense.
...
image::PrimaryBudget5.png[]

*Step 6.* The user adds the expense "movie", which is tracked under the current primary budget - default budget.

image::PrimaryBudget6.png[]

==== BudgetWindow and SwitchPeriodCommand

A budget is like a *sliding window* that moves along the time axis. The window has a fixed size determined by the
BudgetPeriod, an enum class having five values: DAY, WEEK, MONTH, YEAR, INFINITY (for Default Budget).
Only expenses within the current window are shown in the app. Every time
the budget refreshes, the window "slides" to the next period, starting with an empty screen that gradually gets
populated with newly added expenses over time. This is achieved through the BudgetWindow class.

Here is the class diagram of BudgetWindow:

image::BudgetWindowClassDiagram.png[]
...
budgeting period, which are shown in the current GUI screen.
...
BudgetWindow#normalize(Timestamp anchor) method. This method takes into account marginal cases such as months with 30
days, February, and leap year.
...
Nov 10. As a result, the current budgeting period will be Nov 5 - Dec 4. There are 1 expense "chicken rice" recorded
in this current period.

*Step 2.* The user wants to view expenses in September. As such, the user executes `switchperiod t/20-09`
(interchangeable with any other dates within Sep 5 - Oct 4). This command calls
`BudgetWindow#normalize(Timestamp anchor)`, which shifts the window's start and end dates to Sep 5 and Oct 4 respectively, while maintaining the fixed period - MONTH.
As a result, the user sees expenses tracked under the budget during this period.
...
(iii) BudgetPanel, which displays a BudgetCard at top representing the primary budget, and a list of expenses tracked
under this primary budget in this current budgeting period - called through "view primary budget"
...
LogicManager#recordFinalPrimaryBudgetStatus(). The two resulting arrays are then passed into
MainWindow#showWarningIfAny() for a comparision, and if any values have changed, MainWindow#showPopupMessage will show
the corresponding pop up notification to remind the user of the budget progress.
//diagram
//image::PercentageClassDiagram.png[]

==== Storage of Budgets
Budget objects are stored in Json format, through the JsonAdaptedBudget class. The json file has
the same properties as the budget, except: (i) it flattens the BudgetWindow field into start date, end date and period,
for clearer display; (ii) it stores a list of UniqueIdentifiers to expenses, instead of a list of JsonAdaptedExpenses.
...
when creating json files. Also, the resulting JsonAdaptedExpense file will not be too long, since the "budgetName"
property stores only the budget name instead of the full budget object.


===== Aspect: Can one Expense be tracked under multiple Budgets? Or under no budget at all?
...
* **Alternative 2:** Allow expenses with multiple budgets, or without any budget.
...
//===== Aspect: Should Budgets has proportionUsed field?
//and isPrimary field?
...
A `StatsDescriptor` object `desc`, eventually containing the start date and end date is created, and a `StatsCommand` object `cmd` will
be created with `desc`, which will be returned to the `LogicManager`.

NOTE: By right Steps 2 to 3 should be still described in this sequence, but the descriptor details should be abstracted from the main UML.

Step 4. The `LogicManager` calls `StatsCommand#execute()`, which calls `ModelManager#getPrimaryBudget()` to retrieve the primary budget to calculate statistics from.
The `PieChartStatistics` object `statistics`, will be created with `createPieChartStatistics', after which it will called its implemented method
`populate()` to fill in the `attributes` required by the `Statistics Panel`.

NOTE: As `populate()` is a method specified by the Statistics interface, need to show it in the main UML. It may be good to wrap

Step 5. `ModelManager` calls `setStatistics(statistics)` which updates the `Statistics` attribute in ModelManager.

Step 6. A `CommandResult` is then created and returned to `LogicManager`, which will then force a switch
...
The following reference diagram shows how `StatsCommandParser` handle optional start date and end date entries
before creating a `StatsCommand`. This is used mainly to illustrate the interactions in Steps 2 to 3.
...
When designing the interaction of `Statistics#calculateStatistics()` with `statsbasic`, decisions have to be made on the most
