...
public enum Responses {
    ...
    ADD_CARD_JS(
            ...
            i -> {
                ...
                String back = "";
                int priority = res.get(1).size() == 0 ? Priority
                        .LOW_PRIORITY :
    ...
    CAPTURE_JS_ASSERTS(
            RegexUtil.commandFormatRegex("asserts", new String[]{"assert/"}),
            new ResponseGroup[]{ResponseGroup.MAKE_JS},
            i -> {
                ArrayList<ArrayList<String>> res = RegexUtil.parseCommandFormat("asserts",
                        new String[]{"assert/"},
                        i);
                LogsCenter.getLogger(Responses.class).info("COMMAND: ADD_CARD_JS_CAPTURE_ASSERTS");

                // Checks if asserts are supplied
                boolean hasAsserts = res.get(0).size() == 1 && !res.get(0).get(0).isBlank();
                if (!hasAsserts) {
                    Consumers.doTask(ConsumerSchema.DISPLAY_MESSAGE, "You need to enter a test case!");
                    return true;
                }

                String asserts = res.get(0).get(0);
                if (asserts.startsWith("Enter your asserts below.")) {
                    asserts.replaceFirst("Enter your asserts below.", "");
                }
                if (asserts.isBlank()) {
                    Consumers.doTask(ConsumerSchema.DISPLAY_MESSAGE, "You need to enter a test case!");
                    return true;
                ...
                StateHolder.getState().getAddToThis().addNewCard(transientFromState);
                Consumers.doTask(ConsumerSchema.DISPLAY_DECKS, true);
                StateHolder.getState().setCurrState(StateEnum.DEFAULT);
                return true;
            }
