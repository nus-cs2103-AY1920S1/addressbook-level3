...
After the parameters have been parsed, the relavant `CommandResult` object will be instantiated based on the input `Type` and pertaining to the input `SecondaryCommand`. 
...
We have considered between two differing help philosiphies.
...
|=======================
* We have settled on adopting the multi-faceted help philosiphy as the current implementation of help simply does not cut it. Measures have been taken such as allowing the user to type in `help` with no additional
