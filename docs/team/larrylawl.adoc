...
The user interacts with it using a CLI, and it has a GUI created with JavaFX. It is written in Java, and has about 80 kLoC.
...
** *Highlights:*
*** The `display` mechanism follows the MVC framework. It also uses an OOP solution, which features the use of abstract classes to reuse code.
...
ifndef::env-github[]
image::PolicyPopBreakdown.png[width="800"]
endif::[]
...
*** IntelliJ to auto format based on `checkstyle.xml`. (details below)
** Project Management:
