...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Address Book`, which `food` can reference. This would allow `Address Book` to only require one `Tag` object per unique `Tag`, instead of each `food` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
...
|`* * *` |user |delete a food |remove entries that I no longer need

|`* * *` |user |find a food by name |locate details of foods without having to go through the entire list
...
1.  User requests to list foods
2.  AddressBook shows a list of foods
3.  User requests to delete a specific food in the list
4.  AddressBook deletes the food
+
Use case ends.
