...
====
* `genReports`:  A grey highlight (also known as a mark-up) indicates that this is a command that can be inputted into the command line and executed by the application.
* `[subheader]#GenReportCommand.java#`: Orange text with grey highlight indicates a component, class or object in the architecture of the application.
====
