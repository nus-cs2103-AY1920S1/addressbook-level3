...
image::infowindow.png[width=500]
****
...
{empty} +
+
.User interface after book is loaned out.
image::ug-loan-pic.png[width=850]
After this command is entered, the command result display will provide a short summary of which book is loaned
...
* Books that have reached the maximum renew count or are overdue cannot be renewed.
****
...
IMPORTANT: `AMOUNT` can only have up to 2 decimal places.
...
{empty} +
+
.The command result display after paying fines.
image::ug-pay.png[width=500]
As seen from the screenshot above, after a fine amount is paid, a short summary can be seen. The remaining outstanding fine
will be shown. If the borrower pays an amount greater than his/her total outstanding fine, the amount of change to
be given back to the borrower is also shown.
