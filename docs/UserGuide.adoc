...
[IMPORTANT]
====
include::UserGuide.adoc[tag=alice]
====
...
// tag::alice[]
When you first start using TimeBook, it will create a hidden person called `Alice` to store your details and schedule. +
{nbsp} +
To personalize TimeBook and make it yours, rename `Alice` by entering the command `edituser n/Your Name Here`. If you'd like to fill in more details, you can do so using the <<edit-person-command,`edituser` command>>.
// end::alice[]
+
{nbsp} +
