...
=== Minor enhancement: Filter Feature

I have added a filter feature to $aveNUS which allows users to filter food items based on some specific criterion.

==== What it does

The filter feature only shows food that satisfy the criterion given by the user.  This is done by taking
into account  specific fields given from the command line interface.

For example, if the user wishes to see the list of food items which have a price of less than $5.00, only
food items with a price of $4.99 and below can be seen in the display panel.

==== Justification

NUS students might have certain restrictions with regards to the food that they can eat. As a result, it will be
good for them to be able to filter the type of food they can order.

For example, if they want to view halal food, they can just type in the following:

`filter CATEGORY EQUALS_TO halal`.

==== Highlights

While implementing this feature, I had to implement the `filter` command. This is done in a way such that it
will only affect the food list display panel. As a result, the food items stored in the data files will not
be accidentally edited or removed.

In this way, if the user wishes to see his entire food list again, the user can simply type in the command
`list`.
...
This section shows the contributions that I have made to the filter feature of the User Guide.

include::../UserGuide.adoc[tag=filters]
