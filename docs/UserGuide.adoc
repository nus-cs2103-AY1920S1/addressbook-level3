...
The user creating the activity will be included automatically, and additional contacts to add can be specified using keywords. The prefix `p/` is used to specify keywords to find each person. Only contacts found by valid keywords (keywords that find exactly 1 matching contact) will be added to the activity, rest will be ignored with warning message. Changes the current view to this activity (as if `view a/ACTIVITY_ID` was called).
...
Creates a new activity with title 'Breakfast'. If the contact named 'Bernice Yu' doesn't exist, only the contact 'David Li' will be added in as a participant, and a warning message will state that contact named 'Bernice Yu' couldn't be found.
