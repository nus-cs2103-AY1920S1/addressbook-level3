...
Displays the current exchange rate for a specified currency in SGD. +
Format: `exchangerate [AMOUNT] [CURRENCYCODE]`

****
* If AMOUNT and CURRENCYCODE is not specified, all exchange rates are shown.
* CURRENCYCODE can be `SGD`, `USD`, `EUR`, `IDR`, `MYR`, `INR`, `MMK`.
****
...
* `exchangerate 5 USD` +
Shows the exchange rate for 5 USD in SGD.
