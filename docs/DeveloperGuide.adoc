...
The following sequence diagram shows how an example usage scenario would work:

image::ToggleSequenceDiagram.png[]

The sequence diagram is explained below:

. The user launches the application and enters in `toggle wishlist` command to toggle the `wishlist` panel.
. The `commandResult` is obtained in MainWindow after the command is parsed and executed.
. MainWindow checks if the `togglePanel` attribute in `commandResult` is true.
. Since it is true, it retrives the `PanelName` `WISH` from `commandResult` and calls on its own method `handleTogglePanel`.
...
.. (Not shown in sequence diagram) It also checks if the wishlist is already shown in the main panel.
...
image::ToggleActivityDiagram.png[width="200"]

==== Design Considerations

* *Alternative 1 (current method):* Toggle the panels from within MainWindow class.
...
** Cons: May introduce cyclic dependency.
