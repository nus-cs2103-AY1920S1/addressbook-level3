...
** Pros: Aside from tracking a unique `keySet` of ID numbers, `HashMap` caters for deletion of entities as the `keySet` can be iterated through to check for any gaps introduced during deletion.
...
before each test to reset the `HashMaps`.
// end::uniqueIdentificationNumber[]

// tag::undoredo[]
=== Undo/Redo feature
The undo/redo feature allows the user to undo a previous command that they have made, or redo a command they have undone.
...
Classes related to undo/redo and their relationships are shown below.
...
When designing the undo/redo feature, scalability and speed were the key considerations. After analysing the pros and cons of
each option, it was decided that using the Command design pattern was the best option.
...
The `genReport <BODY_ID>` command calls `ReportGenerator#generate(body, sign)`, which creates the document.
...
. Minimise Button
* Minimisation of the application is implemented such that when the user clicks on the  minimise button (`onMouseClicked` event), the following code is called which minimises the window.
```
...
The following sequence diagram shows how a user adding a new body enables the line chart to update dynamically:

image::LineChartAddBodySequenceDiagram.png[]

Similarly, the sequence diagram below shoes how a user deleting a body enables the line chart to update dynamically:

image::LineChartDeleteBodySequenceDiagram.png[]

==== Design Considerations

===== Aspect: How data is stored and updated

* **Alternative 1 (current choice):** Use a tree map to keep track of bodies and initialise the last 10 days.
** Pros: Implementation is easy and ensures only the last 10 days is visible.
** Cons: Series is regenerated for the whole 10 days whenever there is a change. As can be seen from the activity diagram below, the series gets reintialised regardless of whether it will affect a change in the appearance of the line chart.

image::LineChartActivityDiagram.png[]

* **Alternative 2:** Store all data in a separate storage class.
** Pros: Implementation allows user to see beyond 10 days.
