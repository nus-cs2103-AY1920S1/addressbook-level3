...
The `BookmarkPredicate` class keeps track of multiple autotag conditions, including normal conditions and
...
It also contains a single _predicate_ to test whether a bookmark matches the given conditions.
The Predicates package contains several other classes (those with names `XYZContainsKeywordsPredicate`)
that are used to generate this predicate from the keyword sets.
