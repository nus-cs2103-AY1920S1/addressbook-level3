...
This means that data stored in `MatchedDonor` and `MatchedPatient` objects will not be assessible
...
executing a match command. This ensures that the `updateCell()` method would be called to
...
// mebee put more? idk
The find feature is facilitated by the following classes: `FindCommand` `PersonContainsPrefixesPredicate`. +
...
===== Execution Behaviour
// dis statement not rly tru tho since we b explainin more than jus de executerar method
