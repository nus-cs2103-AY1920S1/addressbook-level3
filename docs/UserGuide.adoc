...
//TODO: You can also add a general note not pertaining to any specific paragraph to the _General notes section_
//TODO: located at the bottom of the document.
 ...
 * `PARA_NUM` is the numbered identifier of the paragraph to be marked.
 `PARA_NUM` must
//TODO: be `NULL` or it must
begin with `P` or `S` (e.g. `P3`).
...
//TODO: * `*annotate* 1 n/adding a general note` +
...
At v1.3, moving of notes to the  <<stray-notes, _General notes section_>> is not supported.
