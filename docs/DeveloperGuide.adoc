...
==== Implementation
To facilitate this, the `[green]#Model#` has a `[green]#TrainingManager#` that stores all `[green]#Training#`
...
The `[green]#Training#` class contains a date and a hash map.
The date in the training class represents the date of training session and the hash map is used to check the
attendance of a Person. We used a `[green]#AthletickDate#` class to  represent a date of training. This class is shared
across both the `[green]#Ui#` and backend, allowing training information to be shown in the `view calendar` function.
The hash map uses `[green]#Person#` objects as keys and a boolean as it’s value. This is used to check whether a
`[green]#Person#`
has
attended a training session, indicated by the boolean value attached to  a `[green]#Person#` key. If a
`[green]#Person#` attended a training session on that day, the value associated with the key will be true,
and false if he did not attend.

A `[green]#TrainingManager#` is used to store and manage all `[green]#Training#` sessions in Athletick. It has a list
containing `[green]#Training#` sessions. A `[green]#Training#` is added to this list whenever a user records a
`[green]#Training#` session
using a `[green]#TrainingCommand#`. The sequence diagram below shows how `[green]#TrainingManager#` stores and manages
training records.
...
`[green]#TrainingManager#` provides functions for users to calculate the attendance rate of one individual, or the
entire
team. The sequence diagram below shows a use case of how `[green]#TrainingManager#` gets attendance rate.

image::to_add.png[]

`[green]#TrainingManager#` also allows users to view `[green]#Training#` information on a specified date. The sequence
diagram below
shows a use case of how `[green]#TrainingManager#` gets `[green]#Training#` information.
...
* Alternative 1: Extend on the `[green]#Athletick#` by storing and tagging each person with number of
...
** Pros: Allows storing of specific training information without depending on the `[green]#Athletick#`.
