...
[TIP]
*LightBulbs* indicate a _Tip_, somthing that is helpful to the reader
...
The following activity diagram summarizes what happens *after* a user successfully executes an `AutoAllocateCommand`:
...
===== Aspect: How to update the changes in the manpower list of an event after allocation of employee.
...
image::DistinctDateClassDiagram.png[]
._Class Diagram for DistinctDateProcessor_
image::DistinctDateProcessorClassDiagram.png[]
...
Step 2. The user executes `add_ev n/Free Cofee ...` to add a new event into the Eventlist. The distinctDatesList will be reinitialised with an empty list (all its existing inputs will be flushed).
