...
+
.IncidentManagerParser#parseCommand()
[source, java]
----
...
`JsonIncidentManager#saveIncidentManager()`.
. Again, it appears that the heavy lifting is delegated. Let's take a look at ``JsonSerializableIncidentManager``'s
constructor.
+
.JsonSerializableIncidentManager#JsonSerializableIncidentManager()
[source, java]
----
/**
 ...
 * {@code JsonSerializableIncidentManager}.
 */
...
`JsonSerializableIncidentManager`.
...
this tutorial, you learned more about the inner workings of IncidentManager and how the various components mesh together
...
.. Save the incident Manager in the CSV format instead
.. Add a new command
.. Add a new field to `Person`
.. Add a new entity to the incident Manager
****
