...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the contact's name).
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedAccommodation = new Person(contactToEdit.getName(), contactToEdit.getPhone(), contactToEdit.getEmail(),
                contactToEdit.getAddress(), remark, contactToEdit.getTags());

        model.setPerson(contactToEdit, editedAccommodation);
