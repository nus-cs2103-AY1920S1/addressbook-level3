...
Should there be a need to revert back to a past or future state (if undo or redo is called), `ModelManager`'s `ModelData` will retrieve their data from the
appropriate copy of `ModelData` in the list of duplicates.

`UndoRedoManager` also implements the following operations:

* `UndoRedoManager#undo()` -- Restore `ModelData` in `ModelManager` to their previous versions from the appropriate duplicate in `undoStateList`
* `UndoRedoManager#redo()` -- Restore `ModelData` in `ModelManager` to their future versions from the appropriate duplicate in `undoStateList`
...
If such a change exists, it will be handled by first instantiating a `ModelData` with a deep-copied version of the
...
to match the data of the `ModelData` with index `undoIndex` in `undoStateList` so that the correct version of `ModelData` is being reflected in the GUI.
...
A `ModelData` will be added to `undoStateList` and the `undoIndex` will point to that single `ModelData` in the list.

image::ModelData0.png[]
...
Then, `UndoRedoManager#onModelDataChange(ModelData list)` will be called (as there has been a change to the `eventList`),
...
Finally, the deep-copy of the new `ModelData` state will be committed; added to `undoStateList`.
The `undoIndex` is incremented by one to contain the index of the newly inserted model data state.

image::ModelData1.png[]
...
`ModelData` with index `undoIndex` in `undoStateList`.

image::ModelData2.png[]
...
Since no changes to `ModelData` have been made, the listener methods will not be triggered and no `ModelData` will be saved to `undoStateList`.
Thus, the `undoStateList` remains unchanged.

image::ModelData3.png[]
...
The modified `ModelData` will be deep-copied and a new `ModelData` containing the deep-copies will also be added to `undoStateList`.
The `undoIndex` is incremented by one to contain the index of the newly inserted model data state.

image::ModelData4.png[]
