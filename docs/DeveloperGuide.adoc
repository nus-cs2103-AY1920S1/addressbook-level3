...
* **Alternative 2 (Current choice):** Use brute force to start iterating from "B00001" to obtain the first unused serial number.
** Pros: Will be easy to implement.
...
* `LoanSlipUtil#clearSession()` -- Clears the current loan session by unmounting all loans, usually after generating a pdf version of the loan slip.
...
it adheres to good programming practices as compared to Alternative 2.
// end::printLoanSlip[]
