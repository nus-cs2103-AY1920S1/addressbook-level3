...
=== [Proposed] Engagement Report Generation
==== Proposed Implementation
...
1. Util class `PdfUtil` can be implemented for handling all pdf document creation related methods.
* It will be able to generate a full report document based on the `Engagement` as input.
* Util class will use different templates for each other types of `Engagement` such as `Appointment`, `Interview` and `Meeting`.
1. `Report` class needs to be implemented in order to model all necessary properties that has to be in the document.

image::pdf-report/Report_ClassDiagram.png[]
...
===== Aspect: How System generates a Report document file
