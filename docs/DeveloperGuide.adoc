...
Thus, this feature requires access to the `ModelManager#getPersonByName` method of the `ModelManager` inside the
...
.Sequence Diagram of the Parser for Add Command
...
This is the definition of the methods shown above:
[[resetPredicate]]
* `ModelManager#resetPredicate()`: It resets the `Predicate` object in `ModelManager` to evaluate all `Transaction` objects to true.
* `ModelManager#addTransaction(transaction): It adds the given `Transaction` object to its `TransactionList` objects attributes.

Thus, when the `ModelManager#resetPredicate()` method is called in the `AddCommand`, the UI table will immediately shows the full
...
The following sequence diagram shows how the delete by name command works after it is created which is referenced in <<GeneralLogicSD, 2.3. Logic component: Figure 5>>:

.Sequence Diagram of Delete Command in Home Tab (transaction package)

image::HomeDeleteNameCommand.png[]

In addition, the `ModelManager#resetPredicate()` method defined in <<resetPredicate, 3.1.1 Add Command Feature>> is not called in the `DeleteNameCommand`. If the prior input is a
...
Back Command where `BackCommand` calls `ModelManager#resetPredicate()`. The sequence diagram for the `BackCommand` is shown in the
...
The delete by index implementation would be similar but does not require interaction with the `CheckAndGetPersonByNameOnlyModel` from the
