...
The Generate Schedule Feature is implemented to allow users to have an overview the event schedule. It will display all dates that has an event and the
specific events that happens on those dates listed. The feature is facilitated by a `DistinctDatesProcessor` and requires the use of a
...
The following class diagram shows how the relationships between the different classes used by the `generate_schedule` command:

._Class Diagram for GenerateScheduleCommand_
image::GenerateScheduleClassDiagram.png[align="center"]

---
Given below is an example usage scenario and how the `display_schedule` command behaves at each step.
