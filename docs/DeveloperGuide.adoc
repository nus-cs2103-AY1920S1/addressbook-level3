...
=== Cashier Tab
This tab will act as a shopping cart to show sales item added from the inventory that will contribute to sales made.

To add the sales item into the cart, it will require an input of a description and its quantity.
An optional input of category is provided to guide the cashier to find the desired item.
...
The add command is facilitated by the `VersionedtreasurerPro`. This feature allows the adding of sales items to the cart.

Only description and quantity fields are compulsory to be inputted by the user. If the category field
is input with other incomplete description and quantity fields, suggestions will be shown by Leo,
the assistant according to that category.

The quantity of the item inputted must not exceed the quantity of stock available in the inventory.

Whenever the addition of sales item is successful, a response message will be shown by Leo.
...
When the user input an invalid description or quantity field without any category field, an appropriate response
message will be shown accordingly. If the category field is specified, suggestions according to the category specified
will be shown.
...
==== Edit Feature
The edit command is facilitated by the `VersionedtreasurerPro`. This feature allows the editing of the quantity of
sales item that are already added to the cart. There are two ways to edit - by index or by description.

To edit by index, it will require an input consisting of the index and quantity field. The specified index
is based on the table shown on the Cashier tab.

To edit by description, it will require an input consisting of the description and quantity field. The specified
description should match the description of an item in the table shown on the Cashier tab.

The following sequence diagram shows how an edit command works with the required fields present:
...
==== Delete Feature
The delete command is facilitated by the `VersionedtreasurerPro`. This feature allows the deleting of the items in
the cart shown on the Cashier tab.

To edit a specific item in the cart, the only field required is the index of the item based on the table shown.
...
The command to set the cashier is facilitated by the `VersionedtreasurerPro`. This feature allows
an existing person in the `Address Book` to be set as a cashier. The only field required is the name of an
existing person.
...
==== Checkout Feature
The checkout command is facilitated by the `VersionedtreasurerPro`. This feature confirms
all the sales items in the table as sold and records the whole cart as a transaction under the `Sales` category.

The `Transaction Tab` will be updated with the new transaction labelled as `Sales Items`.

The quantity of every items will be updated in the `Inventory Tab`.
...
The `cashier logic` will write the updated inventory list and transaction to the relevant data files. To update the
view on the `Inventory Tab` and `Transaction Tab`, transaction will be added to the transaction model and
...
==== Clear Feature
The clear command is facilitated by the `VersionedtreasurerPro`. This feature clears all the sales items in the table.
This command is called after the list of items has been checkout. It can also be called by the user.
