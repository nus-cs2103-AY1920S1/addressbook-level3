...
Step 1. The user launches the application for the first time. The `Versionedmoneygowhere` will be initialized with the initial address book state, and the `currentStatePointer` pointing to that single address book state.

image::UndoRedoState0.png[]

Step 2. The user executes `delete 5` command to delete the 5th Spending in the address book. The `delete` command calls `Model#commitmoneygowhere()`, causing the modified state of the address book after the `delete 5` command executes to be saved in the `moneygowhereStateList`, and the `currentStatePointer` is shifted to the newly inserted address book state.

image::UndoRedoState1.png[]

Step 3. The user executes `add n/David ...` to add a new Spending. The `add` command also calls `Model#commitmoneygowhere()`, causing another modified address book state to be saved into the `moneygowhereStateList`.

image::UndoRedoState2.png[]

[NOTE]
If a command fails its execution, it will not call `Model#commitmoneygowhere()`, so the address book state will not be saved into the `moneygowhereStateList`.

Step 4. The user now decides that adding the Spending was a mistake, and decides to undo that action by executing the `undo` command. The `undo` command will call `Model#undomoneygowhere()`, which will shift the `currentStatePointer` once to the left, pointing it to the previous address book state, and restores the address book to that state.

image::UndoRedoState3.png[]

[NOTE]
If the `currentStatePointer` is at index 0, pointing to the initial address book state, then there are no previous address book states to restore. The `undo` command uses `Model#canUndomoneygowhere()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the undo.
...
*Value proposition*: manage budget quickly compared to a typical mouse/GUI driven app
...
|`* * *` |new user |view more information about a particular command|learn how to various commands
...
|`* *` |user |export data |view my spending in another application

|`* *` |user |undo my previous action | revert my mistakes

|`* *` |user |redo my previous action | move forward in history

|`* *` |financial analyst |view daily/monthly/yearly expenditure |have a projected view of all my spending
...
|`*` |user who travels often |see the exchange rates of other countries |view my spending with respect to my currency

|`*` |forgetful user |set a reminder |reminded of my payment deadlines

|`*` |housewife |view the cheapest price of an item |improve my savings

|`*` |user who loves rewards |see how much more I should spend |earn rewards from places I spend at

|`*` |investor |see how much I should invest | earn more money
|=======================================================================
...
'''
[discrete]
=== UC06 Generate statistics

*MSS*

1.  User requests to generate statistics.
...
'''
[discrete]
=== UC10 Import external data from a CSV file
...
'''
[discrete]
=== UC11 Export data to a CSV file
...
'''
[discrete]
=== UC14 Graphing the data

*MSS*

1.  User requests to display spending in the form of a graph.
