...
Do note that if a `CATEGORY` is not specified by the user, it will be set as `GENERAL` by default.
...
When projecting by date and category, transactions tagged by the specified category will be taken into account.
