...
Items in the replenish list with only `Name` and `Tag` field are represented by the `Item` class.
On the other hand, items in the main list (tracking list) with additional fields such as `ExpiryDate`,
`Quantity` and `ReminderThreshold` are represented by the `XpireItem` class.
To utilise the concept of inheritance and reduce duplicate data and methods, the `XpireItem` is written to inherit from `Item`.
...
An `XpireItem` in the main list can be transferred into the replenish list using the `ShiftToReplenishCommand`.
Any `XpireItem` whose quantity have hit `0` will also be automatically added to the replenish list.

===== Implementation
When the user executes a `ShiftToReplenishCommand`, the target item in the main list be adapted from a `XpireItem` into an `Item`.
The adapted item will be subsequently added to the replenish list, while the target item is removed.
The following sequence diagrams demonstrate how the command makes changes to
both the main list and replenish list.
...
On the other hand, items can be transferred out of the replenish list using the `ShiftToMainCommand`.
