...
|Design of calculateRecommendation()
...
We chose this design because adding a `recommendationValue` field for each `Food` might not make sense in
the design of our application. A `Food` object by itself has no sense of `recommendationValue` and thus it
might not be reasonable to force a connection between the two for the sake of efficiency.

Furthermore, we do not have to update the recommendation of each `Food` item after every command with this design.
...
* Cons:
** The recommendation value of each `Food` must be updated after every command to ensure validity of
