...
.  `Statistics` contains an ObservableMap of `MonthList` class.
.  `MonthList` contains an ObservableMap of `DailyList` class.
...
There are 2 main checks to be carried out when interacting with `Category`.

* When adding and editing a `Category`, there is a need to check that the new Category added does not
...
=== Data Presentation: ViewPie/ViewTable Command
