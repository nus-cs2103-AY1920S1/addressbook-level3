...
StudyBuddy Pro is a desktop-based application intended for
students in the School of Computing at NUS that prefer to use a to work with a Command Line Interface (CLI) while still having the benefits of a Graphical User Interface (GUI).

StudyBuddy Pro effectively tackles the problem of target users having spent large amount of time for quality study revision.
To lessen time consumption to produce same study revision quality for target users, StudyBuddy Pro provides a centralized platform for them to revise using flashcards, take notes and generate cheatsheets.
...
=== Callouts Signs and Syntax

Do refer to signs below that will be used across the documents for references. These will prove to be useful when you are reading this document.

.Table title
|===
| Signs/Syntax | Explanations
...
[TIP]
====
Indicates additional information that are helpful. Not to worry, tips can be good to know but are not vital.
====
...
Assuming that the user is in _flashcard_ mode.
Given below is the Sequence Diagram for interactions within the `Logic` component for the `execute("delete 1")` API call.

.Interactions Inside the Logic Component for the `delete 1` Command
image::DeleteSequenceDiagram.png[]

NOTE: The lifeline for `DeleteFlashcardCommandParser` should end at the destroy marker (X) but due to a limitation of PlantUML, the lifeline reaches the end of diagram.
