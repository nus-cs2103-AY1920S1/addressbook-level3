...
|Interfaces containing only required methods from the models are created.
The methods are called via these interfaces to update the relevant data in inventory and transaction.

*Pros*: Only necessary methods will be accessed by the Cashier's `Logic`, preventing any unwanted changes through other methods.
...
*Pros*: Time performance is better when elements are be accessed from the head of the list.
...
of the list directly. As the sales list will be updated and accessed regularly, an ArrayList is more fitting.
