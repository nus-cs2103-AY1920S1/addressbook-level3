...
* What it does:
** A built-in attendance tracker allows coaches and captains to track their team’s attendance using the `attendance`,
`select`, `view attendance` and `calendar` commands.
...
removes the need for manual calculation as it can be tedious for coaches and captains, especially if they have to go
