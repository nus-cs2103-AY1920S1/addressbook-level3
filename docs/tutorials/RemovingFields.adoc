...
In scenarios like this, you can consider refactoring the existing `Person` addressModel to suit your use case.
...
The `address` field in `Person` is actually an instance of the `seedu.address.addressModel.person.Address` class.
