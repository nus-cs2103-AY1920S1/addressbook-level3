...
The `get-task-stats` command calls `Model#getFilteredTasksList()`, `Model#getFilteredMembersList()` and `Model#getFilteredMappingsList()` to obtain lists of all the members, tasks and mappings saved in the application. Using the lists, a Statistics object is formed. 'Model#setStatistics' is called to updated the statistics in ProjectDashboard.
...
Step 3. In order for task statistics to be displayed in a comprehensive manner, when the `get-task-stats` command is called,
`TaskStatisticsView` class is also called to display the task stats.

The following sequence diagram shows how calling the 'get-member-stats' operation leads to the comprehensive UI display of task statistics.

image::GetStatisticsUISequenceDiagram.png[]
Figure 10. Operational flow of GUI when 'GetStatisticsCommand' is called
