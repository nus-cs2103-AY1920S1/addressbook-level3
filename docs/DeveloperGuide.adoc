...
[NOTE]
If the `commandHistory` stack is empty then there are no previous states to be restored. The `undo` command uses `Model#canUndoAddressBook()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the undo.
...
Thus, the `commandHistory` and `commandRedoHistory` stacks remain unchanged.
...
Each Duty Shift is an `Event` object consists of a `PersonReferenceId`, `Timing`, `Status`. `Timing` class has 2 `DateTime` object as they indicate the start and end times of the duty shift. The `UniqueEventList` contains 0 or more duty shifts.
