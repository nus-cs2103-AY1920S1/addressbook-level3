...
`seedu.description.ui.CommandBox.CommandExecutor`.
...
command and store any possible changes in an `EditProblemDescriptor`. Recall that we can verify the contents of
`editProblemDesciptor` through the `Variable` tool window.
+
...
`JsonAlgoBase#saveAlgoBase()`.
. Again, it appears that the heavy lifting is delegated. Let's take a look at ``JsonSerializableAlgoBase``'s
constructor.
+
.JsonSerializableAlgoBase#JsonSerializableAlgoBase()
[source, java]
----
/**
 ...
 * {@code JsonSerializableAlgoBase}.
 */
public JsonSerializableAlgoBase(ReadOnlyAlgoBase source) {
    problems.addAll(
        source.getProblemList()
              .stream()
              .map(JsonAdaptedProblem::new)
              .collect(Collectors.toList()));
}
----
. It appears that a `JsonAdaptedProblem` is created for each `Problem` and then added to the
`JsonSerializableAlgoBase`.
...
this tutorial, you learned more about the inner workings of AlgoBase and how the various components mesh together
...
.. Save the description book in the CSV format instead
.. Add a new command
.. Add a new field to `Problem`
.. Add a new entity to the description book
****
