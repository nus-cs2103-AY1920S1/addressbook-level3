...
as `name` which may be duplicated within the system.

==== Implementation

The generation of unique `IdentificationNumbers` is facilitated by `UniqueIdentificationNumberMaps`.

===== Determing the next free number

`UniqueIdentificationNumberMaps` keeps three `HashMap`, one for each entity. +
In each `HashMap`, the `Integer` ID number serves as the key, which maps to the worker it is assigned to.
This keeps track of the numbers currently assigned to all the workers, and allows the next free `Integer` to be assigned to a newly added worker. +

The code block below demonstrates how the next free number is determined.
...
In the above `putWorker` method, the set of `Keys` representing the existing ID numbers are generated and iterated through.
This sequential iteration checks for any number that is not assigned to any worker due to the deletion of the latter, which removes the mapping of the ID number to the deleted worker.
...
** Pros: Caters for deletion of entities as the `Keys` can be iterated through to check for any gaps introduced during deletion.
This also allows the assigned (mapped) entity to be accessible through the `HashMap` via the `IdentificationNumber#getMapping()` method.
** Cons: Harder to implement.

===== The steps below detail the generation of a unique `IdentificationNumber`:
...
. Consequently, `generateNewWorkerId()` creates a new `IdentificationNumber`, where the number is determined after the execution of `addEntity()` in `UniqueIdentificationNumberMaps`.
. `addEntity()` subsequently calls `putWorker()`, which inserts the worker into the worker `HashMap` and returns a number ID that is not currently assigned to a worker.
