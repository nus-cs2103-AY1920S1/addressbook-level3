...
has been not been contactable for a given period of time. He / She then needs to contact the police to proceed with a
...
of the `notif`. `long` is used because `notifCreationTime.getTime()` returns a `long` which is useful in storage.
 It will be explained in further detail later.
...
`Body` to `CONTACT_POLICE` if there is no change in its status for the specified `period`. The `UpdateCommand` is
called to make the change.
5. After the status of the `Body` is updated, a `NotifWindow` is shown to alert the user of the automatic
update.
6. On clicking the notification bell or using the `showNotif` command, a pop-over opens and lists all the bodies for
which police needs to be contacted.
...
The following activity diagram illustrates what happens when a user adds a new body and the `NotifCommand` is
instantiated:
...
`notifCreationTime` are stored in a JSON file along with the other `Entities`. When the `MainApp` is initialized, the
following happens:
...
* **Alternative 2:** Show pop-up on app initialization for `Notif` s in storage for which the difference between system
...
The generate PDF feature is facilitated by `[subheader]#ReportGenerator#` class.
...
* `[subheader]#ReportGenerator#generate(body, sign)#` -- Creates report containing sign name of manager in a PDF file for the specific body.
* `[subheader]#ReportGenerator#generateAll(sign)#` -- Creates reports containing sign name of manager in a PDF file for all bodies registered in Mortago.
* `[subheader]#ReportGenerator#generateSummary(sign)#` -- Creates a tabular summary report containing sign name of manager  in a PDF file for all bodies registered in Mortago.
...
NOTE: The lifeline for `[subheader]#GenReportCommandParser#` and `[subheader]#ReportGenerator#` should end at the destroy marker (X) but due to a limitation of PlantUML, the lifeline reaches the end of diagram.

The `genReport <BODY_ID> (sign)` command calls `[subheader]#ReportGenerator#generate(body, sign)#`, which creates the document.
...
The following code snippet from `[subheader]#GenReportCommand.java#` demonstrates how an error message is displayed when manager inputs an invalid command and when report is not successfully generated:
...
*Precondition*: Mortuary manager closes all existing report PDFs.
