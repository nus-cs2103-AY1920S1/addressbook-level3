...
* `ProjectDashboard#getTasksNotStarted()` -- Retrieves the current theme applied to +Work.
* `ProjectDashboard#getTasksDoing()` -- Retrieves the current theme applied to +Work.
* `ProjectDashboard#getTasksDone()` -- Retrieves the current theme applied to +Work.
* `ProjectDashboard#getTasksByDeadline()` -- Retrieves the current theme applied to +Work.
* `ProjectDashboard#getMeetingList()` -- Retrieves the current theme applied to +Work.
...
The following steps wil guide you on how `Task` data is processed and retrieved through the use of object diagrams when the user views the dashboard. It will omit operations
invlolving `Meeting` data as well as in the `Ui` component for brevity.
...
`Logic#getFilteredTasksDone()` and `Logic#getFilteredTasksByDeadline()`.
...
Step 4. This executes `ProjectDashboard#splitTasksByStatus()`, to populate `filteredTasksNotStarted`, `filteredTasksDoing` and `filteredTasksDone`. +
Also, `ProjectDashBoard#splitTasksByDeadline()` is called to populate `filteredTasksByDeadline`.

Step 5. Relevant data is retrieved by `Ui`.
...
===== Aspect: Storage and retrieval of `Task` related data for dashboard.
...
When the enters `home` to view the dashboard, the tasks are split by `TaskStatus` and deadline in `ProjectDashboard` and dispatched to the `Ui`.
...
** Cons: The constant recalibration and splitting of task data may tax the memory of the application, as it is storing the same tasks in multiple containers. This may affect performance
for large number of tasks.
* **Alternative 2:** The `Task` objects will only contain attributes which are not filtered in the dashboard, such as `Name`. They are stored in a `HashMap` as values and the keys are filtered attributes such as status and deadline.
** Pros: Retrieval of data is much faster, as `ProjectDashboard` does not have to store multiple copies of tasks in memory. Instead,
due to the mappings between `TaskStatus` and the `Task` assigned those statuses, they can be retrieved and displayed more efficiently.
...
benefit the dashboard greatly it would cause other components and views to become inefficient.
...
** Cons: User cannot customise the data directly due to the nature of `Enum`.
...
** Cons: The `UserSettings` interface changes whenever the data related to a particular settings option is updated. `UserSettings` should only be aware of the various settings the user
is able to customise.

We decided to opt for design option one, so as to be in line with the Single Responsibility principle. This would make it easier to extend the functionality
of the settings feature which would allow developers to extend its functionality in a more safe and modular manner.
