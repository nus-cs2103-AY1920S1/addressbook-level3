...
** *What it does*: This feature represents moving all bought items into the fridge (grocery list). The feature is able to deal with previously existing grocery items with the same name and expiry date as the bought item. The feature is also able to deal with the case in which user buys more or less amount than specified in the shopping list.

** *Justification*: This feature is crucial to the product and user experience; it is a single command solution to merge all the bought shopping items into the grocery list without having to manually add them with multiple `glist add` commands. This feature takes many factors into consideration while modifying the shopping list and grocery list according to the bought items to be more intuitive and convenient.
...
*** If a grocery item with same name and expiry date as the bought item already exists in the grocery list, add the quantity of the grocery item by the amount bought
*** the bought item is just added to the grocery list otherwise.
...
*** If the user buys less than the amount stated in the shopping list, the shopping item's amount is just reduced by the amount bought and the bought tag of the shopping item is removed (as there are no bought items left to move to fridge).
...
** *Justification*: This feature significantly adds to the user experience. This is because, users tend to have very long shopping lists and might want to prioritize some over the others. Due to this feature, all prioritized items are shown at the top of the shopping list to grab the user's attention.
...
2. Within urgent items, shopping items are sorted by alphabetical order in the shopping list; shopping list is sorted by urgent status first and then alphabetical order.
...
** *Justification*: This feature makes the user experience much more convenient. Since the fully bought items are moved to the bottom of the list, the user will save time by looking at urgent and not bought items at the top of the list. Also, printing the amount along with the amount status (partially or fully bought) helps keep the user informed about how much more to buy.

** *Highlights*
*** The UI element `ShoppingItemCard` receives the bought list as an attribute. This is so that for each shopping item the bought list can be checked to deduce and display the bought status of the shopping item.
*** If an urgent item is completely bought, it loses its urgent tag, gets a `Fully Bought` tag and is moved to the bottom of the shopping list. This is intuitive to the user; when a required amount of the shopping item is bought already, it should no longer be prioritised.
*** If a fully bought shopping item's amount is edited to a higher amount than the amount bought, the shopping item will be marked as partially bought and moved up the list (it should not be at the bottom anymore since it is not fully bought).
...
Similarly, if a partially bought shopping item's amount is edited to an amount smaller than or equal to bought amount, the item will be marked as 'Fully Bought!' and lose 'Urgent!' tag if it has one. +
...
|===
|_Below are some notable sections I contributed to our User Guide, which showcase my ability to write documentation that is beneficial to our target end-users._
|===

include::../UserGuide.adoc[tag=shoppinglistfeatures]
