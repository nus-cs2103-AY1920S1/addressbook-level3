...
The notification feature prompts the user with new window if a meal have been missed.
...
.. Test case: No meals in input for current month. +
   Expected: Report is *NOT* generated. An error message will be shown in the result display box of CaloFit.
.. Test case: Any number of meals has been entered for current month. +
   Expected: Report is generated normally.
.. Test case: Any number of meals has been entered for current month and system time is changed to a different month or year. +
...
.. Test case: same as previous, with additional `set 1000` at the end. +
...
.. Test case: same as previous, with additional `add n/carbonara c/690` before `report`. +
