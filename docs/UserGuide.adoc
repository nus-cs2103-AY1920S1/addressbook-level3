...
NOTE: User can generate multiple report documents with the same engagement, but with different SENDER and RECEIVER names. System will not allow generating documents that already exists in the directory.
...
NOTE: Documents explorer view will not display the newly generated document until user clicks the green refresh button.
...
Once document generation is executed, system will automatically open the document file. Alternatively, user can double click the list item to open the document file on their local computer file system. Below is the sample of generated document from our system.
