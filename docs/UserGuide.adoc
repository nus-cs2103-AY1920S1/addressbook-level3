...
NOTE: The date in the dt field must be a valid date, if an invalid date is used i.e. 31/11/2019 it will
be run down to the nearest valid date i.e. 30/11/2019.
...
NOTE: The revision tasks are not synchronised with the note or the question. Once created, the task content will not
change if you modify the corresponding note or question. We deliberately disassociate the task from notes and questions once created
because we would like to allow you to customise the task headings in *version v2.0* of the app.

NOTE: Revision tasks with the same heading, date *and* time are considered duplicate tasks and cannot co-exist
...
NOTE: `HEADING` is case sensitive.

NOTE: You cannot edit the status (done or not done) of a task with this command. Instead, you may use `rdone`.
You cannot do "undone" operation as explained in `rdone` feature.
