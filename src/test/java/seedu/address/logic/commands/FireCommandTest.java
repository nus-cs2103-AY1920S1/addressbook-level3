...
public class FireCommandTest {
    ...
    @Test
    public void execute_memberAndTaskValid_deleteSuccessful() throws Exception {
        ...
        CommandResult commandResult = new FireCommand(new Index(0), new MemberId("test")).execute(modelStub);

        assertEquals(String.format(FireCommand.MESSAGE_REMOVE_TASK_SUCCESS), commandResult.getFeedbackToUser());
    }

    /*@Test
    public void execute_noMapping_throwsCommandException() {
        ModelStubAcceptingTasMemMappingAdded modelStub = new ModelStubAcceptingTasMemMappingAdded();
        Task newTask = new TaskBuilder().build();
        Member firstMember = new MemberBuilder().withId(new MemberId("test")).build();
        Member secondMember = new MemberBuilder().withId(new MemberId("testing")).build();
        modelStub.addMember(firstMember);
        modelStub.addMember(secondMember);
        modelStub.addTask(newTask);

        TasMemMapping mappingFirstMember = new MappingBuilder().withTask(0).withMember(0).tasMemMappingBuild();
        modelStub.addMapping(mappingFirstMember);
        FireCommand fireCommand = new FireCommand(new Index(0), secondMember.getId());

        assertThrows(CommandException.class, FireCommand.MESSAGE_NO_MAPPING, () ->
                fireCommand.execute(modelStub));
    }*/
    ...
    private class ModelStub implements Model {
        ...
        @Override
        public List<LocalDateTime> findMeetingTime(LocalDateTime startDate, LocalDateTime endDate, Duration meetingDurationggG) {
            throw new AssertionError("This method should not be called.");
        }
     ...
     */
    private class ModelStubAcceptingTasMemMappingAdded extends FireCommandTest.ModelStub {
