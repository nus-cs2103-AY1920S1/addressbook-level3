...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, NAME_DESC_BOB + URL_DESC_BOB + ADDRESS_DESC_AMY
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND, new AddCommand(expectedBookmark));

        // multiple tags - all accepted
        Bookmark expectedBookmarkMultipleTags = new BookmarkBuilder(BOB).withTags(VALID_TAG_FRIEND, VALID_TAG_HUSBAND)
                .build();
        assertParseSuccess(parser, NAME_DESC_BOB + URL_DESC_BOB + ADDRESS_DESC_BOB
    ...
    public void parse_optionalFieldsMissing_success() {
        ...
        assertParseSuccess(parser, NAME_DESC_AMY + URL_DESC_AMY + ADDRESS_DESC_AMY,
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        // missing name prefix
        assertParseFailure(parser, VALID_NAME_BOB + URL_DESC_BOB + ADDRESS_DESC_BOB,
                expectedMessage);

        // missing url prefix
        assertParseFailure(parser, NAME_DESC_BOB + VALID_URL_BOB + ADDRESS_DESC_BOB,
                expectedMessage);

        // missing address prefix
        assertParseFailure(parser, NAME_DESC_BOB + URL_DESC_BOB + VALID_ADDRESS_BOB,
                expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_NAME_BOB + VALID_URL_BOB + VALID_ADDRESS_BOB,
                expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        // invalid name
        assertParseFailure(parser, INVALID_NAME_DESC + URL_DESC_BOB + ADDRESS_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Name.MESSAGE_CONSTRAINTS);

        // invalid url
        assertParseFailure(parser, NAME_DESC_BOB + INVALID_URL_DESC + ADDRESS_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Url.MESSAGE_CONSTRAINTS);

        // invalid address
        assertParseFailure(parser, NAME_DESC_BOB + URL_DESC_BOB + INVALID_ADDRESS_DESC
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Address.MESSAGE_CONSTRAINTS);

        // invalid tag
        assertParseFailure(parser, NAME_DESC_BOB + URL_DESC_BOB + ADDRESS_DESC_BOB
                + INVALID_TAG_DESC + VALID_TAG_FRIEND, Tag.MESSAGE_CONSTRAINTS);

        // two invalid values, only first invalid value reported
        assertParseFailure(parser, INVALID_NAME_DESC + URL_DESC_BOB + INVALID_ADDRESS_DESC,
