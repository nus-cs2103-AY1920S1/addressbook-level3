...
NOTE: To avoid accidental rescheduling of other patient's appointments, the user must first narrow down the search to a single patient using the `appointments [<REFERENCE_ID>]` command. +
...
NOTE: To avoid accidental rescheduling of another doctor's shift, the user must first narrow down the search down to a single doctor using the `appointments [<REFERENCE_ID>]` command. +
