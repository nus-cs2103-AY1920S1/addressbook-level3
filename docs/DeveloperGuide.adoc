...
*Step 6*. The `QuestionBank#questionsFiltered` is then sorted in ascending order of their question length.

*Step 7*. A new `CommandResult` will be returned with a message stating the search term and the number of results returned.

image::QuestionsFindCommandActivityDiagram.png[]
_Figure 6. Activity diagram of find command_
...
different meanings e.g fog and frog, will be more often included in the search results.
