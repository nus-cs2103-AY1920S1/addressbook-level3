...
public class AddBudgetCommandParserTest {
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        assertParseFailure(parser, VALID_DESCRIPTION_SCHOOL + AMOUNT_DESC_SCHOOL + START_DATE_DESC_SCHOOL + PERIOD_DESC_SCHOOL,
                expectedMessage);

        // missing amount prefix
        assertParseFailure(parser,  DESCRIPTION_DESC_SCHOOL + VALID_AMOUNT_SCHOOL + START_DATE_DESC_SCHOOL + PERIOD_DESC_SCHOOL,
                expectedMessage);

        // missing start date prefix
        assertParseFailure(parser,  DESCRIPTION_DESC_SCHOOL + AMOUNT_DESC_SCHOOL + VALID_START_DATE_SCHOOL + PERIOD_DESC_SCHOOL,
                expectedMessage);

        // missing period prefix
        assertParseFailure(parser,  DESCRIPTION_DESC_SCHOOL + AMOUNT_DESC_SCHOOL + START_DATE_DESC_SCHOOL + VALID_PERIOD_SCHOOL,
                expectedMessage);

        // all prefixes missing
        assertParseFailure(parser,  VALID_DESCRIPTION_SCHOOL + VALID_AMOUNT_SCHOOL + VALID_START_DATE_SCHOOL + VALID_PERIOD_SCHOOL,
                expectedMessage);
    }
}
