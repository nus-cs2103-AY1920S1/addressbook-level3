...
* `[purple]#ReportGenerator#generateAll(sign)#` -- Creates reports containing sign name of manager in a PDF file for all bodies registered in Mortago.
...
===== 4.6.1.1. Current Implementation
...
** Pros: Easier to create drop shadows to simulate elevation
...
Alternative 1 was chosen because, aside from the pros that it has to offer, bulk of the screen space in the Mortago is taken up by surfaces to optimize the amount of information available to the user,
hence by giving surfaces a darker brightness, this improves *overall* visual ergonomics.

==== Replacing Window's Title Bar

This section details how the default window's title bar is replaced with a custom title bar.

===== 4.6.2.1. Current Implementation
