...
It sorts the notes list in `Model` by executing `Model#sortNotesRecord(Comparator<Note> noteComparator)`. The sorting is passed to
the `NotesRecord` which uses the comparator implemented by `Note` to sort and update its internal list.

*Step 2*. The `NoteSortCommand#execute(Model model)` returns a success message for the sorting task.

*Step 3*. The filtered list in `NotesRecord` and updates the User Interface of the newly sorted notes list.
