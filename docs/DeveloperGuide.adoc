...
Step 1. The user launches the application for the first time. The `commandHistory` will be initialized with an empty storage ,
...
Step 2. The user executes `delete 5` command to delete the 5th Spending in the spending book.
...
Step 3. The user executes `add n/David ...` to add a new Spending.
