...
The following are code snippets from `getRecommendedItems(description)`:
...
descriptions that are of at least length 3. While iterating through the `InventoryList`, these subsets are compared with the
actual descriptions of all items in the inventory to check if either contains the other.
