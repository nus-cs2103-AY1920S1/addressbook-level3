...
are extremely relevant in the real world and knowldege that I can use when I work in the future. It has allowed me
to be a more competent and knowledgable software engineer.

== PROJECT: TutorAid

== Overview

TutorAid is an easy-to-use Command Line Interface (CLI) based application that helps to makes the lives of tutors easier
by organizing their workflow and all their related tasks into one location. It is extremely beneficial to Computing
tutors as they are already highly adept at typing on CLI interface and TutorAid also provides a Graphical User Interface (GUI) interface for users to easily view and interact
with TutorAid.
...
application and thus, TutorAid was created. All of TutorAid's features and implementations are well documented
...
* *Major Enhancement 2*: Register and Login feature.
...
** Implementation: This enhancement alters the way TutorAid is used. Normally, users could enter into the application once they launched it.
 But with the register and login feature, it only allows users to login when they have the correct credentials. In addition, for future implementations, data
encryption and file encryption will be added as well. If more modifications are allowed, the account creation would linked to a backend server such that more security can be provided.
 The register and login feature is aided by a password encryption method. When users register, the passwords of the accounts created will be hashed with a salt so that there will be more security and less
invasion of data. I created a Login Window as well so that the users can have a visual cue when they login into their account.
