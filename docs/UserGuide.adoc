...
By: `AY1920S1-CS2103-T11-1`      Since: `Sep 2019`      Licence: `MIT`
...
* Date/time, operator name, and operator name is autofilled.
****
=== Submit a report: `submit`
Submits a just-filled report if one is available, else shows a list of unsubmitted draft reports. +
Format: `submit`
****
* Once a new report has just been filled, the IMS prompts the operator to type `y` to submit. The operator can either type `y` or `submit` to submit the report.
* If the operator issues the `submit` command at any other time, they will be shown an indexed list of unsubmitted draft reports. The IMS will then prompt them to enter the index number of the report they wish to submit. The operator types that number and presses `enter` to submit the chosen report. 
* The submission of a report will trigger three other operations: 1) an ID will be generated to tag the report, 2) an automated email will be sent to the operator's supervisor, and 3) the report will be saved in the database.
****
=== Adding a person: `add`
