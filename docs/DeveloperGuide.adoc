...
// tag::followup[]
=== Follow-up and Reminders feature
==== Implementation
Both mechanisms are facilitated by the `Reminder` class.

It implements the following operations:

* `Reminder#addReminder(reminder)` -- Adds a new reminder.
* `Reminder#getReminders()` -- Retrieves the reminders for output.

These operations are exposed in the `Model` interface as `Model#addReminder(reminder)` and `Model#getReminders()` respectively.
...
image::Profile0.png[]
_{Show activity diagram of opening profile window}_
...
image::Profile1.png[]
_{Show activity diagram of closing profile window}_

[NOTE]
The user is able to close the `ProfileWindow` form by clicking the 'x' on the top right of the form as well.

This sequence diagram summarizes what happens when a user executes the Profile Command:

image::ProfileSequenceDiagram.png[]
_{Show extended sequence diagram of opening and closing profile window}_
...
{*TODO* : Please update user stories if any new enhancements have been planned / made in your implementation. *DELETE WHEN DONE*}
...
2.  AddressBook adds new visitation record for chosen patient
+
Use case ends.
...
+
[none]
** 1a1. AddressBook shows an indexed list of visitation records
+
Use case resumes at step 1.
...
2.  AddressBook removes user-defined macro
...
[discrete]
=== Use case: Open Message of the Day

*MSS*

1. User requests to open Message of the Day
2. Message of the Day prompt opens with Follow-Up and Reminders details
