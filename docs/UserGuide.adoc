...
This command allows you to add a new deliveryman to the deliverymen database. The name and phone number must be
provided.
...
* At least one field must be edited (eg. name, phone number, tag(s)).
...
This command allows you to delete an existing deliveryman in the deliverymen database. The index of the deliveryman to
...
you can click on any of the three buttons to show the status list that you wish to view, as shown in Fig X.Y below.
...
will prompt the status lists as well (that can be done through `lists` command explained in _Section 3.4.4_).
...
NOTE: You are not allowed to change the status of a deliveryman who has the status *DELIVERING*. When you assign an
order to the deliveryman, his status will be automatically set to *DELIVERING*. When you set an order as
completed, the status of the assigned deliveryman will automatically be set back to *AVAILABLE*.
