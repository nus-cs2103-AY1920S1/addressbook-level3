...
image::./yijie/ScheduleViewDomainModel.png[]
...
After having the `ScheduleView` class that creates the graphics for TimeBook, we needed to control what graphics to show. As such,
...
`show g/Three musketeers` in the command line. The state of `ScheduleViewManager` will be initialised to show only the group's schedule for the first week.
...
The state of `ScheduleViewManager` is then modified to show the second week of the group's schedule.
...
executes the `lookat` command to inspect Alice's and Carl's schedules. The state of `ScheduleViewManager` is once again modified to only show the specified group members' schedules.
...
.2+| User experience of the schedule view.
| 1. Use a separate container for headers and schedule contents (table cells & blocks). **(Current choice)**
| 1. User can read the schedule in our application a lot more easier.
| 1. Duplicated codes in the ScheduleView class to generate the separate containers.

| 2. A single container to contain the schedule view.
| 1. Easier to implement, lesser lines of codes.
| 1. UX will decrease due to increase difficulty to read schedules in the application.

4+| We understand many users want to look at date headers as they use our application to plan their group meetings.
Therefore, we chose to implement separate headers and contents so that the user can still look at the date headers as they scroll the schedule view.
|===
...
| 1. Slightly more challenging to implement in order to make sure that the schedule view is not cluttered.
...
|===
| Aspect: | Choice | Pros | Cons
.2+| Color schemes for time slot blocks in schedule view.
| 1. Select about 15 good contrast colours to dark theme for schedules. **(Current choice)**
| 1. More aesthetically pleasing to the user.
| 1. Groups that have more than 15 members will have the same colours for their blocks in the schedule.

| 2. Randomly generate colours depending on group size.
| 1. All members in a group will very likely have different colours to indicate their schedule.
| 1. Schedules with darker colours cannot be seen from a dark theme UI.

2. The graphics of the schedule generated will always have different colour for every request.

4+| It is unlikely for users of TimeBook to have a large number of group members in a group. Most projects have a limit to 5 persons
per team and thus, it is unnecessary to randomly generate color for each group member. And even if a TimeBook user really has a group of
more than 15 group members, the order of where the blocks are placed corresponds to the order of group members list.
Hence, the user should still be able to figure out who's block it belongs to even if it is the same colour.
|===
