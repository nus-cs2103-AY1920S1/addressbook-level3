...
When a command is being executed, `LogicManager` will add it into the `UndoRedoStack`.
...
It will then call `UndoableCommand#undo()` of the command that is removed. This works as every UndoableCommand object stores the previous Model states.
