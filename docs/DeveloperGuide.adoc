...
---
I will now walk you through a Sequence Diagram to show how these commands utilise word bank's data structure and storage.

// tag::chenhui-findCommandSequenceDiagram[]
.Sequence diagram detailing a successful `CardCommand` through different components
image::FindCommandSequenceDiagram.png[]
// end::chenhui-findCommandSequenceDiagram[]
...
For example, "import w/bank1, f/~/downloads"

//1. Input gets parsed by the ParserManager. Depending on the input, a specific `Command` is returned.
//In this case, a `ImportCommand` object is instantiated.
...
//7. In addWordBank method, it `WordBank` is added into the `ObservableList<WordBank>`, which is the same as that
//contained within Model. It is to synchronise data with `Model` and `UI`.
...
`LoadBankPanel` is the corresponding class and fxml that displays the word banks for the user. +
It only has access to an `ObserverableList` of `WordBank`, which gets updated within `Model` or `Storage`. +
...
Afterwhich, the callback essentially performs an `ImportCommand`, to load the word bank.
