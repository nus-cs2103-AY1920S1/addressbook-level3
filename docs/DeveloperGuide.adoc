...
The undo/redo mechanism is facilitated by the `CommandHistory` class.
...
* `ReversibleActionPairCommand#execute()` -- Saves the current address book state in its history.
* `ReversibleActionPairCommand#undo()` -- Restores the previous address book state from its history.
* `ReversibleActionPairCommand#redo()` -- Restores a previously undone address book state from its history.
