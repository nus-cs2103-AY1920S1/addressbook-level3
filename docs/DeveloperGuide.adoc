...
Step 5. Upon successful updating of the current view list, a `CommandResult` object will be created by the `SearchCommandParser`
to encapsulate a positive feedback message that will be shown to the user. The `CommandResult` will then be returned
to the `LogicManager`.
...
* **Alternative 1 (initial choice):** Modify the input command format to include "&" as a separator. The "&" separator will be placed between 2 keywords to signify an AND condition between the them.
** Pros:
...
* **Alternative 2 (current choice):** Make the `search` command "stackable". Every `search` command will now only
...
* **Alternative 1 (current choice):** Modify the `ModelManager`.
** Pros:
...
* **Alternative 2:** Modify the commands.
** Pros:
