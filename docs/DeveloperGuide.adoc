...
=== [underline]#Word Bank data structure and it's features#
A quick look at `WordBank` and `Card` through it's `UI`.
{nbsp} +

image::HomeCommands.png[]

.`HomeCommands` and the `WordBank` it works on.
image::WordBanks.png[]

{nbsp} +

image::CardCommands.png[]
.`CardCommands` and the `Card` it works on.
image::Cards.png[]
{nbsp} +

==== Data Structure Overview
Internally, the `WordBank` contains an observable list.
