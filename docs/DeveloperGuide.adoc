...
interactions with each `Flashcard` object currently in `StudyBuddyPro`. The class
diagram below summarizes the relevant `Model` components that facilitate the `remind`
command.

image::remindDiagrams/RemindCommandModelsClassDiagram.png[]

The interactions of all relevant components will be explained by walking through
Use Case 9[TOADD LINK AND USECASE].
...
[TIP]
The `exit` command is a global command and can be used in any mode,
not just in the _flashcard_ mode.

An activity diagram describing the
...
When implementing the remind

===== Aspect: How to keep track of when a `Flashcard` object was last viewed?
...
===== Aspect: Should the user be given the option to toogle the `exit` command reminder or should it be a set feature?
...
application, and forget about the feature since it is rarely used. When the user later wants
