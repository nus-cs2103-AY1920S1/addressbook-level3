...
This was a project for a Software Engineering Module CS2103T in National University of Singapore. I am in a team of 4 Computer Science students and we were given the task of enhancing the existing Command Line Interface application that the module provided us with. We decided to morph the application into a Financial Tracker for Finance Secretaries in organisations. The name of the application is FinSec.

This new application, allows for these Finance Secretaries to easily store their claims requests, income inflows and keep track of budget accurately. On top of these, the secretary has to store the contacts of all the people who have made transactions with him so that it ensures easier tracking in the future.
...
* *Code contributed*: [https://nus-cs2103-ay1920s1.github.io/tp-dashboard/#search=lawncegoh&sort=groupTitle&sortWithin=title&since=2019-09-06&timeframe=commit&mergegroup=false&groupSelect=groupByRepos&breakdown=false&tabOpen=true&tabType=authorship&tabAuthor=lawncegoh&tabRepo=AY1920S1-CS2103T-W12-1%2Fmain%5Bmaster%5D
...
** Documentation:
*** Shifted the contents of the Developer Guide around as well as made major changes to my own section: https://github.com/AY1920S1-CS2103T-W12-1/main/pull/143[#143]
...
made for `sort` and `check` features. There is also a section on the 2 features I am proposing to implement in v2.0_
|===
...
I have made for `goto` and `check` features._
|===

include::../DeveloperGuide.adoc[tag=gotofeature]
