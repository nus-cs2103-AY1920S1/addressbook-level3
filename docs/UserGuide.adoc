...
E.g., *`addtx`*`de/DESCRIPTION a/AMOUNT`, `DESCRIPTION` and `AMOUNT` are parameters which can be used as *`addtx`*`de/AirPods a/299.00`.

* Items in square brackets are optional. +
E.g. *`badd`*`de/DESCRIPTION a/AMOUNT [c/CATEGORY]` can be used as
** *`badd`*`de/Saving for iPhone 11 Pro Max a/271.80 c/Gadgets`
** *`badd`*`de/Saving for iPhone 11 Pro Max a/271.80`.
...
E.g., if the command specifies *`addtx`*`de/DESCRIPTION a/AMOUNT`, *`addtx`*`a/AMOUNT de/DESCRIPTION` is also acceptable.
====
...
IchiFund data are saved in the hard disk automatically after you enter any command that changes the data. +
...
transactions. `filtertx` does the job for you.
...
record your transactions. `addtx` eases this process.
...
Occasionally, you may make a mistake entering your transactions. Perhaps you entered the wrong amount, or made
a heinous spelling error in your description. Fret not! You can correct these mistakes using `edittx`, and it would
...
You can rectify this error simply by entering the command `edittx 2 a/20.00`:
...
influx of precious money, do take a moment to update IchiFund using `deletetx`.
...
==== Creating monthly budget: `addbud`

Format: `addbud de/DESCRIPTION a/AMOUNT [c/CATEGORY] [m/MONTH y/YEAR]`

You can use the `addbud` command to create a monthly budget.
...
`addbud de/General budgeting a/500`.


==== Deleting monthly budget: `deletebud`

Format: `deletebud INDEX`

You can use `deletebud` to delete the budget.
...
[start=2]
2. Type `deletebud 2` into the command bar and press enter.
You should see the _Saving my tummy_ budget disappear from the budget list.

==== Finding monthly budget by keywords: `findbud`

Format: `findbud KEYWORD [MORE_KEYWORDS]`

You can use `findbud` to find budgets with description that matches all the given keywords. To illustrate this, let us suppose you want to find all budgets with the word _saving_ in its description.
To do so, type `findbud saving` into the command bar and press enter.
You should see the search result in the budget list.

To revert the budget list so that all budgets are listed, simply execute `findbud` without any argument.
...
* *Transaction Find* : `filtertx [m/MONTH] [y/YEAR] [c/CATEGORY] [ty/TRANSACTION_TYPE]`
...
* *Transaction Repeater Find* : `findrep [s/KEYWORD]... [c/CATEGORY] [p/DAY_OF_MONTH]... [ty/TRANSACTION_TYPE]`  (Coming in v2.0)
* *Loan Add new* : `loannew a/Amount [d/DATE] [n/NAME]`  (Coming in v2.0)
* *Loan Payment* : `loanpaid i/INDEX [n/NAME] [a/AMOUNT]`  (Coming in v2.0)
* *Loan Categorical Search* : `loanshow [s/KEYWORD]`  (Coming in v2.0)
* *Loan Net Amount* : `loantotal [s/KEYWORD]`  (Coming in v2.0)
* *Loan Value Update* : `loanset i/INDEX [a/AMOUNT] [s/DATE] [n/NAME] [d/DESCRIPTION]`  (Coming in v2.0)
* *Loan Sort By Value* : `loansort s/BY`  (Coming in v2.0)
* *Budget Add* : `addbud de/DESCRIPTION a/AMOUNT [c/CATEGORY] [m/MONTH y/YEAR]`
* *Budget Delete* : `deletebud INDEX`
* *Budget Find* : `findbud KEYWORD [MORE_KEYWORDS]`
