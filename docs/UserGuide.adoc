...
Now, the ice cream that you just bought is on the tracking list as seen from the diagram below.
...
// tag::delete[]
==== Deleting an item or its fields(s): `delete`
If you would like to delete an item or its related fields from the main list, the `delete` command can be used.
...
Let's say you have just consumed the entire bag of potato chips.
...
To save you the trouble of keying in the details again, our system has already automatically added the chips to your replenish list for you.
...
Also, `undo` only works for the last 10 commands. Therefore, think before you type!

==== Redo an earlier command : `redo`
Redo an earlier command that you have entered in. +
Format: `redo`

This command works in the opposite direction as `undo`, it will redo any command that you have undid in the past.
