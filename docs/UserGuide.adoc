...
Employee Format: `edit_em INDEX [n/NAME] [g/GENDER] [p/POSITION] [no/PHONE] [e/EMAIL] [a/ADDRESS] [on/JOINDATE] [t/TAG]...`

Event Format Format: `edit_ev INDEX [n/EVENT NAME] [at/VENUE] [m/MANPOWER NEEDED] [on/START DATE] [till/END DATE] [t/TAG]...`
...
* `find Music` +
`delete_ev 1` +
Deletes the 1st event in the list shown after the `find` command.
...
*Step 5*: To *allocate* a particular employee to an event or an `allocatem` command, double-click the employee card on the left list. Notice the employee to allocate
