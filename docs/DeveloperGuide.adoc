...
* `AverageMap#asUnmodifiableObservableMap()` - returns an read only version of `internalMap`.
...
Step 2. The user enters `average a/daily rt/bloodsugar n/4` in SugarMummy to get daily average blood sugar.
The `AverageCommand` then calls `Model#updateFilteredRecordList` to filter a list of records containing only
specified record type. The `AverageCommand` then calls `Model#calculateAverageMap()` which updates the `internalMap` with
new average values.
...
then the command will fail to execute and throw a `ParseException`.
This step is illustrated in the activity diagram below.
...
===== Aspect: Data structure for `internalMap`.
...
** Pros: Reduce coupling as `Model` does not need to depend on average graph UI. Moreover,
when record list changes, the average graph can be updated automatically by adding listener.
** Cons: `ObservableMap` does not support ordered keys. Need to write more codes to sort the keys.
* **Alternative 2:** Use `TreeMap<LocalDate, Double>` where we store each average values in an
`Average` class.
** Pros: Ordering of keys is maintained by `TreeMap`.
** Cons: `Model` needs to be coupled with average graph UI so that the graph UI can be updated
when record list changes.

Choice 1 is chosen because cons of alternative 1 can be solved easily by converting `ObservableMap`
into a `TreeMap` at places that need ordered keys.

===== Aspect: How to display average graph.

The dilemma arises because user, especially recently diagnosed Type II diabetics,
do not know the normal range of BMI and blood sugar level. An intuitive and aesthetically
pleasing method suitable method is needed to convey this information to user.
