...
3) The `Password` object is called from the filtered password list using the index and copies the password value via
...
Given below is an example usage scenario and how the validations behave at each step.
...
Step 4. A `ParseException` is thrown from `ParserUtil#parseExpiryDate(String expiryDate)` and the error message is shown on the command box.

The following sequence diagram shows how the expiry validations work:
...
. Through the `goto note` command, the user arrives at the notes component of the app. The user then executes the command
`open 1`.

. The app recognises that it is an `open` command and proceeds to check if the index `1` given is a valid index using
...
. The user proceeds to edit the Title, Description and Content field of the note through this panel and saves his
...
Below is an example run through to illustrate how the undo command is executed:
...
. The user executes `delete 5` command to delete the 5th note in the note book. The delete command calls Model#commitNoteBook(),
causing the state of the note book before the `delete 5` command executes to be saved in the `*UndoStack*`. The current state
of the note book becomes the state of the note book after `delete 5` command executes.
. The user now decides that deleting the note was a mistake, and decides to undo that action by executing the `undo` command.
...
. The current state of the note book is first stored in the `*RedoStack*`.
...
For the redo command, a key point to note is that it is not logical to redo a command from the `*RedoStack*` when an undoable
command is executed. Therefore, anytime an undoable command is executed, the `*RedoStack*` is purged. This behavior is follows
...
By adopting a simpler implementation, we are able to introduce less bugs into system. Furthermore, this implementation better
...
[width="80%",cols="3,10,10",options="header"]
|=========================================================
|Aspect |Alternative 1 |Alternative 2

|Implementation of expiry tagging system
a|The system saves a copy of the entire address book whenever the commands are executed.

* Pros:
* Cons:

I decided to proceed with this option because
