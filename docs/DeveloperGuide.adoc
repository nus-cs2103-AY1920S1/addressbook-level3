...
* *Resetting food database* which clears modifications from the user on food list
...
.*1. Food Model*
It encapsulates `FoodName`, `FoodType`, and four `NutritionValues`. These fields are visualized in `FoodCards`, which collectively compose the `FoodFlowPane`.
The six fields are `Comparable` to support `sortFoodList` function. Additionally, `NutritionValues` are used by `FoodCalculator` to obtain summary statistics.
...
.*3. Predicates*
Both implement `Predicate` and hold desired conditions as collections, such as `List` and `Set`.
They literate through the whole list to select foods that matches any of given conditions.
...
The following class diagram summaries these main components interact with each other.
...
that are intended to be shown. The existence of a certain flag depends on available `FoodTypes`.
Flags will be eventually translated to <<recmf_predicates, FoodTypeIsWantedPredicate>> and applied on `UniqueFoodList`.

Data Structure::
A `HashSet` is used to hold specified `FoodTypes` that are translated from user-input flags.
[NOTE]
...
It is similar to and simpler than the implementation of specifying flags. A `List` of strings as food names will be supplied to <<recmf_predicates, FoodNameContainsKeywordsPredicate>>.
...
.*3. Sorting* +
It is implemented via supplying a `FoodComparator` to `UniqueFoodList#sortFoods`. Afterwards, to maintain or change the ordering, `model#sortFoodList()` method is added into `execute` method of relevant commands.
[NOTE]
...
** The default sort order is currently set to be by food type, which can be modified via `FoodComparator#DEFAULT_SORT_ORDER_STRING`.
...
Diagram::
The following sequence diagram shows how recmfmix operation works. It also indicates the general flow between `recmf` commands with `Model`, `Logic`, and `Ui`.
...
.. For the first time, the `foodlist.json` storage file will be created and written with the pre-loaded food data. Otherwise, data is loaded from the existing storage file.
...
.. `UniqueFoodList` resets its internal list to hold the sample data from from `SampleFoodDataUtil`.
...
==== Future Development Suggestions [Proposed Features]
