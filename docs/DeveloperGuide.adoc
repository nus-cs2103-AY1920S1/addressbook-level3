...
** Pros: Model does not need to be exposed to parser, decreasing coupling.
...
** Pros: Easy to implement.
** Cons: Users cannot add tags to themselves unless they are an admin.
...
A sufficiently advanced user can access the data file directly to manipulate account details. Data file encryption will resolve this issue, but will only be implemented in v2.0
// end::accessControl[]
...
*** User might confuse relevant flags, eg ds/ and l/, as they both pertain to districts.
...
* **Alternative 2:** Utilise separate command `vehicles`
