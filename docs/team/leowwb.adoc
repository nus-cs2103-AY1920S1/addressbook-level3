...
TIP: A tip or general piece of advice
...
** What it does: It allows the user to save a subset of their flashcards to a specified external file. The file format can be either '.docx' or '.json', and is inferred from the file extension specified by the user. To avoid clutter, unimportant data (e.g. user rating) is not exported with the flashcards.
...
** Credits: All writing of flashcards to the '.docx' and '.json' formats is done with the use of two external libraries - Apache POI and Jackson, respectively. 
...
include::../DeveloperGuide.adoc[tag=flashcardimport]
