...
Duke Code is optimized for those who prefer to work with a CLI (Command Line Interface) while still enjoying the
...
. Copy the file into the home folder for Duke Code.
...
Opens a window displaying the user guide within the GUI. In the user guide, all available functions within Duke Code
...
Your questions will be loaded the next time you open Duke Code.
...
*A:* Copy the problem set text file into Duke Code’s home folder on the other computer
...
with the file that contains the data of your previous Duke Code folder.
