...
the command `delete 1` in the Home (Transaction) tab.

.Component interactions for `delete 1` command within Home (Transaction) tab.
...
=== Cashier Tab
This tab will act as a shopping cart to add and record sales items to be sold from the inventory.
...
The following sequence diagram shows how the SetCashierCommandParser checks for existing person:

.Sequence Diagram of SetCashierCommandParser (cashier package)

image::SetCashierCommandCashierSeq1.png[]
...
image::SetCashierCommandCashierSeq2.png[]
...
==== Checkout Feature
This feature confirms all the sales items in the table as sold as one transaction under the `Sales` category.
...
During the execution of the command, `getCashier()` method will be called to create a `Transaction` object. If the
cashier is null, the command cannot proceed and Leo will prompt the user to set a cashier. +
