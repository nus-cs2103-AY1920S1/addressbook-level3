...
// tag::searchfitering[]
=== [Proposed] Search filtering feature
==== Proposed Implementation

The search filtering feature would be an enhancement to the current available search feature. Using different categories of predicates
to narrow down the search space of the archived module list. The user should be able to able to combine different categories
of search to find modules that passes all the user defined filters. This can be achieved through the use of `FilteredList`
which can be updated incrementally with each filter applied. This would require additional classes that searches their individual fields
for the keywords. For example:

* `DescriptionContainsKeywordsPredicate#test()` -- Tests that a `module` description matches any of the keywords given.
...
* `ModuleCreditContainsKeywordsPredicate()#test()` -- Tests that a `module` credits matches any of the keywords given.
* `PreclusionContainsKeywordsPredicate()#test()` -- Tests that a `module` preclusions matches any of the keywords given.
* `SemesterContainsKeywordsPredicate()#test()` -- Tests that a `module` semester matches any of the keywords given.

The user should be able to use any combination of filters and the order of input should not matter. Examples of valid find commands:
...
During the execution of `FindCommand#execute()`, each predicate is applied one by one to the `FilteredList`, the contents of the `FilteredList` are copied
before the next predicate is applied.

NOTE: Should one of the predicate result an empty `Filtered List`, it is possible to skip the rest of the predicates.
