...
* `Override` the `getPrintableString` in classes `Name`, `Topic`, `Email`, and `Difficulty` so that each produces a printable string representation of the object. e.g. `Name: John Smith`, `Topic: 12349862`
...
* Consider adding a new command to the Duke Academy. e.g. an `edit` command. Notice how little you need to change in the `LogicManager` class to extend its behavior so that it can execute the new command.
