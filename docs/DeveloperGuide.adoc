...
*Pros*: No need to construct `*EncryptedFile*` objects for invalid files. +
*Cons*: Both the command parsing and execution requires interaction with the file system. `*Parser*` violates the single responsibility principle.
+
...
*Pros*: No need to create extra classes in `*Model*`, such as `*ViewableFile*` +
*Cons*: UI has to interact directly with the file system
+
*Alternative 2*: Read file content during command execution +
*Pros*: File system access is limited to command execution +
*Cons*: Need to pass the file content to UI via `*CommandResult*`
+
