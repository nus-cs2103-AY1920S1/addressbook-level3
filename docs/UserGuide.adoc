...
** `title\` : Find all modules with *title* that contains the given *sentence*.
*** Example: `find title\ software engineering` will list all modules with titles containing `"software engineering"`.
** `desc\` : Find all modules with *description* that contains the given *sentence*.
...
* Sentences used in title\ and desc\ are tolerant of minor spelling mistakes.
** Example: `find desc\ enginering` will display results similar to `find desc\ engineering`.
* Search is performed using string matching, which can result results being shown by searching random keywords.
...
****
[NOTE]
title\ and desc\ will seach a single sentence. mod\, prereq\, preclu\ and sem\ will search for all modules with any of the keywords without checking for spelling errors.
...
If deadline list is empty, creates a list. Else, adds to the end of the list. Priority tags can be
`HIGH`, `MEDIUM` OR `LOW`.
...
==== `sort`(Coming in mid V1.4)

1.Sort by Time

Format: `deadline MODULE_LIST_NUM a/sort by/time`

Sorts deadline tasks according to time, latest deadline at the top and later ones below.

Example: `deadline 1 a/sort by/time`


2.Sort by Priority

Format: `deadline MODULE_LIST_NUM a/sort by/priority`
...
Example: `deadline 2 a/sort by/priority`
...
Format: `link add n/LINK_TITLE l/LINK_URL`
...
Format: `link edit n/LINK_TITLE [nt/EDITED_TITLE nl/EDITED_LINK]`

Edits the link identified by its current title for the respective module.

Example: `link edit t/course website nt/module website`

==== `go`

Format: `link go n/LINK_TITLE`
...
Format: `link delete n/LINK_TITLE`
