...
with the actual changes on data done with a call to model during `execute(model)`. We decided to extend this command pattern by creating a `statsPayload` object. This object will hold the user
queries for the statistic calculation (if there is any) and be bundled along with the `CommandResult` class.
