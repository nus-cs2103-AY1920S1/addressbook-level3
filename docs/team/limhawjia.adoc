...
====
Justification : +
As the complexity of our app increased, we realized that the functionality of the application can be split into 4 separate cohesive units. Each unit deals with a different aspect
of our application. These aspects are _commands_, _questions_, _program evaluation_ and  _notes_ respectively.
Instead of using a single logic class to encapsulate all of the functionality, each cohesive unit was made into its own logic class, keeping them loosely coupled.
This helped to make the codebase more testable and maintainable.
====
...
==== *Implemented Observer pattern* +
  ...
  * Allow various different components such as the UI to observe changes to the state of the application without introducing them as unnecessary dependencies.
====
Justification : +
...
* `execute()` no longer takes in `Model` as argument. Parsing of command arguments is also handled by the `Command` itself.
* Open-closed principle is maintained by requiring commands to be registered into the `CommandLogic` with a https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/function/Supplier.html[`Supplier`].
* The https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/function/Supplier.html[`Supplier`] is in charge of injecting dependencies into the various commands.

====
Justification : +
The previous implementation of the `Command` in AddressBook3 requires its execution to rely on a `Model` instance. The command arguments also had to follow a fixed format. However, as the complexity of our application grew, our commands started to vary. Most commands no longer relied on a `Model` to be executed. Commands also started to have their own unique rules for parsing arguments.
====
...
image::ProgramEvaluation.png[align="center" width="1000"]

.Compile error example
image::CompileError.png[align="center" width="1000"]

.Timeout error example
image::TimeLimitExceeded.png[align="center" width="1000"]
...
.Notes page preview
image::NotePage.png[align="center" width="1000"]
...
* [Refactor] - Redesign underlying architecture
* [Major Feature] - Evaluation of user program
* [Major Feature] - Note-taking feature
* [Minor Feature] - Tabs
