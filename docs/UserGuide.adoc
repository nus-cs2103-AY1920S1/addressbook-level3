...
The user creating the activity will be included automatically, and additional contacts can be specified by keywords. Only contacts found by valid keyword (keywords that find exactly 1 matching contact) will be added to the activity, rest will be ignored with warning message. Changes the current view to this activity (as if `view a/ACTIVITY_ID` was called).
...
* `activity t/Chalet p/John Doe p/` +
