...
We decided to call her ELISA (Exceptionally Loud and Intelligent Student Assistant). Our application enables users to add and track their tasks and events while receiving reminders for them. The assistant is in the form of a chat bot and she was given a sassy and humorous personality to enhance the user experience.
...
The sorting feature allows users to sort their items so that they are able to better arrange their items in a way that is most useful to them. Each list has its own sorting algorithm while there are also general sorting algorithm using the priority or a description of an item.
