...
// tag:tobuylist[]
=== [Proposed] ToReplenish/ToBuy List
==== Implementation
_{Explain here how the ToReplenish/ToBuy List is implemented}_
...
===== Aspect: How ToReplenish/ToBuy List executes
...
==== Implementation
_{Explain here how the set reminder feature is implemented}_
...
The undo/redo mechanism is facilitated by `VersionedXpire`.
It extends `Xpire` with an undo/redo history, stored internally as an `XpireStateList` and `currentStatePointer`.
Additionally, it implements the following operations:

* `VersionedXpire#commit()` -- Saves the current Xpire state in its history.
* `VersionedXpire#undo()` -- Restores the previous Xpire state from its history.
* `VersionedXpire#redo()` -- Restores a previously undone Xpire state from its history.

These operations are exposed in the `Model` interface as `Model#commitXpire()`, `Model#undoXpire()` and `Model#redoXpire()` respectively.

Given below is an example usage scenario and how the undo/redo mechanism behaves at each step.

Step 1. The user launches the application for the first time. The `VersionedXpire` will be initialized with the initial Xpire state, and the `currentStatePointer` pointing to that single Xpire state.

image::UndoRedoState0.png[]

Step 2. The user executes `delete|5` command to delete the 5th item in the Xpire. The `delete` command calls `Model#commitXpire()`, causing the modified state of the Xpire after the `delete|5` command executes to be saved in the `XpireStateList`, and the `currentStatePointer` is shifted to the newly inserted Xpire state.

image::UndoRedoState1.png[]

Step 3. The user executes `add|Strawberry|3/2/2020|1` to add a new item. The `add` command also calls `Model#commitXpire()`, causing another modified Xpire state to be saved into the `XpireStateList`.

image::UndoRedoState2.png[]

[NOTE]
If a command fails its execution, it will not call `Model#commitXpire()`, so the Xpire state will not be saved into the `XpireStateList`.

Step 4. The user now decides that adding the item was a mistake, and decides to undo that action by executing the `undo` command. The `undo` command will call `Model#undoXpire()`, which will shift the `currentStatePointer` once to the left, pointing it to the previous Xpire state, and restores the Xpire to that state.

image::UndoRedoState3.png[]

[NOTE]
If the `currentStatePointer` is at index 0, pointing to the initial Xpire state, then there are no previous Xpire states to restore. The `undo` command uses `Model#canUndoXpire()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the undo.
...
The `redo` command does the opposite -- it calls `Model#redoXpire()`, which shifts the `currentStatePointer` once to the right, pointing to the previously undone state, and restores the Xpire to that state.

[NOTE]
If the `currentStatePointer` is at index `XpireStateList.size() - 1`, pointing to the latest Xpire state, then there are no undone Xpire states to restore. The `redo` command uses `Model#canRedoXpire()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the redo.

Step 5. The user then decides to execute the command `list`. Commands that do not modify the Xpire, such as `list`, will usually not call `Model#commitXpire()`, `Model#undoXpire()` or `Model#redoXpire()`. Thus, the `XpireStateList` remains unchanged.

image::UndoRedoState4.png[]

Step 6. The user executes `clear`, which calls `Model#commitXpire()`. Since the `currentStatePointer` is not pointing at the end of the `XpireStateList`, all Xpire states after the `currentStatePointer` will be purged. We designed it this way because it no longer makes sense to redo the `add n/David ...` command. This is the behavior that most modern desktop applications follow.
...
* **Alternative 1 (current choice):** Saves the entire Xpire.
...
* **Alternative 1 (current choice):** Use a list to store the history of Xpire states.
** Pros: Easy for new Computer Science student undergraduates to understand, who are likely to be the new incoming developers of our project.
** Cons: Logic is duplicated twice. For example, when a new command is executed, we must remember to update both `HistoryManager` and `VersionedXpire`.
