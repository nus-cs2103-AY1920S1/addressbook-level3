...
image::tag/TagActivityDiagram.png[]
...
The information stored in the GuiState is linked to the `UI` through listeners. For example, when the attributes of the `TabManager` object is updated, it triggers callback functions in the UI controllers that update the state of the UI displayed.
