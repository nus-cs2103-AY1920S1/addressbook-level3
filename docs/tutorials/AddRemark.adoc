...
:imagesDir: ../images/add-visitationRecord
...
In this tutorial, we'll walk you through the implementation of a new command -- `visitationRecord`.
...
The command should have the format of `visitationRecord INDEX r/REMARK`.
An example of the command is `visitationRecord 2 r/Likes baseball`.

We'll assume that you have already set up the development environment as outlined in the Developer's Guide.

== Create a new `visitationRecord` command
...
public class RemarkCommand extends Command {
    ...
    public static final String COMMAND_WORD = "visitationRecord";

    @Override
    public CommandResult execute(Model model) {
        return new CommandResult("Hello from visitationRecord");
    }
}
----
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `AddressBookParser#parseCommand()` to recognize the `visitationRecord` keyword.
...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the visitationRecord of the person identified "
            + "by the index number used in the last person listing. "
            + "Existing visitationRecord will be overwritten by the input.\n"
...
public class RemarkCommand extends Command {
    ...
    private final String visitationRecord;
     ...
     */
    public RemarkCommand(Index index, String visitationRecord) {
    ...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        throw new CommandException(String.format(MESSAGE_ARGUMENTS, index.getOneBased(), visitationRecord));
    }
...
Now let's move on to writing a parser that will extract the index and visitationRecord from the input provided by the user.
...
This appears to be what we need to get a String of the visitationRecord.
...
public RemarkCommand parse(String args) throws ParseException {
    ...
    String visitationRecord = argMultimap.getValue(PREFIX_REMARK).orElse("");

    return new RemarkCommand(index, visitationRecord);
}
----
...
That means we should add a `Remark` class so that we can use a `Remark` object to represent a visitationRecord given to a person.
...
== Add a placeholder element for visitationRecord to the UI
...
```
<Label fx:id="visitationRecord" styleClass="cell_small_label" text="\$visitationRecord" />
```
...
.$visitationRecord shows up in each entry
image::$Remark.png[]
...
public PersonCard(Person person, int displayedIndex) {
    //...
    visitationRecord.setText(person.getRemark().value);
}
----

.The visitationRecord label is bound properly!
...
----
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added visitationRecord to Person: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed visitationRecord from Person: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
...
Let's create a test for `RemarkCommand#execute()` to test that adding a visitationRecord works.
