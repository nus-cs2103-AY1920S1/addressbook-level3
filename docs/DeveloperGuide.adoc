...
* After validation of the individual objects, an `Earnings` object would be created with the parameters `date`, `classId`
 and `amount`.
...
** `description` would use `ParserUtil#parseDescription()` to ensure that the description typed by the user is in the correct format.

** `time` would use `ParserUtil#parseTime()` to ensure that the content is in the correct format.
...
`AddReminderParser#parse()`. In this case, it would use `ParserUtil#parseDescription()` and `ParserUtil#parseTime()`.
It would then return a `AddNotesCommand` object with an `Notes` object.
...
`Reminder` object with the same parameters created previously, it would then be added into the notes list.
