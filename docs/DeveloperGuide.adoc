...
The Health Records feature allows user to record their health progress using the commands
`add health`, as well as making record changes with `edit health`, `delete health` and `clear health`.

Additionally, users can create and personalize their own profile with commands `add profile`,
...
It can be observed that `Person` share similar attributes to `Record`, *Weight* and *Height*.
...
will be called. The `LinkProfile` acts as a bridge for `AddRecordCommand` to invoke `EditProfileCommand`,
...
|Way of implementating Record `Type`
|`Enum` class is used to store record types.

- **Pros**: More static control over the record types with varying validity checks. +
- **Cons**: Limited options of health record types as no changes can be made. +
...
- **Pros**: Increase flexibility and possible extension of having user defined health types. +
