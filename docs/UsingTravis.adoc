...
* Runs the `./gradlew clean test -i` command (see <<UsingGradle#, UsingGradle>> for more details on what this command means).
...
.  Wait for Travis CI to finish running the build on your new commit. You should see your `README` file displayed.
