...
/**
 * Changes the remark of an existing note in the address book.
 */
...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the note identified "
            + "by the index number used in the last note listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the note in the filtered note list to edit the remark
     * @param remark of the note to be updated to
     */
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the note's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a note.

=== Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.note`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let's go on a 5 minute adventure to get some placeholder text to show up for each note.
...
public PersonCard(Person note, int displayedIndex) {
    //...
    remark.setText(note.getRemark().value);
}
----
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedNote = new Person(personToEdit.getName(), personToEdit.getPhone(), personToEdit.getEmail(),
                personToEdit.getAddress(), remark, personToEdit.getTags());

        model.setPerson(personToEdit, editedNote);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedNote));
    }
