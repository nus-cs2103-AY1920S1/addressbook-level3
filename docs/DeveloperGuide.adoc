...
4. The `ModelManager` retrieves a `Predicate<Transaction>` from the new `TransactionContext`.
5. The `ModelManager` filters the list of transactions using the `Predicate<Transaction>`.
