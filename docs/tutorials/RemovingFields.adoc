...
In scenarios like this, you can consider refactoring the existing `food` model to suit your use case.

In this tutorial, we'll do exactly just that and remove the `address` field from `food`.
...
The `address` field in `food` is actually an instance of the `seedu.address.model.food.Address` class.
...
Other usages like in `EditFoodDescriptor` may require more careful inspection.

Let's try removing references to `Address` in `EditFoodDescriptor`.

. Safe delete the field `address` in `EditFoodDescriptor`
...
For example, removing the `address` field from the `food` class will require you to modify its constructor.
...
For example, there is a `$address` in each `foodCard` that has not been removed nor identified.

image::$address.png[width=1090px, height=890px]

A quick look at the `foodCard` class and its `fxml` file quickly reveals why it slipped past the automated refactoring.
...
.invalidfoodAddressBook.json
[source, json]
```
{
  "foods": [ {
    "name": "food with invalid name field: Ha!ns Mu@ster",
    "price": "9482424",
