...
This feature allows users to set reminders for their tasks with certain deadline and delete them when respective tasks are completed.
...
These two attributes are also utilized in formatting how a reminder should be displayed on the User Interface.
...
In this command, the parameter values behind the prefixes are string values for `parser` to convert them into `Date` and `ReminderMessage` attributes later on.
...
To illustrate how a reminder is added to the Model component when user inputs `AddReminderCommand`, the sequence diagram is shown below.
...
[NOTE]
The above sequence diagram only demonstrates how an `AddReminderCommand` is constructed from valid user input and how it is executed. It does not show how a reminder is added into the storage component and how the UI component is updated.

The following are the brief explanation of <<AddReminderSequenceDiagram>>

1. The user enters `reminder add d/today m/Pay bill`.
2. `SpendingBookParser#parseCommand()` is called by `LogicManger`
3. This creates `ReminderCommandParser` and it checks if the given user input is to add a reminder, delete a reminder or invalid.
4. Next, `AddReminderCommandParser` is created and it validates the parameters provided by `ReminderCommandParser`.
5. After successful validation, `AddReminderCommand` is created and passed to back callers until it reaches to `LogicManager`.
6. `LogicManger` then executes the command, adding the new reminder to the model through `Model#addReminder()`.
...
5. After successful validation, `DeleteReminderCommand` is created and passed to back callers until it reaches to `Logic Manager`.
6. `LogicManger` then executes the command, removing the specified reminder from the model through `Model#deleteReminder()`.
...
1. The date value of the deadline attribute is limited to before the end of next year and must be in the future. This prevent users from entering overdue tasks as inputs.

2. In this feature, the reminder message cannot be constructed with empty spaces. The purpose of this limitation is to prevent users from entering a reminder without specifying a message to be displayed.
// end::reminder[]
