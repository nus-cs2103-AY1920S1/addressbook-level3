...
* Items with `…`​ after them can be used multiple times including zero times e.g. `[goal/GOAL]...` can be used as `{nbsp}` (i.e. 0 times), `goal/lose weight`, `goal/lose weight goal/sleep early` etc.
* Parameters can be in any order e.g. if the command specifies `d/description dt/2018-01-20 12:00`, `dt/2018-01-20 12:00 d/description` is also acceptable.
====
//tag::datalog[]
=== DATA LOG
