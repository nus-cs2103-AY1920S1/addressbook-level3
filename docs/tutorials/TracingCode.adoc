...
command and store any possible changes in an `EditContactDescriptor`. Recall that we can verify the contents of
`editContactDesciptor` through the `Variable` tool window.
+
...
public CommandResult execute(Model model) throws CommandException {
    ...
    Contact editedAccommodation = createEditedContact(contactToEdit, editContactDescriptor);
    if (!contactToEdit.isSameContact(editedAccommodation) && model.hasContact(editedAccommodation)) {
        throw new CommandException(MESSAGE_DUPLICATE_CONTACT);
    }
    model.setContact(contactToEdit, editedAccommodation);
    model.updateFilteredContactList(PREDICATE_SHOW_ALL_CONTACTS);
    return new CommandResult(String.format(MESSAGE_EDIT_CONTACT_SUCCESS, editedAccommodation));
}
----
...
`JsonPlanner#savePlanner()`.
. Again, it appears that the heavy lifting is delegated. Let's take a look at ``JsonSerializablePlanner``'s
constructor.
+
.JsonSerializablePlanner#JsonSerializablePlanner()
[source, java]
----
/**
...
----
. It appears that a `JsonAdaptedContact` is created for each `Contact` and then added to the
...
this tutorial, you learned more about the inner workings of Planner and how the various components mesh together
...
.. Save the planner in the CSV format instead
.. Add a new command
.. Add a new field to `Contact`
.. Add a new entity to the planner
****
