...
=== Appointment feature
The Appointment feature provides users the ability to manage appointments for patients. It can help patients to add, change, delete appointments. It also helps user to find the missing appointments and settles each missing appointment efficiently.

* e.g. `newappt -id E0000001A -start 01/11/19 1800 -reoccur m -num 2` - allows the user to create two monthly appointments to patient whose referenceId is E0000001A.
* e.g. `editappt -entry 1 -end 02/11/19 1800` - allows the user to change a patient's first appointment timing to be the input timing if there is no conflict.
...
`newappt` Command - Adds a appointment or reoccurring appointments to a patient.

`ackappt` Command - Acknowledges a appointment once the patient reaches.

`appointments` Command - Lists all upcoming appointments or lists a specific patient's upcoming appointments.

`editappt` Command - Change a current appointment's timing to a different timing.

`cancelappt` Command - Cancels a appointment found within the appointment schedule.

`missappt` Command - Lists all missing appointments.

`settleappt` Command - Marks a missing appointment's status as settled.
...
Each Appointment object consists of a `PersonReferenceId`, `Timing`, `Status`. `Timing` class has 2 `DateTime` object as they indicates the starting and ending time of the appointment. The `UniqueEventList` contains 0 or more Appointments.

The current implementation of Appointment will do check with patient object by the unique referenceId and it will also check timing with doctors' dutyRoster. If the referenceId exists within the Model#UniquePersonList  and the timing is valid, then constructing the Appointment object. This ensures that the patient is registered before making an appointment and the appointment's timing is valid.

==== `newappt` Command
The addAppointment command behaves similarly to the registration of patient and doctor. The command takes in the parameters required to construct `ReferenceId`, `DateTime` and `Status`. The image below shows how the Appointment object is constructed.
...
The `ackappt` command will mark the patient's the most upcoming appointment's status as Acknowledge when the patient is reaching and it will update `UniqueEventList` to display the rest appointments that are belong to the patient.
...
The appointments with referce id behaves similarly to appointments with the exception that it does not take in any arguments. Instead, it automatically executes with the predicate EventContainsApprovedStatusPredicate. updateFilteredAppointmentList() is called and the entire list of upcoming appointments is shown to the user.
...
The missAppointment command searches for appointments that patients did not come. The filtered appointments are found in ModelManager. The list is instantiated by filtering the `UniqueEventList` using EventsMissedPredicate which checks all APPROVED-Status appointments' ending times with current time.
...
* e.g. `newshift -id W0000001A -start 01/11/19 0900 -end 01/12/19 2100 -reoccur m -num 2` - allows the user to create two monthly duty shifts to doctor whose referenceId is STAFF001.
...
`shifts` Command - Lists all duty shifts or lists a specific doctor's duty shifts.
...
The class diagram below illustrates the interactions between the Duty Shift class and its associated classes.
