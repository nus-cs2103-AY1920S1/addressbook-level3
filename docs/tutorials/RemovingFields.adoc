...
In this tutorial, we'll do exactly just that and remove the `address` field from `Problem`.
...
The `address` field in `Problem` is actually an instance of the `seedu.address.model.Problem.Address` class.
...
. Safe delete the field `address` in `EditProblemDescriptor`
...
For example, removing the `address` field from the `Problem` class will require you to modify its constructor.
...
For example, there is a `$address` in each `ProblemCard` that has not been removed nor identified.
