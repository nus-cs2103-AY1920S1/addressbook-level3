...
=== Categories
Ever feel like your list of expenses are extremely messy? This is where categories come in handy! Every entry belongs to one category, which makes it easier for you to organize your data! Categories can be either grouped under Expense or Income. Wishes and Budgets share the same category as Expense.
...
Example Usage:
[start = 1]
. If you would like to edit an existing Category *Business* to *Bursary* under Income, enter the command `editCategory o/Business n/Bursary`.
...
Example Usage:
[start = 1]
. If you would like to remove an existing Category *Food* under Expense, type `deleteCategory cat/Expense n/Food`
...
Format: `add typ/Expense c/<category> n/<description> a/<expenditure> d/<date> [t/<tag>]`

* `<category name>`: The `category` that the entry belongs to.
* `typ/Expense`: The type of the entry, which can be `Expense, Income, Budget, Wish`.
...
Example Usage:
[start = 1]
. If you would like to add a record of a recent splurging on mala, key in the command  `add cat/Food typ/Expense n/Mala amt/5.50 d/2019 11 09 tg/food tg/indulgence` .
...
Format: `add c/<category name> typ/Income n/<description> a/<expenditure> d/<date> [t/<tag>]`

* `<category name>`: The `category` that the entry belongs to.
* `typ/Income`: The type of the entry, which can be `Expense, Income, Budget, Wish`.
* `<description>`: The name of this entry.
...
key in the command  `edit 2 tg/irreversiblecatastrophicdestructiontomykidneysandhealth`.
...
Carrying out any other command in the Statistics Table screen will reset the Table to show the current month
...
Typing only the month in format *[yyyy-mm]* will show the breakdown of statistics by the specified month.
Carrying out any other command in the Statistics Pie Chart screen will reset the Pie Chart to show the current month
statistics for convenience of the user.
...
Format: `viewPie [p/<SpecifiedMonth>]`
...
[start = 1]
. Let's say you want to view the Statistics in bar chart form for period *2018/01*. You then type in the command `viewPie p/2018-01`.
. GuiltTrip will display to you the statistics for this period of time broken down in table form as shown below.
...
[start = 1]
. Let's say you want to view the entries sorted by Category in ascending order. You then type `sort typ/Category s/ascending`
...
Format: `find [c/<category>] [n/<description>] [a/<amount>] [d/<date>] [t/<tag>]`

Example:

* `find c/food n/mala`: lists each entry in expenditure and income with title containing keyword “mala” and category of Food.
* `find c/business d/2019-09-09`: lists each entry in expenditure and income with date of "2019-09-09" and category of business.
* `find d/2019-09-09 t/a t/b`: lists each entry in expenditure and income with date of "2019-09-09" and tags of a and b.

==== Search Wish: `findWish`

Search Wish by Category, Description, Amount, Date or Tags. There must be at least one find parameter.

Format: `findWish [c/<category>] [n/<description>] [a/<a>] [d/<date>] [/<tag>]`

Example:

* `findWish c/food n/mala: lists each entry in Wish with title containing keyword “mala” and category of Food.`
* `findWish c/business d/2019-09-09: lists each entry in Wish with date of "2019-09-09" and category of business.`
* `findWish d/2019-09-09 t/a t/b: lists each entry in Wish with date of "2019-09-09" and tags of a and b.`
...
Format: `findBudget [c/<category>] [n/<description>] [a/<expenditure>] [d/<date>] [t/<tag>]`

Example:

* `findBudget c/food n/mala`: lists each entry in Budget with title containing keyword “mala” and category of Food.
* `findBudget c/business d/2019-09-09`: lists each entry in Budget with date of "2019-09-09" and category of business.
* `findBudget d/2019-09-09 t/a t/b`: lists each entry in Budget with date of "2019-09-09" and tags of a and b.
