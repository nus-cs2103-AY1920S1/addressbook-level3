 ...
 * outside of the TemplateArchiveCommand code. For example, inputs "1" and "1 abc" take the
 * same path through the TemplateArchiveCommand, and therefore we test only one of them.
 * The path variation for those two cases occur inside the ParserUtil, and
 * therefore should be covered by the ParserUtilTest.
 */
public class TemplateArchiveCommandParserTest {

    private TemplateArchiveCommandParser parser = new TemplateArchiveCommandParser();

    @Test
    public void parse_validArgs_returnsArchiveCommand() {
        assertParseSuccess(parser, "1", new TemplateArchiveCommand(INDEX_FIRST));
    }

    @Test
    public void parse_invalidArgs_throwsParseException() {
        assertParseFailure(parser, "a", String.format(MESSAGE_INVALID_COMMAND_FORMAT,
                TemplateArchiveCommand.MESSAGE_USAGE));
    }
}
