...
[NOTE]
Tag names should be *alphanumeric*. This means that they should not contain spaces and special characters. +
...
{nbsp} +
To add the tags: +
1. Type `tag add 2 t/holiday t/hangout` and press `enter` to execute it. +
...
3. You could see the tags `holiday` and `hangout` in the expense.
image:tagging/AddTagThreeCopy.png[]

[NOTE]
You are not able to add *existing* tags. +
If you input *duplicate* tags, they will only be added once. i.e Duplicate tags in an expense is *not* allowed.
