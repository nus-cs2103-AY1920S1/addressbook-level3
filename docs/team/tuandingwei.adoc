...
** Highlights: The implementation of the CAP feature was challenging but satisfying. It requires in-depth design considerations around the students. The implementation also required an overhaul of existing commands and codes. The structure remains intact but the code required heavy refractoring along with new implementations. The single 'add' command initiates a series of major operations that generates the CAP, displays the right degree classifications and generates the pie chart. The implementation requires knowledge in object-oriented programming, data structure and algorithms. Many classes are interlinked hence increased the complexity of the modification and implementation.
...
* *Minor enhancement*: Added a sort command that allows the student to sort all the modules in chronological order.
