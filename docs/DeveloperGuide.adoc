...
Step 5. With the list of movies returned, the application can then read the internal `Movie` object returned. The `Poster` object in the `Movie` object can be used to load the poster of the movie, and the other fields can be shown to the user as seen in Figure 12.
...
The relationship between all the classes, and their methods are shown in the following class diagram in Figure 13:
...
===== Implementation
Given below in Figure 14 is a _sequence diagram_ of how the recommendations are generated when called from the `ApiManager`.
...
image::SearchActivityDiagram.png[][width=590"]
...
[NOTE]
If the user enters a wrong command, such as prefix other than that specified in the User Guide and incorrect keywords,
...
image::SearchSequenceDiagram.png[][width="790"]
...
possible ways to search for show
** Pros:
*** Easy to implement
...
*** Higher run time as there would be a need to search based on all 3 ways
...
image::AddActivityDiagram2.png[width="300"]
...
image::SyncSequenceDiagram.png[width="790"]


The following activity diagram summarises the workflow of Sync:

image::SyncActivityDiagram.png[width="300"]
