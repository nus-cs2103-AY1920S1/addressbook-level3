...
*** cons: Will bloat the UI since there will be extra details to be displayed
...
`CalendarPanel` keeps `UniqueList<Schedule>` and `UniqueList<Order>` objects from `Model` as class variables.
On instantiation, all the schedules in `UniqueList<Schedule>` will be converted to `Appointments` and then added to `Agenda`.
...
* If the user uses the schedule command -- calendar will display the week with the date requested by the user.

Changes to the schedules are made in the model by editing the `UniqueList<Schedule>` object.
`CalendarPanel` adds a listener to its `UniqueList<Schedule>` variable and listens to the changes and generates the updated list of `Schedules` into `Appointments` to add to `Agenda`.
...
`CalendarPanel` adds a listener to its `CalendarDate` variable and listens to the changes and sets the agenda view to the new `Calendar` in `CalendarDate` accordingly.
...
We do not keep a list of `Appointment` in `CalendarPanel` as `Appointment` is only used for `Agenda` while the rest of our code uses `Schedule`.
