    ...
    private static List<PrefixPosition> findAllPrefixPositions(String argsString, Prefix... prefixes)
        ...
        for (Prefix prefix : prefixes) {
            if (prefix.equals(PREFIX_TAG)) {
                List<PrefixPosition> tagsPrefixPos = findPrefixPositions(argsString, prefix);
                for (PrefixPosition prefixPosition : tagsPrefixPos) {
                    positions.add(prefixPosition);

                }
            } else {
                findPrefixPositions(argsString, prefix);
                int position = findPrefixPosition(argsString, prefix.getPrefix(), 0);
                if (position != -1) {
                    PrefixPosition prefixPosition = new PrefixPosition(prefix, position);
                    positions.add(prefixPosition);
                }
            }
        }
