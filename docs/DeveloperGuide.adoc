...
* `LoansManager#updateLoanStatus(Loan toUpdate, Status newStatus)` -- Updates the `Status` of `Loan toUpdate` to `newStatus`.
* `LoansManager#deleteLoan(Loan toDelete)` -- Deletes `Loan toDelete` from `loans`.

Finally, `LoansManager` also maintains a `persons` list, to keep track of the persons referenced by each `loan`.
The `persons` list contains no duplicates persons.

The state of `loans` is saved between sessions.
...
[NOTE]
If the `person` of the loan `toAdd` already exists in `persons`, then it is set to reference the existing `person`.
Otherwise, the `person` referenced by the loan `toAdd` is added to `persons`.
...
`LoansManager#updateLoanStatus(Loan toUpdate, Status newStatus)` is called
...
[NOTE]
If `toUpdate` cannot be found by the `LoansManager` in `loans`, an error message will be displayed to notify the user.
...
[NOTE]
If the `person` that `toDelete` references no longer has any `loan` that references it after the deletion of `toDelete`,
then the `person` will be deleted from `persons`.

[NOTE]
If `toDelete` cannot be found by the `LoansManager` in `loans`, an error message will be displayed to notify the user.
