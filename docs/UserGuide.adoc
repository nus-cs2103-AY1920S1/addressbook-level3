...
- Venues that are too long will be truncated / not visible on the calendar schedule. Simply hover your cursor over the schedule, the full venue should appear as a temporary pop-up in a while.
****
...
****
* As `find` commands accept an unlimited number of keywords, the system will continuously suggest
the same set of possible keywords when space is entered.
...
dates that already have orders scheduled on them. This helps check for duplicates.
