...
As seen from the above, a new member with name 'New Member', member id 'NM' and tag 'UG' is added to +Work.
...
As seen from the above, the specified image is set as the profile picture of the member with member id 'NM'.
...
image::Assign.png[width="790"]

As seen from the above, the task 'Review Budget' with task id 1 has been added under member 'John Doe' with member id 'JD'.
...
image::Fire.png[width="790"]
This indicates that the task 'Review Budget' with task id 1 has been removed from its assignment to team member 'John Doe' with member id 'JD'. +
