...
development of an algorithm which internally checks schedule of individual employees and prevents
schedule clashes.
...
*automated/manual allocation of employees to event commands* and a *de-allocate command*, as well as the
...
to events, which is a key component for event management companies that our app is targeting.
The algorithm used to check for conflicting schedule is also highly beneficial for users as we
all know how tedious such tasks can be.
...
*** Reported bugs and suggestions for other teams (examples:  https://github.com/AY1920S1-CS2103T-F13-1/main/issues/222[1],
