...
===== Implementation
Rank Command extends the Command Abstract Class, the rationale is explained in `Choice of Design Pattern` sub-section of the Rank feature.

Rank Command has 2 sequential checks:
...
Finally, rank returns an athlete's report card for a competition. For example,
