...
.  On GitHub, create a new user account and give this account collaborator and admin access to the repo. +
   Using this account, generate a foodal access token https://github.com/settings/tokens/new[here].
+
[NOTE]
foodal access tokens are like passwords so make sure you keep them secret! If the foodal access token is leaked, please delete it and generate a new one.
+
[NOTE]
...
* Click `Generate Token` and copy your new foodal access token.
--
...
--
* name: `GITHUB_TOKEN`
* value: foodal access token copied in step 1
...
Otherwise, other people will be able to see the foodal access token and thus have access this repo. +
