...
.. View in a glance the score distribution of your class.
...
[NOTE]
To change the priority of a `high`, `medium` or `low` note, the prefix priority/ is to be followed by `unmarked`. (e.g. note 2 priority/unmarked).
...
Create statistics reports of student scores using excel data files. +
...
WARNING: Failure to adhere to the following specifications might result in either the excel file being rejected or erroneous statistics.
...
WARNING: Failure to adhere to the following specifications might result in some of the statistical data generated not being useful to the user.
