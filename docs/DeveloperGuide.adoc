...
This is accomplished by executing the Schedule Command using user inputted `ACTIVITY_INDEX`, `DAY_INDEX`and `start time`.
...
* **Alternative 1 :** Create a new `Day` class with updated activity list for that day.
...
* **Alternative 2 (current choice):** Directly updates the activity list in the current `Day` class.
** Pros: More intuitive and simple to implement. Requires less computational time to execute.
