...
The above diagram shows the general design of the Logic component within each package. Further details on the
implementation of the Parser and Command can be found below in <<RTLogic, Section 2.3.1. Home and Reimbursement Tab>>,
<<ICLogic, Section 2.3.2. Inventory and Casheir Tab>> and <<PLogic, Section 2.3.3. Members Tab>> below.

[[RTLogic]]
==== Home and Reimbursement Tab
This section will show the general structure of the Logic Component of the Home Tab and Reimbursement Tab.
Given below is a general Class Diagram showing the structure of the Logic Component for these 2 tabs.

.Structure of Logic Component for Transaction and Reimbursement Tab.
...
The parsers represented by the `ABCParser` and `XYZParser`:
`ABCParser`:
...
[[PLogic]]
==== Members Tab
This section will show the general structure of the Logic Component of the Members Tab. This is integrated from the
existing `AddressBook`. Given below is a general Class Diagram showing the structure of the Logic Component for these 2 tabs.

.Structure of Logic Component for Members Tab.
image::LogicForAB.png[]

Generally, the following steps are taken when a user inputs a command into the `CommandBox`.
...
[[GeneralLogicSD]]
Given below is a general Sequence Diagram depicting interactions within the `Logic` component.

.Interactions Inside the Logic Component for a Command
image::GeneralSequenceDiagram.png[]

Note that the diagram depicts only a general sequence of events within the `Logic` component.
More detailed Sequence Diagram's for commands in each Tab can be found in <<Implementation>>
Any deviations from this general sequence diagram will also be covered there.
...
Back Command where `BackCommand` calls `resetPredicate()`.

The `BackCommand` is not initialised by a parser but initialised by the `TransactionTabParser` instead.
