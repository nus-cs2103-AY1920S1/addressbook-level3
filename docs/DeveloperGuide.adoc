...
[[RTLogic]]
==== Details on `Logic` Implementation for Home and Reimbursements Tab
This section will show further details of the `Logic` component of Home Tab and Reimbursements Tab.
...
.Structure of `Parser` for Home and Reimbursements Tab.
image::LogicForR&T.png[]

[NOTE]
The `XYZTabParser` represents the `TransactionTabParser` for Home Tab and `ReimbursementTabParser` for Reimbursements
Tab.
...
This section will show further details of the `Logic` Component of Inventory Tab and Cashier Tab.
Given below is a Class Diagram showing the structure of `Parser` within the `Logic` component:
...
==== Details on `Logic` Implementation for Overview Tab
[[OverviewLogic]]
...
has to match a name already existing in the `AddressBook` which is shown in Members Tab.
...
.Sequence Diagram of the add command in Home Tab (transaction package)
...
[NOTE]
This update of the list of transactions is done for every command that is executed successfully in Home Tab.
...
to generate an updated list of reimbursements for the user to view in Reimbursements Tab. The following sequence diagram shows how
Reimbursements Tab is updated from the `MainWindow`:

[[update-reimbursement]]
.Sequence diagram of updating the reimbursement list in Reimbursements Tab (transaction package)
...
when there is a command executed successfully in Home Tab.
...
.Activity diagram of the add command in Home Tab (transaction package)
...
Since Reimbursements Tab tabulates the amount to be reimbursed to a person, if the inputted amount is a negative
amount to indicate a spending that needs to be reimbursed, Reimbursements Tab will update and show this record.
...
.Activity diagram of the delete command in Home Tab (transaction package)
...
The following sequence diagram which is referenced in <<GeneralLogicSD, 2.3. Logic component: Figure 5>>, shows how the `AddCommand` works and is referenced in <<GeneralLogicSD, 2.3. Logic component: Figure 5>>:
