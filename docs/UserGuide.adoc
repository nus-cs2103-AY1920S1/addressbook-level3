...
toggle between tabs by either clicking on the tabs themselves, pressing the `tab` key or by using
the `list` command in each feature.

[NOTE]
You can also use the `enter` key to return focus to the command box.

You can learn more about each feature's individual GUI and its respective commands in the coming sections.
...
Often, we strive to reach certain milestones in our CCAs. For example, in learning piano, you might be aiming to
