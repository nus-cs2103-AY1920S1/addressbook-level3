...
_fig 3.7.1 – Types of Tabs highlighted in Orange_
...
In the example in fig 3.7.1, there is a problem called "Sequences" in the current display tab.
...
Format: `st t/TAB_TYPE i/TAB_INDEX`
...
* `switchtab t/display i/1` – Switches to the first display tab (i.e. `problems` tab)
* `st t/1 i/1` – Same effects as the previous command but in a shorter format.
* `switchtab t/details i/3` – Switches to the third details tab
* `st t/2 i/3` – Same effects as the previous command but in a shorter format.
