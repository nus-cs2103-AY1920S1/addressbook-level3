...
//An unusual aspect of this design is the fact that `SelectiveBookmarkTagger` inherits from `BookmarkTagger`,
//rather than containing a `Tag` directly. The rationale for this will be explained in the following section:
//<<autotag-design-considerations>>.

The `BookmarkPredicate` class keeps track of multiple types of autotag conditions.
To do so, it maintains _separate sets of keywords_ for each condition category; for instance, name keywords are stored
...
image::AutotagState0.png[400, 350]
...
Bookmark `b2`, which matches the predicate's conditions, is replaced by a copy of itself
(`b3`) that contains the additional tag **Help**.

.The state of Mark after adding an autotag
image::AutotagState1.png[450, 400]
...
* A new bookmark with the name `JavaFX new scene` is created. This bookmark matches the conditions for the
autotag **Help**, so it is tagged **Help**.

.The state of Mark after adding and automatically tagging a new bookmark
image::AutotagState2.png[450, 400]
...
NOTE: The **sd** frame should cover the whole diagram, but it does not due to a limitation of PlantUML.
...
The activity diagram below summarizes what happens when a valid autotag is added.
The mechanism for tagging bookmarks when a bookmark is added or modified is similar.
...
This section explains the key reasons why certain implementations were selected
over others when designing the autotag mechanism.

[[autotag-representation]]
===== Aspect: How to tag bookmarks based on specific conditions

* **Alternative 1:** Implement all the logic in a single class, which first checks
...
* **Alternative 2 (current choice):** Separate the checking and tagging mechanisms into two classes. One
