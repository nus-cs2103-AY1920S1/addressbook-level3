...
to generate an updated list of reimbursements for the user to view in Reimbursement Tab. The following sequence diagram shows how
Reimbursement Tab is updated from the `MainWindow`:
...
[NOTE]
This update of the Reimbursement Tab is done for every command after the list of transactions is updated (shown in <<updateTL, 3.1.1. Add Command Feature: Figure 15>>)
...
Since Reimbursement Tab tabulates the amount to be reimbursed to a person, if the inputted amount is a negative
amount to indicate a spending that needs to be reimbursed, Reimbursement Tab will update and show this record.
