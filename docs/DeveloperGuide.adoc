...
==== Implementation
We store every single `**Transaction**` added by the user into `**ObservableList<Transaction>**`, which is a list object in `**TransactionList**`. This list gets updated whenever the user
adds, updates or removes a `**Transaction**` from it. We used an `**ObservableList**` because whenever there are changes to the list, any other component
of `**THRIFT**` using it will automatically reflect its changes.

We implemented adding a `**Transaction**` through the following commands: **add_expense**/**add_income**. This adding process leverages on polymorphism: `**Expense**` and `**Income**` are both subclasses of the abstract class `**Transaction**`.
...
.A Class Diagram illustrating the implementation of Tags inside a Transaction
image::tag-untag/Polymorphism.png[]

Due to the usage of  polymorphism, many of the driver functions in `THRIFT` uses `Transaction` for both `Expense`
and `Income` objects. For example, when tagging a new `Expense`/`Income`, the `TagCommandParser` will treat them as the same object:
...
. Missing parameters
. Incorrect syntax (i.e. missing prefix, if it is required)
. Illegal values in parameters (i.e. special characters and symbols entered for an integer-only field)
. Multiple occurrences of parameters which only expects a single entry
...
Step 1. The user launches the application with data from previous sessions. THRIFT contains only one `**Transaction**` containing no tags and the user is going to perform tagging and un-tagging operations on it.

image::tag-untag/TagUntagTransactionWoTag.png[]
...
[NOTE]
Tags that already exist inside the specified `**Transaction**` will be ignored, and if that results in no tags being updated, an error will be shown to the user.

Since the user input is valid, a `**TagCommand**` command object is created and executed. As a result, a copy of the `**Transaction**` object with the specified `**Tag**` inside replaces the original in the `**TransactionList**` at the same position.

image::tag-untag/TagUntagTransactionWTag.png[]
...
[NOTE]
Tags that do not exist inside the specified `**Transaction**` will be ignored, and if that results in no tags being updated, an error will be shown to the user.
...
The following activity diagrams summarises what happens when the user executes a command to **tag** or **untag** a `**Transaction**`.

.Activity Diagram of the Tagging process
image::tag-untag/TagActivity.png[]

.Activity Diagram of the Un-tagging process
image::tag-untag/UntagActivity.png[]
...
==== Implementation
**Update** mechanism utilizes <<Design-Logic, *Logic*>> operations with the `**UpdateCommand**` class in place of `**Command**`.
...
image::UpdateCommandDiagrams/UpdateStep1.png[]
...
NOTE: The 1st transaction specified here is an `**Income**`

.Example user input for update command
image::UpdateCommandDiagrams/UpdateStep2.png[]
...
image::UpdateCommandDiagrams/UpdateStep3.png[]

Step 4. `UpdateCommand#execute` replaces original transaction in the list with the updated transaction.

image::UpdateCommandDiagrams/UpdateStep4Part1.png[]
image::UpdateCommandDiagrams/UpdateStep4Part2.png[]
.Replacement of original transaction with updated transaction in `UpdateCommand#execute()`
image::UpdateCommandDiagrams/UpdateStep4Part3.png[]

===== Execution observed by user
The following activity diagram gives an overview of what the user observes when executing **update** comnand
...
- Cons: Allowing modification of transactions violates immutability principle.
...
The *List* command allows the user to bring up a list of `**Transactions**`, and filter it by month or by its tags.
This feature is implemented by using a `**Predicate<Transaction>**` to filter the `**Transactions**` by month or tags.
...
. Incorrect syntax (i.e. having a prefix that does not refer to month nor tag)
. Illegal values in parameters (i.e. special characters and symbols entered for a integer-only field)
...
Step 1. The user executes *list m/jan* to filter the list to bring up transactions of January.
...
Step 4. This `**ListCommand**` object will contain a `**Predicate**` which is used to filter through each `**Transaction**`.

Step 5. The final list of filtered transactions will be brought up.
...
** Pros: The filter feature could be easily improved in the future to further accommodate new arguments other than month and tag.
...
image::encryption/EncryptionClassDiagram.png[]
