...
* **`add`**`n/factorial d/define a function factorial that takes in a number and n and returns the factorial of the number t/recursion a/Halim` : adds a problem called 'factorial' tagged with 'recursion' and authored by 'Halim'
...
* Words in `UPPER_CASE` are the parameters to be supplied by the user e.g. in `add n/NAME`, `NAME` is a parameter which can be used as `add n/Two Sequences`.
...
[NOTE]
`apply` a problem-finding rule is essentially the same as executing a pre-filled `find` command.
...
==== Editing tag color: `edittagcolor`

Edits an existing tag's tag color +
Format: `edittagcolor INDEX c/TAGCOLOR`

Examples:

* `edittagcolor 1 c/RED`

[NOTE]
Tag color can be "GREEN" "BULE" "YELLOW" "RED" "PINK" "BLACK".
...
Format: `add n/NAME [d/DESCRIPTION] [start/START_DATE] [end/END_DATE]`
...
* *Edit tag color* : `edittagcolor INDEX c/TAGCOLOR` +
e.g. `edittagcolor 1 c/RED`
