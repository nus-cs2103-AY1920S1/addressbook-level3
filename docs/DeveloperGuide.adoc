...
* can save the Mark data in json format and read it back.
...
`Folder` is simply another field on `Bookmark`, just like `Url` or `Name`, and has a single `String` property that contains the folder the bookmark is located.
...
Step 2. The user executes `folder GER1000 p/Work` command to create a new `FolderStructure` in the subfolders of `work`. Starting from the `root`, a depth first search will be performed to locate `work`. When found, `ger1000` will be added to its subfolders.
...
The following sequence diagram shows how the folder operation works:
...
** Cons: The whole `BookmarkList` abstraction will have to be rerwritten
// end::folders[]
