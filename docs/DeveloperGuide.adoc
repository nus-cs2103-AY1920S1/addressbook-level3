...
** Cons: User may see a slight wait time whenever staistics are generated as it does not only update
the data for what's been modified but recalculates all the statistics each time, as it is not stored.
* **Alternative 2:** Generate statistics each time there is change in the data and store them.
** Pros: Statistical data will be displayed faster as it is recalculated and stored in the background after each change.
** Cons: Requires managing storage of statistical data and may take up extra memory resources.
// end::statistics[]
