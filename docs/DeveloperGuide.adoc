...
To avoid a dependency of the `Logic` module on the `UI` module when selecting an image, there is the interface
`NoteImageSelector` exclusive to the `parser.note` package whose `selectImage` method provides the necessary image.
