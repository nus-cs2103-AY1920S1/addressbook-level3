...
.Activity diagram of an instance of `GameTimer` when `Hints` are enabled
...
contains a `java.util.Queue` of timestamps (in milliseconds). `GameTimer` polls from the `HintTimingQueue` and checks against
these polled timestamps to update the `Hints` provided periodically. +
 +
The described activity is visualized via the activity diagram as *shown above*.
