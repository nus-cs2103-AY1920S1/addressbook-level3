...
public class UnLockCommand extends Command<DiaryModel> {
    ...
    private static final String MESSAGE_UNLOCK_ENTRY_SUCCESS = "Entry is now unprivate";
    ...
    public CommandResult execute(DiaryModel model) throws CommandException {
        ...
        if(!model.hasPassword()) {
            ...
            return new CommandResult(MESSAGE_UNLOCK_ENTRY_SUCCESS + targetIndex.getOneBased());
            //If the details don't match, don't show the memory
        } else {
            return new CommandResult(MESSAGE_UNLOCK_ENTRY_FAILURE + targetIndex.getOneBased());
