...
`UndoCommand` -- UndoCommand is not an UndoableCommand, it only represents a Command that undoes the changes of
the previous UndoableCommand. +

`RedoCommand` -- RedoCommand is not an UndoableCommand, it only represents a Command that nullify the changes of the
previous UndoCommand executed. +
|===

include::UserGuide.adoc[tag=undoredolist]

*Step 1.* When a user inputs a command, the application will parse the input using PlannerParser and generate the
Command object.

*Step 2.* For example, the user inputs `delete activity 5` command to delete the 5th activity in activity list.
The user's command input is pass into `PlannerParser to generate a Command`.

*Step 3.* The Command is then `checked if it is an UndoableCommand` or not. If it is an UndoableCommand, EventFactory
will parse the Command to generate an Event.

[NOTE]
EventFactory is a static class that will parse an UndoableCommand and generate the corresponding Event.

*Step 4.* The Command generate due to the user's input is a DeleteActivityCommand, which is an UndoableCommand.
Therefore, the `EventFactory generates a corresponding DeleteActivityEvent`.

*Step 5.* The Event generated is `added to the undoEventStack` stored in CommandHistory. The `redoEventStack in
CommandHistory is also cleared` upon identifying an UndoableCommand.

*Step 6.* The `DeleteActivityCommand is then executed`, generating the relevant CommandResult.

The following sequence diagram shows how the user's input is handled for `'delete activity 1'`:

.Generating and storing DeleteActivityEvent
...
*** Convenient for future extensions for new Commands added. Just need to ensure for every UndoableCommand, there is a Command
...
abstracted to their corresponding Event classes if they are UndoableCommands.
** Cons:
*** Every UndoableCommand requires a Command to reverse its effects. Hence a need to create Commands that are not utilized
by the user, but only used within application.
* **Option 2 :** Saves the entire Model date (comprising of accommodations, activities, contacts and days).
** Pros:
*** Easy to implement.
** Cons:
