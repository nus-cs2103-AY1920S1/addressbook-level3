...
The _Sequence Diagram_ below shows how the components interact with each other for the scenario where the user issues the command `deleteprob 1`.
...
The `UI` component uses JavaFx UI framework. The layout of these UI parts are defined in matching `.fxml` files that are in the `src/main/resources/view` folder. For example, the layout of the link:{repoURL}/src/main/java/seedu/algobase/ui/MainWindow.java[`MainWindow`] is specified in link:{repoURL}/src/main/resources/view/MainWindow.fxml[`MainWindow.fxml`]
...
image::design/model/PlanSearchRulePackageDiagram.png[width='90%']
...
.. Test case: `addprob n/1234 ` +
   Expected: No problem is added. The name of the problem cannot contain numbers.
...
.. Test case: `editprob x` +
   Expected: No problem is edited. Index of problem should be a valid integer.
