...
==== Set a task's status to `doing`: `doing-task`
...
==== Set a task's status to `done`: `done-task`
...
This sets the status of task 2 to `done`.

==== List all existing tasks: `list-tasks`
...
* Type `list-tasks` from whichever view you are in.
