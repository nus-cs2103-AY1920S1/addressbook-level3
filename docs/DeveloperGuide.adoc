...
The following sequence diagrams break down the intricacies in the view operation that works to display the sorted items:
...
Figure 15 pictures the process of retrieving the internal sorted list of items of `ReplenishList`.
...
.Sequence Diagram showing how sort command changes order of items
...
.Sequence diagram illustrating the correction mechanism for command "sort|dat"

image::RecommendationsSortSequenceDiagram.png[]

The following sequence diagram `find similar words` illustrates the process omitted above.
