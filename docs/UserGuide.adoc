...
Shows a list of all problems in the address book. +
Format: `list`

=== Editing a problem : `edit`

Edits an existing problem in the address book. +
...
Edits the phone number and email address of the 1st problem to be `91234567` and `johndoe@example.com` respectively.
...
Deletes the specified problem from the address book. +
Format: `delete INDEX`
...
* `list` +
`delete 2` +
Deletes the 2nd problem in the address book.
* `find Betsy` +
`delete 1` +
