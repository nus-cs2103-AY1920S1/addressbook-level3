...
Makes the Food Card more compacted, depending on the user's preference. The optional information will now be displayed
in one line.
...
Makes the Food Display Card expand, such that each line only has one attribute of the food item.
...
Finds a food items based on specific keywords. Following which, the food items containing the specific keywords will
...
THe directions are as followed: `ASC` or `DESC`.
