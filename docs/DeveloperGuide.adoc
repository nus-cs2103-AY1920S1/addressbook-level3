...
=== Sorting feature
The Sorting feature allows users to sort their food items based on certain criteria. You are allowed to sort based on certain fields and directions. +
First and foremost, they will be able to sorts the food items based on their default ordering, where it is based on ascending price, name and then category. +
...
****
1. The `FIELD` are as followed: `NAME`, `DESCRIPTION`, `PRICE`, `CATEGORY`, `LOCATION`, `OPENING_HOURS`, `RESTRICTIONS`.
2. The `DIRECTION` are as followed: `ASCENDING` or `DESCENDING`.
****

==== Classes for sorting feature in Model

The Sorting feature was implemented with a new set of classes introduced to the Model. The `CustomSorter` stores the comparator for `autosort` and `makesort`. +
...
The `FoodComparator` stores fields which will be needed for various `FIELD` to be compared. +
The `DefaultComparator` helps to sort food items based on their natural ordering. This is called via the `default` method.
...
With the recommendation feature, a new set of classes were added. A singleton class `RecommendationSystem` encapsulates
the methods and classes related to this feature. This `RecommendationSystem` class implements the interface `Recommender`
which specifies the behaviour of the system, which include the ability to generate recommendation values.
...
using the `recommend` command. Each food is passed to the recommendation system and it calculates the
recommendation value based on the table below:

include::UserGuide.adoc[tag=recommendationbonus]

include::UserGuide.adoc[tag=recommendationpenalty]

The sequence diagram below shows how a sample Food is passed into `RecommendationSystem` to output a
recommendation value:

.Sequence diagram for `calculateRecommendation`
image::RecValueSequenceDiagram.png[width=400]
