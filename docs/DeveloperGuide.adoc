...
.  `Logic` uses the `MenuParser` class to parse the user command.
...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Menu`, which `Person` can reference. This would allow `Menu` to only require one `Tag` object per unique `Tag`, instead of each `Person` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
