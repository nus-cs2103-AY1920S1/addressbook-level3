...
Shown below is the sequence diagram (Figure 13) for when a scheduling conflict happens:
...
Shown below is the sequence diagram (Figure 14) for when a valid resolve command is issued:

.Sequence diagram when a valid resolve command is issued
image::ScheduleResolve.png[]

The following activity diagram (Figure 15) summarizes what happens when a user enters a `schedule` command:

.Workflow when a user enters a `schedule` command
...
* **Alternative 1(current choice): ** `SuggestPossibleCommand` to handle the predicates.
** Pros:
...
* **Alternative 2: ** Predicate Factory class to handles all predicates.
** Pros:
