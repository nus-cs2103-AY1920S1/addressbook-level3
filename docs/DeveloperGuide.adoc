...
command. The recommendation system will then take into account these preferences to generate a more accurate
list of recommendations. Users will then able to find the food that they are likely to enjoy more accurately.
...
Each `UserRecommendations` object contain 6 sets, namely:
...
The items stored in the sets are first converted to lowercase before adding them to the sets so that a case-insensitive
...
image::RecommendationClassDiagram.png[]
...
The _UserRecommendations_ class stores the list of user likes and dislikes as a set in lowercase to prevent duplicates.
...
A similar execution sequence is performed for a dislike command, replacing instances of like with dislike and vice versa.
...
We chose this design because of its simplicity of implementation. Additionally, adding a hidden `recommendationValue`
...
Furthermore, we do not have to manually update the recommendation of each `Food` item after every command with this design.
...
* Cons:
** The recommendation value of each `Food` must be updated before and after every command to ensure validity of
