...
* Assume you are planing to expand the functionality of the IncidentManager (but keep it as a CLI application).
What other user stories do you think IncidentManager should support? Add those user stories to the `DeveloperGuide.adoc`.
...
Assume that IncidentManager confirms the change with the user before carrying out the operation.
...
* Currently, IncidentManager data are saved to the file after every command.
...
Note how the `IncidentManager` class implements the `ReadOnlyIncidentManager` interface so that clients who don't need write access to the `IncidentManager` can access the `IncidentManager` through the `ReadOnlyIncidentManager` interface instead.

image::ReadOnlyIncidentManagerUsage.png[width=500]
...
* Do some enhancements to the IncidentManager GUI. e.g. add an application icon, change font size/style
...
* Consider adding a new command to the Incident Manager. e.g. an `edit` command. Notice how little you need to change in the `LogicManager` class to extend its behavior so that it can execute the new command.
...
* Is it possible to make the `IncidentManagerParser` class more OCP-compliant in terms of extending it to handle more
command types?
...
=== Exercise: Enhance IncidentManager

* Enhance IncidentManager in some way. e.g. add a new command
