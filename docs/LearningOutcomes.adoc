...
command types that mutate the data. e.g. `AddExpenseCommand`
...
* Add a `throws Exception` clause to the `AddExpenseCommand::execute` method. Notice how Java compiler will not allow it,
