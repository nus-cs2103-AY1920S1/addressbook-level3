...
the sorted list and remove the respective meals from the meal log.

The removal of meals is done through the `Model#removeMeal()` which would remove the meal from the meal log list.
...
The `Model` then returns the `Statistics` object that is created based on the `CalorieBudget` and `MealLog` in the `Model`.
Additionally, the following operations are implemented in `Statistics`:

* `Statistics#generateStatistics(CalorieBudget, MealLog)` -- gets the statistics of the current object. Maximum, Minimum and Average calorie intake per day of the month are calculated internally in this method.
