...
* e.g. `newappt i/001A str/01/11/19 1800 rec/m num/2` - allows the user to create two monthly appointments to patient whose referenceId is 001A.
* e.g. `editappt 1 str/02/11/19 1800` - allows the user to change a patient's first appointment timing to be the input timing if there is no conflict.
...
`cancelshift` simply takes in the index of the target appointment to cancel according to the displayed appointment list.
...
`newshift` Command - Adds a appointment or reoccurring appointments to a patient.
