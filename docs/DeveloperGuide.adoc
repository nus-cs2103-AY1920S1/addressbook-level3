...
=== [Proposed] Calendar week change
==== Proposed implementation

The calendar week change is enabled by creating multiple json files. Each file is used to save the user's task data
for one particular week. When user asks for the data to be cleared, the json storage file for that particular week would
be deleted. One additional json file `timetable.json` is created to save the user's module (lecture, tutorials, etc.) info as these are
persistent data that would be displayed on each week's calendar.

==== Design Considerations

===== Aspect: How next & prev executes

* **Alternative 1 (current choice):** Read and load data from the next/previous week's separate json storage file.
...
* **Alternative 2:** Save all data in a single json file.
...
1.  User request to add module
2.  Modulo requests user to input the module's time schedule
3.  User inputs the time schedule
4.  Module adds the new module to the user's timetable
+
Use case ends.
...
[discrete]
=== Use case: Edit lecture time

*MSS*

1.  User request to edit the lecture time of a certain module
2.  Modulo asks the user to input the new timing
3.  User inputs the time
4.  Module edits the lecture time in the user's timetable
+
Use case ends.
...
** 2a1. Modulo shows an error message.
+
* 4a. The module doesn't exist
+
[none]
** 4a1. Modulo shows an error message.
+
...
. Deleting a module's lecture/tutorial/lab
//.. Prerequisites: List all persons using the `list` command. Multiple persons in the list.
.. Test case: `deletemod CS2105 lecture` +
   Expected: CS2105 lecture is deleted from the calendar. If there are more than 1 lecture slots, both slots would be deleted.
.. Test case: `deletemod CS2105 lab` , CS2105 lab doesn't exist in the calendar +
