...
===== Implementation
As shown in  <<Logic component,Section 2.3>> above, both `XpireParser` and `ReplenishParser` implement the interface `Parser`.
...
==== Implementation
Items in both lists are also automatically sorted by their name then date in chronological order.
This auto-sorting mechanism is facilitated by `SortedUniqueXpireItemList` and `SortedUniqueReplenishItemList`
that both implement `SortedUniqueItemList`, in a relationship illustrated in  <<Model component,Section 2.4>> above.

`SortedUniqueXpireItemList` supports a new function, `SortedUniqueItemList#setMethodOfSorting()`, that specifies the
...
subsequently sorted based on the comparator defined.
...
Figure 13 above shows a `view|replenish` command executed to change the current view from that of the main tracking list `Xpire`
to the replenish list, while Figure 14 below materializes this process.
...
Figure 15 below pictures the process of retrieving the internal sorted list of items in `ReplenishList`.

.Sequence Diagram : retrieve internal sorted list
image::AutoSortViewPartSequenceDiagram.png[]
As items in the replenish list lack expiry dates, items are automatically sorted by their names. Therefore, a `nameComparator` is returned by default, as depicted above.
As the ability to sort manually by date is thereby also rendered irrelevant, it is disallowed entirely in the replenish list.
...
the `SortCommand` object created executes the `sort|date` command. Figure 17 below exhibits the specific process which sorts the items by their expiry dates.
...
Figure 24 below depicts the flow of events that check for spelling errors when a user executes an unknown command.
...
Figure 25 below on the other hand, presents what happens when a user executes a command with invalid arguments.
...
In the example below encapsulated in a sequence diagram, "date" is misspelled as "dat" in a sort command.
...
The sequence diagram titled `find similar words` here expands on the process omitted above in Figure 26.

.Sequence diagram showing how "date" is found to be the most similar word to "dat"

image::FindSimilarSequenceDiagram.png[]

The function `findSimilar` in `StringUtil` is called upon to return a set of strings that are most similar to the misspelled argument, `dat`.
