...
Employee Format: `add_em n/NAME g/GENDER p/POSITION no/PHONE e/EMAIL a/ADDRESS on/Join Date [t/TAG]...`

Event Format: `add_ev n/NAME at/VENUE m/MANPOWER NEEDED on/START DATEtill/END DATE[t/TAG]...`
...
Employee Format: `edit_em INDEX (must be a positive integer) [n/NAME] [g/GENDER] [p/POSITION] [no/PHONE] [e/EMAIL] [a/ADDRESS] [on/JOINDATE] [t/TAG]...`

Event Format Format: `edit_ev INDEX (must be a positive integer) [n/EVENT NAME] [at/VENUE] [m/MANPOWER NEEDED] [on/START DATE] [till/END DATE] [t/TAG]...`
...
Step 4: To perform a `free` command, click the *free* button as shown in the *Fetch Window* in step 2. The two lists will be updated as shown in the following figure.
Now, if you are interested to allocate a particular employee to an event, continue to step 5.

image::afterfree.png[width="600"]

Step 5: To *allocate* a particular employee to an event, double-click the employee card on the left list.
Notice the employee to allocate has moved to the list on the right as shown in the figure below:
...
>>>>>>> e097b68b5332ff2472d74fdfa6fea5d9526b153e
...
* *Help* : `help`
* *Search* : `search`
* *List Both Employees and Event* : `list`
...
e.g. `clear_ev_dt 2`
* *Generate Employee Payslip [coming in v2.0]*: `pay n/EMPLOYEE_NUMBER f/DDMMYYY t/DDMMYYY` +
e.g. `pay 2 f/20082019 t/20102019`
* *Generate Event Contract [coming in v2.0]*: `contract n/EVENT_ID` +
e.g. `contract n/005`
...
* *Generate Statistics [coming in v2.0]*: `stats_g` +
e.g. `stats_g`
* *Clear* : `clear`
* *Exit* : `exit`
