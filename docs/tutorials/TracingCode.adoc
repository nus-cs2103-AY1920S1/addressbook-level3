...
...
----
. Let's see what `EditStudentCommandParser#parse()` does by stepping into it.
...
`JsonClassroom#saveClassroom()`.
. Again, it appears that the heavy lifting is delegated. Let's take a look at ``JsonSerializableClassroom``'s
constructor.
+
.JsonSerializableClassroom#JsonSerializableClassroom()
[source, java]
----
/**
 ...
 * {@code JsonSerializableClassroom}.
 */
...
----
. It appears that a `JsonAdaptedPerson` is created for each `Student` and then added to the
`JsonSerializableClassroom`.
...
this tutorial, you learned more about the inner workings of Classroom and how the various components mesh together
