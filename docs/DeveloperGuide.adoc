...
image::LogicClassDiagram.png[]____
...
The diagram below shows a generalised view of the `Model` component within each package.

.Structure of the Generalised `Model` Component
image::ModelCD.png[]

[NOTE]
`XYZObjects` are only present in the `transaction`, `reimbursement`, `person` and `inventory` packages.
