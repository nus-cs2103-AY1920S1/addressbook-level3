...
****
* `TAB` input only accepts `transaction`, `budget`, `ledger` and `projection` in v1.4.
****
...
****
* You are only able to `sort` by `date` and `amount` in v1.4.
...
****
* `YEAR` only accepts integers from 1900 to 9999.
* `MONTH` only accepts integers from 1 to 12.
...
Imagine that you wanted to find out what you spent while shopping in October 2019.
