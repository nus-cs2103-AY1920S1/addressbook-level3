...
All `statsCommand` type will have call the constrcutor of `commandResult` class and pass the `StatsPayload` object in.
...
`mainWindow` class in SMLJ.

====== Main method to be considered here

Refer to the source code, the main overarching method is `executeCommand(String commandText)` in `MainWindow.java`,
in particular:

* `CommandResult commandResult = logic.execute(commandText)`;
* `performUiChanges(commandResult)`;

The first correspond to when a statistic command is entered by the user and parsed:

image::StatsCommandSequenceDiagram.png[]

After the logic component completes it execution and return a `CommandResult`, the second code method call
is for `UI` module to
handle the call with the `statsPayload` to the logic component:
