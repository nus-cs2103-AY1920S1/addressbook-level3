...
==== Adding priority tags to a study plan : `tagsp`
Attaches a priority tag to the study plan. +
Format: `tagsp PRIORITY_LEVEL STUDY_PLAN_INDEX`

[NOTE]
The `PRIORITY_LEVEL` must be entered in the format `priority_` followed by one of the priority level types
as shown below.

****
List of priority tags:
...
* `tagsp priority_low 3`

==== Removing priority tags from study plan: `removesptag`
Removes the priority tag from the study plan. +
Format: `removesptag PRIORITY_LEVEL STUDY_PLAN_INDEX`

Examples

* `removesptag priority_high 1`
* `removesptag priority_low 3`
