...
Adds an event to a person in the TimeBook. Only events without clashes in the person's existing schedule can be added to TimeBook.
...
Delete all schedule data of the specified person from the TimeBook +
Format:
...
e.g. `add -n James Ho -p 22224444 -em jamesho@example.com -a 123, Clementi Rd, 1234665 -r Always helpful -t friend -t colleague`
* *Edit Person* : `editperson -ed PERSON_NAME [-n NEW_NAME] [-p PHONE] [-em EMAIL] [-a ADDRESS] [-t TAG]...` +
e.g. `editperson -n James Lee -em jameslee@example.com`
* *Delete Person* : `delete -n NAME` +
e.g. `deleteperson -n Benedict`
* *Find Person* : `findperson -n KEYWORD [MORE_KEYWORDS]` +
e.g. `find James Jake`
* *Add Event to Person's Schedule* : `addevent -n NAME -e EVENT_NAME -t START_DATE-END_DATE-LOCATION` +
e.g. `addevent -n John -e CS2103T Lecture -t 18102019:1600-18102019:1800-i3`
...
* *Edit Group* `editgroup -ed GROUP_NAME [-g NEW_GROUP_NAME] [-r REMARK] [-d DESCRIPTION]` +
e.g. `editgroup -ed CS2103 -g CS3203 -r Project Discussion Group`
* *Delete Group* : `deletegroup -g GROUP_NAME` +
e.g. `deleteperson -n CS2103`
...
* *Export Visual Representation* : `export-vr [-n NAME]… [-g GROUP_NAME]…` +
e.g. `export-vr -g TimeBook`
