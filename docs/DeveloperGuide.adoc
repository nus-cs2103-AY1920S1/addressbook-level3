...
The search feature allows users to search for shows from the online database, the internal database, their watchlist or watched-list.
...
Step 3. A new `SearchCommand` is created, with the hash map containing the contents to be searched as a field of `SearchCommand`.
