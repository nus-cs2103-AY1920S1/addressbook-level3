...
****
* The account provided has to be exist before transactions can be added to it.
...
Edits the specified transaction, setting the specified fields. At least one field should be provided.
...
* If a new account is provided, that account will be the new active account.
****

==== Delete transaction(s): `txn delete`

Deletes the transaction with the specified ID on the current list of filtered transactions.
...
Lists the transactions from the current active account, with optional filters and sorting methods.
...
****
The explanation of the sorting abbreviations are as follows:

* aw: Sort transactions by ascending date.
* dw: Sort transactions by descending date.
* ax: Sort transactions by ascending amount.
* dx: Sort transactions by descending amount.
* ad: Sort transactions by description, ascending alphabetically.
* dd: Sort transactions by description, descending alphabetically.
****
// tag::transactions[]
...
* `setActiveAccount(nameOrIndex) -> Account`
