...
Usage:
 ...
 recur add [NAME] [DESCRIPTION] [AMOUNT] [START DATE] [END DATE] [INTERVAL]
+
Example:  +
...
. _Delete an existing occurrence:occur rm_ +
Deletes an existing occurrence that is being displayed on the GUI. +
{nbsp} +
Usage:
...
Example: +
`occur rm 3` +
Deletes the occurence with index 3. +
{nbsp} +

. _List all occurrence: occur list_ +
Lists all the occurrence in the record list. +
{nbsp} +
Usage:
...
Example: +
`occur list` +
Displays below occurences (for example) in the GUI: +
