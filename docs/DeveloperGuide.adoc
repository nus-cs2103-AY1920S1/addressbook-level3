...
`LogsCenter` plays an important role at the architecture level. Here is how it works:

* Uses `LogsCenter` to write log messages to the App's log file.
...
=== Reminder Feature
==== Implementation
A bookmark can have a reminder, and a reminder can use to open the bookmark.
This mechanism is facilitated by `Reminder` and `ReminderAssociation`.

`Reminder` containing a reminding time, the `Url` of the `Bookmark` and a `Note`.
...
* **Alternative 1 (current choice):** Use two hashmap to store both the relationship from reminder to bookmark and bookmark to reminder.
** Pros: More OOP and can easily find the relationship.
** Cons: More complex when doing any operation to a reminder. Need to check two hashmap.
