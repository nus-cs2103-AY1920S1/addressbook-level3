...
==== Prepare for a new session: `loadSess`
...
Format: `loadSess COMPETITION_NAME`

Example:

* `loadSess Test of Strength 2019`
...
==== Starts the session `start`
The session can be started after all participating athletes have submitted their 9 attempts for their 3 lifts.
This command will sort the athletes and the weight they are attempting in increasing order, after being sorted in this order:
Squat Attempts 1,2,3 , Bench Attempts 1,2,3 , Deadlift Attempts 1,2,3.
...
Format: `lift n/NAME ATTEMPT_INDEX Y/N`

Examples:

* `lift n/christabel teo 8 N`

* `lift n/shawn koh 3 Y`
...
Ends the ongoing session, and resets all data stored in the old session. 

(This command will also be triggered when the user keys in `next` when there are no more attempts to be made by any participants.)
