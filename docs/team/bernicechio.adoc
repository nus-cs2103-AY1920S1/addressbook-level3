...
====
* `genReports`:  A dark red text (also known as a mark-up) indicates that this is a command that can be inputted into the command line and executed by the application.
* `[purple]#GenReportCommand.java#`: A purple text indicates a component, class or object in the architecture of the application.
====
