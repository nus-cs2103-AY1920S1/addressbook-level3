...
Lets begin by explaining some key terms:
...
First, we take a quick look at `WordBank` and `Card` as it is displayed through it's `UI`.
...
I will talk about how the `Card` and `WordBank` is designed, along with `WordBankList` that encompasses the WordBanks.

Internally, the `WordBank` contains an observable list of `Card`.

[NOTE]
To avoid having the UI display different things with what is actually in the storage, the UI takes information from the
internal observable list within `Word Bank`, and we manipulate the internal list through `Word Bank` class.

.Class diagram of `Card`.
image::MyCardClassDiagram.png[]

.Class diagram of `Word Bank`.
image::MyWordBankClassDiagram.png[]

.Class diagram of `Word Bank List`.
image::MyWordBankListClassDiagram.png[]

.Simplified class diagram of `Model`.
image::Model.png[]

.Simplified class diagram of `Storage`.
image::Storage.png[]

==== Implementation of Word Bank functions Overview

This section describes the sequential flow of events in updating of a `Card`

.Sequence diagram describing the updating of `Card` using `CardCommands through different components
image::mySequenceDiagram.png[]

This section describes the sequential flow of events in updating of a `WordBank`

.Sequence diagram describing the updating of `WordBank` using `HomeCommands through different components
image::mySequenceDiagram.png[]

==== Implementation of Drag and drop feature

==== Design Considerations
There were a few considerations for designing the `Word Bank` Data structure this way.
// Talk about how the model contains WBL and WB, without reference to storage
...
3+|**Why did we choose Alternative X:** +
To do
|===

There were a few considerations for designing the `Word Bank` functions this way.
