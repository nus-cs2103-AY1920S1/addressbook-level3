...
This pairing is encapsulated as a `ReversibleActionPairCommand`, which consists of
two `ReversibleCommand` classes: the command to be executed and its inverse.
...
image::CommitActivityDiagram.png[width="400"]

The undo/redo mechanism is facilitated by the `CommandHistoryManager` class, which is found in the `logic` component.
The history stores actions that can be undone as a `ReversibleActionPairCommand` and implements the following operations:
...
On initialisation of the application, the undo and redo stack of the `CommandHistoryManager` would be empty.
...
Step 1. Suppose that the user has already executed several commands earlier, with the latest command being [blue]`newpatient ... -name John Doe`,
which registers a patient named 'John Doe'.

image::UndoRedoState2.png[width="500"]

Step 2. The user now decides that adding the patient was a mistake, and decides to undo that action by executing the [Blue]`undo` command.
The undo command will call `CommandHistoryManager#performUndo()`, which in turn
invokes the `UnregisterPatientCommand` which reverts the system to its previous state.

.The sequence diagram shows how the undo operation works:
...
The [blue]`undo` command uses `Model#canUndoAddressBook()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the undo.

After the successful execution of the `UnregisterPatientCommand` command,
the action from the top undo stack is transferred to the top of the redo history stack.

image::UndoRedoState3.png[width="500"]

Conversely, the [blue]`redo` command does the opposite -- it calls `CommandHistoryManager#performRedo()` which restores the address book to that state by invoking the original command again.
Similarly, if the `commandRedoHistory` is empty, then there are no undone states to restore. The redo command uses `Model#canRedoAddressBook()` to check if this is the case.
If so, it will return an error to the user rather than attempting to perform the redo.

Step 3. The user then decides to execute the command [blue]`patient S9482963D`.
A command that searches for a patient whose id matches [blue]`S9482963D`,
...
`CommandHistoryManager#performUndo()`, `CommandHistoryManager#performRedo()`.

Thus, the undo and redo stacks remain unchanged.

Step 4. The user executes [blue]`newappt ...` to schedule a new appointment for a patient.
This action invokes `CommandHistoryManager#addToCommandHistory()`, pushing the new action pair command in the undo stack.
However, the commands in the redo stack will be purged.

We designed it this way because it no longer makes sense to redo the [blue]`newpatient ... -name John Doe`​ command.
...
.AutoComplete Sequence Diagram showing the sequence upon typing the character 'a' into an empty Command Box.
image::AutoCompleteSequenceDiagram.png[]

.AutoCompleter Class Diagram
image::AutoCompleterClassDiagram.png[]
