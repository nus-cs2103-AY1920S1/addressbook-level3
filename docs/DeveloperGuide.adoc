...
Let's now see what happens when a command executes.
[NOTE]
...
. `ViewPanelPlaceholder` will then have to remove the `ViewPanel` child and add it back to allow the changes to be reflected in the GUI.
