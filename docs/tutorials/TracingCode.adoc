...
command and store any possible changes in an `EditEateryDescriptor`. Recall that we can verify the contents of
`editEateryDesciptor` through the `Variable` tool window.
+
...
public CommandResult execute(Model model) throws CommandException {
    ...
    Eatery editedEatery = createEditedEatery(personToEdit, editEateryDescriptor);
    if (!personToEdit.isSameEatery(editedEatery) && model.hasEatery(editedEatery)) {
        throw new CommandException(MESSAGE_DUPLICATE_PERSON);
    }
    model.setEatery(personToEdit, editedEatery);
    model.updateFilteredEateryList(PREDICATE_SHOW_ALL_PERSONS);
...
public JsonSerializableAddressBook(ReadOnlyAddressBook source) {
    eateries.addAll(
        source.getEateryList()
              .stream()
              .map(JsonAdaptedEatery::new)
              .collect(Collectors.toList()));
}
----
. It appears that a `JsonAdaptedEatery` is created for each `Eatery` and then added to the
