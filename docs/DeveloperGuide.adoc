...
The `Finance` Feature is implemented to allow users to get the detail of all employees in a easy manner.
...
3. `employeeSalaryPaid` - the total amount of salary has paid to th specific employee
...
**Step 7**. The command calls `Finance#updatecard()` to update the `Pending To Pay` of
...
*I decided to proceed with this option* because it creates less fields of employees, the 'Pending To Pay'
and 'Total Salary' is calculated on real time. User do not need to type so many fields when they add the employees.
