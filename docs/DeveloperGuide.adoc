...
Each budget, once added, will recur infinitely. This is achieved by the refresh() method, which checks whether
...
it has a method `getCurrentPeriodExpenses()` that generates a filtered list of expenses within the current budgeting
...
Additionally, util classes like `Percentage` which wraps a percentage for easier calculation and validation of proportionUsed.
...
The following activity diagram summarizes what happens when the app launches and when a new expense is added:
...
* **Solution (Current implementation):**
Adopts Alternative 1. Every expense must be tracked under one and only one Budget. If the expense is added before
...
The feature will read the expenses, and budgets and generate various metrics to be displayed.

The Statistic feature supports command words such as `stats`, `statscompare` and `statstrend`.
...
NOTE: To be included later once I figure out


1. The user enters the command `stats sd/01-10-2019 ed/01-11-2019` to visualise the total percentage of each category
...
2. The newly created `StatsCommandParser` calls `parse("stats sd/01-10-2019 ed/01-11-2019")`,
...
3. The `StatsCommandParser` object calls `ParserUtil.parseTimestamp(argMultimap.getValue(PREFIX_START_DATE).get())` which parses the arguments to get the start date.
...
4. The LogicManager calls `StatsCommand.execute()`, which calls `Model#getFilteredExpenseList()` to retrieve the list of expenses stored in MooLah.
...
5. Percentages, categories and the title description of the `Statistics` will be used to create the `CommandResult` object,
...
NOTE: As of Week 10, the design of `StatsCommand.execute()` is still a work in progress because of other concurrent priorities,
and other dependencies the old command has to support. This applies for the rest of the command words.

==== Design Considerations

===== Aspect: Why do we choose PieChart instead of other graphical formats?

* ** Alternative 1:** Table view
** Pros: Sequential view, focuses on number of entries
** Cons: May be a duplicate of the `ListExpense` command, just sorted via categories

* ** Alternative 2:** Pie chart
** Pros: Offers percentages of expenditure
** Cons: Loses the elements of number of entries in each category

* **Solution (Current Implementation): **
Adopts Alternative 2. It is important that every aspect of the user experience gets covered, so Statistics can be made more
meaningful by focusing on percentages instead of number of entries. This could very well be complemented with the list view
in the primary budget. In the event the user wants to view the individual components of a category, `FindExpense` can be used,
while the entire timeline of expense adding for the current budget can be accessed with `ListExpense` or simply the `Undo` command
to revert the page switching back to the default sequential view.
...
Users will be given the option to choose a Visual Representation method, which will determine the output format of statistics.
Currently, statistics under `stats` mode can only be displayed in a pie chart, and users may have different preferences for formats.
Various options such as checking across different budgets other than within the same budget can also be considered.
