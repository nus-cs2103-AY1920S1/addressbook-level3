...
Note: S1 means the 1st attempted weight for the Squat and so on. Users can use commands
`listPerson` and `listCompetition` to see the full set of persons and competitions respectively
in order to give the full set of ability to add any person to any competition in the system
...
Format: `startSession COMPETITION NAME`

Example:

* `startSession NUS 2022`
...
Ends the session, and shifts the user back to the out-of-session state.
This command is also automatically triggered when a competition has ended,
which means there are no more attempts for any lifters.
