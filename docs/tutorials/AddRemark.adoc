...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the book identified "
...
public class RemarkCommand extends Command {
     ...
     * @param remark of the book to be updated to
     */
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the book's title).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a book.
...
.PersonCard.java
----
public PersonCard(Person book, int displayedIndex) {
    //...
    remark.setText(book.getRemark().value);
}
----
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedBook = new Person(bookToEdit.getName(), bookToEdit.getPhone(), bookToEdit.getEmail(),
                bookToEdit.getAddress(), remark, bookToEdit.getTags());
     ...
     * {@code bookToEdit}.
     */
    private String generateSuccessMessage(Person bookToEdit) {
