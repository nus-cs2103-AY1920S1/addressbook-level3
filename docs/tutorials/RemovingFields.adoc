...
In scenarios like this, you can consider refactoring the existing `Student` model to suit your use case.

In this tutorial, we'll do exactly just that and remove the `address` field from `Student`.
...
The `address` field in `Student` is actually an instance of the `seedu.address.model.student.Address` class.
...
For example, removing the `address` field from the `Student` class will require you to modify its constructor.
