...
The following sequence diagram shows how items added are auto-sorted:

.Sequence Diagram modelling how items added are auto-sorted
...
.Sequence Diagram: set method of sorting as default
...
.Sequence Diagram showing how the view operation displays items in order
...
Given below are the steps of an example scenario of how `ViewPanel` is updated:
...
. `MainWindow` passes the list as parameters to the constructor of the `ViewPanel`.
. `ViewPanel` has a `displayItems` method that is able to construct a Collection of `ItemCard` by mapping each `XpireItem` or `Item`
to an `ItemCard` .
...
dose an additional check on whether the `XpireItem` has already expired. the remaining days before the `XpireItem` expires is less than or equal to its `ReminderThreshold`. If yes,
