...
==== Implementation
The game screen appears after the user enters the command `game` into the command box. A separate scene handled by a separate thread is created to run the game so that Elisa's scenes and threads are not overloaded. When the game screen is launched, the following classes and methods are invoked:
