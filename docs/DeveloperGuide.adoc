...
Upon calling of `editmode INDEX` command:
...
showing the restaurant under EditMode, while the `SplitPane` displaying the list of Restaurants
originally will be filled with the restaurant's `Food` menu instead and `Order` list.
