...
Below is a sequence of steps that illustrates the interaction between various classes when the `budget` command is entered.

<<addSeqDiagram>> is a sequence of steps that illustrates the interaction between various classes when the `goto contacts`
command is entered.
[[addSeqDiagram]]
...
==== Overview
In the Goto Command, there are various message attributes: `MESSAGE_SUCCESS_CONTACTS`, `MESSAGE_SUCCESS_CLAIMS`, `MESSAGE_SUCCESS_INCOMES`, `MESSAGE FAILURE`,
...
==== Current Implementation
Figure 4.3.1.1 is a sequence of steps that illustrates the interaction between various classes when the `goto` command is entered.
`goto claims`
...
* A `View` instance is then created when the ParserUtil:parseView method is called. This method takes in the argument from the GotoCommandPaser::parse method parameter
...
* The code snippet below shows the `GotoCommand::execute` method +
...
* A check to ensure that the model passed into the `GotoCommand::execute` method is not null is made before the rest of the method continues.
...
`MESSAGE_USAGE` that informs the user if the `check` command was successfully executed and if so which individual pop-up is showing.
==== Current Implementation
