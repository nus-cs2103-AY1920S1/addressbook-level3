...
* wants meal recommendations within their specified remainingBudget
...
|`* * *` |greedy user |have reminders when I am about to hit the limits I set for my expenditure |plan my remainingBudget better for the weeks ahead.
...
|`* * *` |user |only be able to see what I can afford for meals and beverages |save time scrolling through meals that fit my remainingBudget.
...
2. $aveNUS shows a list of recommended food items based on their specified remainingBudget.
...
[discrete]
=== Use case: Set remainingBudget

*MSS*

1. User requests to set remainingBudget.
2. $aveNUS calculates the daily remainingBudget based on the specified weekly remainingBudget.
+
Use case ends.
...
[none]
* 1a. The remainingBudget set is invalid (such as a negative number).
+
[none]
