...
To begin, let's download some .csv files with dummy data and import that data into Scheduler.

Click <<Importing existing availabilities `import`, this link>> to import a .csv file of availabilities into Scheduler!
...
=== I have an Excel file. How do I import that into Scheduler?
...
* The duration of a `s/SLOT` must follow the number of minutes as specified by the value of `duration` in preferences.json.
