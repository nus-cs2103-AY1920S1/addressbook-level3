...
image::Typee_highlevel_architecture_diagram.png[]
...
Step 3. The user executes `add n/David ...` to add a new person. The `add` command also calls `Model#saveEngagementList()`, causing another modified engagement list state to be saved into the `historyList`.
...
Step 5. The user then decides to execute the command `sort start_ascending`. Commands that do not modify the engagement list, such as `sort`, will usually not call `Model#saveEngagementList()`, `Model#undoEngagementList()` or `Model#redoEngagementList()`. Thus, the `historyList` remains unchanged.
...
1. Window component, which is a type of `VBox`, that acts as a container for other different UI windows.
...
1. New Ui Model class `Tab` is implemented to contain the respective fxml controller classes in a OOP manner. Below is the class diagram for `Tab` class
...
The game window provides a simple typing game where there are moving words that users can type in order to score points.
Once users correctly type the specified word, the word disappears and the increase in points is reflected in the player
information panel in the game window. When the user fails to type the word before the moving word reaches the bottom of
screen, the decrease in health points is reflected in the player information panel in the game window. After the health
points reaches zero, `GAME OVER` is displayed.
...
*Value proposition*: manage appointments and contacts faster than a typical mouse/ GUI driven app
...
|`*` |bored secretary |have a built-in game feature |keep myself entertained

|`*` |highly driven secretary |have a way to practice typing |improve my efficiency during work
...
2. System adds the appointment to the existing appointment list.
...
2.  System displays the list of appointments in chronological order
+
Use case ends.

*Extensions*
[none]
* 1a. User specifies ordering method
+
[none]
** 1a1. System displays the list of appointments in specified order
+
Use case ends.
...
5.  Typing game shows the final score of the User
+
Use case ends.
...
1. System displays the list of appointments
1. User requests for generating PDF of an engagement for a selected engagement.
1. System generates a PDF file in a specific external directory
1. System shows successful message
+
Use case ends.

*Extensions*
[none]
* 3a. System fails to generate PDF
[none]
** 3a1. System shows error message.
+
Use case resumes at step 2.
