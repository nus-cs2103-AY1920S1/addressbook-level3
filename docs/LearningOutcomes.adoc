...
* `Override` the `getPrintableString` in classes `Name`, `Credits`, `Email`, and `Address` so that each produces a printable string representation of the object. e.g. `Name: John Smith`, `Credits: 12349862`
...
return getPrintableString(p.getCredits(), p.getEmail(), p.getAddress());
----
