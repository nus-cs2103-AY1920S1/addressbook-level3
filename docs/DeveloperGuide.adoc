...
// tag::accessControl[]
=== [Proposed] Access Control feature
...
* (2) Identity Verification: Verification of identity via unique credentials and a confidential password
* (3) Account Management Restrictions: Access level restrictions for commands affecting other accounts

===== Command Restrictions

Prior to login, the user is only granted access to the `Login` and `Add` commands. This is achieved via a guard statement in the `LogicManager` checking whether the command created is an instance of `Login` or `Add` command:
...
=== [Proposed] Account Management feature
==== Proposed Implementation

The account management feature ...

==== Design Considerations
===== Aspect: How login & logout executes
...
=== [Proposed] Incident Draft Creation feature
==== Proposed Implementation

The incident draft creation mechanism is facilitated by ...
...
// tag::incidentfill[]
=== [Proposed] Incident Fill feature
==== Proposed Implementation

The incident fill mechanism is facilitated by ...
...
// tag::incidentsearch[]
=== [Proposed] Incident Search feature
==== Proposed Implementation
...
===== Aspect: How incident search executes
...
// tag::incidentedit[]
=== [Proposed] Incident Edit feature
==== Proposed Implementation

The incident edit mechanism is facilitated by ...
...
The undo/redo mechanism is facilitated by `VersionedIncidentManager`.
It extends `IncidentManager` with an undo/redo history, stored internally as an `incidentManagerStateList` and `currentStatePointer`.
Additionally, it implements the following operations:

* `VersionedIncidentManager#commit()` -- Saves the current incident Manager state in its history.
* `VersionedIncidentManager#undo()` -- Restores the previous incident Manager state from its history.
* `VersionedIncidentManager#redo()` -- Restores a previously undone incident Manager state from its history.

These operations are exposed in the `Model` interface as `Model#commitIncidentManager()`, `Model#undoIncidentManager()` and `Model#redoIncidentManager()` respectively.

Given below is an example usage scenario and how the undo/redo mechanism behaves at each step.

Step 1. The user launches the application for the first time. The `VersionedIncidentManager` will be initialized with the initial incident Manager state, and the `currentStatePointer` pointing to that single incident Manager state.

image::UndoRedoState0.png[]

Step 2. The user executes `delete 5` command to delete the 5th person in the incident Manager. The `delete` command calls `Model#commitIncidentManager()`, causing the modified state of the incident Manager after the `delete 5` command executes to be saved in the `incidentManagerStateList`, and the `currentStatePointer` is shifted to the newly inserted incident Manager state.

image::UndoRedoState1.png[]

Step 3. The user executes `add n/David ...` to add a new person. The `add` command also calls `Model#commitIncidentManager()`, causing another modified incident Manager state to be saved into the `incidentManagerStateList`.

image::UndoRedoState2.png[]

[NOTE]
If a command fails its execution, it will not call `Model#commitIncidentManager()`, so the incident Manager state will not be saved into the `incidentManagerStateList`.

Step 4. The user now decides that adding the person was a mistake, and decides to undo that action by executing the `undo` command. The `undo` command will call `Model#undoIncidentManager()`, which will shift the `currentStatePointer` once to the left, pointing it to the previous incident Manager state, and restores the incident Manager to that state.

image::UndoRedoState3.png[]

[NOTE]
If the `currentStatePointer` is at index 0, pointing to the initial incident Manager state, then there are no previous incident Manager states to restore. The `undo` command uses `Model#canUndoIncidentManager()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the undo.
...
The `redo` command does the opposite -- it calls `Model#redoIncidentManager()`, which shifts the `currentStatePointer` once to the right, pointing to the previously undone state, and restores the incident Manager to that state.

[NOTE]
If the `currentStatePointer` is at index `incidentManagerStateList.size() - 1`, pointing to the latest incident Manager state, then there are no undone incident Manager states to restore. The `redo` command uses `Model#canRedoIncidentManager()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the redo.

Step 5. The user then decides to execute the command `list`. Commands that do not modify the incident Manager, such as `list`, will usually not call `Model#commitIncidentManager()`, `Model#undoIncidentManager()` or `Model#redoIncidentManager()`. Thus, the `incidentManagerStateList` remains unchanged.

image::UndoRedoState4.png[]

Step 6. The user executes `clear`, which calls `Model#commitIncidentManager()`. Since the `currentStatePointer` is not pointing at the end of the `incidentManagerStateList`, all incident Manager states after the `currentStatePointer` will be purged. We designed it this way because it no longer makes sense to redo the `add n/David ...` command. This is the behavior that most modern desktop applications follow.
...
* **Alternative 1 (current choice):** Saves the entire incident Manager.
** Pros: Easy to implement.
...
* **Alternative 1 (current choice):** Use a list to store the history of incident Manager states.
** Pros: Easy for new Computer Science student undergraduates to understand, who are likely to be the new incoming developers of our project.
** Cons: Logic is duplicated twice. For example, when a new command is executed, we must remember to update both `HistoryManager` and `VersionedIncidentManager`.
...
[none]
* 2a. The username is not found.
+
[none]
** 2a1. IMS shows an error message that user does not exist.
Use case ends.

[none]
* 2b. The password is incorrect.
+
[none]
** 2b1. IMS shows an error message that password is incorrect.
Use case ends.
...
.  Data storage should be allow for easy access (directly manipulable).
