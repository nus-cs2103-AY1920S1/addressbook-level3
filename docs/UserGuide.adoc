...
[TIP]
If no category or difficulty is stated (i.e. `start`), program will start the quiz based on all the questions in the
question bank. `cat/`, `diff/` and `timer` optional parameters are only allowed in Custom Mode.
