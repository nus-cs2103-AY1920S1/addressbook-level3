= StudyBuddyPro Level 3 - Developer Guide
...
The _figure 1_ below displays the high-level design of our StudyBuddyPro.
...
Diagram 7 : Code Snippet of `StudyBuddyItemContainsTagPredicate#test()`
...
c. Upon initialization of the StudyBuddyPro and switching to the Flashcard window, the StudyBuddyParser’s function enum will be set to parse ‘Flashcard’ commands.
...
`Notes` are used in the creation of `CheatSheet` objects as well as in general use of StudyBuddy Pro. This Add Tags
...
** Viewable through `filter` commands
...
`Note` depends on a static method in `NoteFeatureUtil#parseNoteFragmentsFromNote()` to create `NoteFragments`.
`NoteFragments` are accessible through the `filter` methods in `ModelManager#collectTaggedItems()` and
`ModelManager#collectTaggedNotes()`.

The method of parsing `Content` relies on the use of `Prefixes` around each note fragment tag. Each note fragment
tag is specified with a start ("/\*") and end ("*/") marker, and its content and tags are also represented with
`Prefixes` 'C/' and 'TAG/'.
...
When a `add` command is made, only the `Prefixes` 't/', 'c/', and 'tag/' are identified. A `Note` is then created with
the desired `Title`, `Content`, and `Tags`. After creation, the `Content` of the `Note` is parsed immediately for the
`Prefixes` '/\*', '*/', 'C/', and 'TAG/'. All four `Prefixes` must be present for the note fragment tag to be
considered valid. Otherwise, an exception is thrown.
