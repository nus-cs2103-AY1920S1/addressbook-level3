...
This section describes the sequential flow of events in the life cycle of a `GameTimer` object with *no hints*.
...
The `AppManager` provides information regarding the duration in which the `Timer` should run for.

==== Flow of Events - `Hints` Enabled

.Activity diagram of the `Timer` component when `Hints` are enabled
