...
ifndef::env-github[]
a| icon:lightbulb-o[role="icon-tip", size="2x"] | Tips that might be helpful
a| icon:info-circle[role="icon-note", size="2x"] | Additional information that is good to know
a| icon:exclamation-circle[role="icon-important", size="2x"] | Important information that should be noted
endif::[]
|===

These icons and their following descriptions enable greater clarity in understanding the document.

In addition, if there are any additional information related, it will be described in a block below:
====
...
This enhancement functions well with existing commands. The design approach was challenging for the sort function as it particularly required some restructuring of how the list would be implemented. It required an in-depth analysis of design alternatives.
...
==== Credits
*natty* library was used to aid in natural language parsing. However, lots of modifications were applied onto it in order to allow support for _DD/MM/YYYY_ dates, which is a missed feature on the author's part.
...
|===
| Listed below are sections I contributed to the User Guide. This includes the Features, Flexible Date Formats, particularly in the Find and Sort, and Command Summary section.
|===
...
|===
| Listed below are sections I contributed to the Developer Guide. This includes the User Profile, Value Proposition and major features like Find and Sort.
|===
