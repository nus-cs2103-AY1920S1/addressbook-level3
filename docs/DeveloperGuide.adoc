...
image:BetterModelClassDiagram.png[align=center]
...
image::SortingClassDiagram.png[width=500, align=center]
...
image::MakeSortSequenceDiagram.png[align=center]
...
image::SaveActivityDiagram.png[width=350, align="center"]
...
image::BudgetSequenceDiagram.png[align=center]
...
.Design considerations of the for `Purchase` model
[cols="50,50"]
|===
|Alternative 1 (Chosen Implementation) |Alternative 2

a|Storing the purchased food item as a `Food` object in the `Purchase` object
...
a|Creating an additional immutable `PurchasedFood` class to store information regarding the purchased `Food` object.
...
We chose alternative 1 due to the fact that `Food` is implemented as a immutable class.
Therefore, we are able to get the required from the `Food` object the user selected to construct the new purchased `Food` object.
And at the same time, ensure the data integrity of that information.
Also, by encapsulating the `Food` object within the `Purchase` object and making it a private field, we are able to ensure that the purchased `Food` is not modified in an unintended manner.
...
image::BuySequenceDiagram.png[align=center]
...
Command word to change the theme is `theme`. Currently there are only two themes available which are light and dark
theme. As such, the only valid commands are `theme dark` or `theme light`.

The sequence window below shows how a sample `theme` command is executed:

.Sequence diagram for sample `theme` command
image::ThemeSequenceDiagram.png[align=center]

The following activity diagram shows how the `theme` command works.

.Activity diagram for sample `theme` command
image::ThemeActivityDiagram.png[align=center]
...
.Design considerations of the theme change feature
[cols="50,50"]
|===
|Alternative 1 (Chosen Implementation) |Alternative 2

a|Using CSS styling and just simply changing the `StyleSheet` for every `theme` call
...
a|Creating a class that automates the theme change according to color scheme.
...
We decided to go with alternative 1 and utilize `StyleSheet` instead of creating a class that automates the theme change.
...
This implementation also allows for better organization of the themes which makes it easy to keep track of the
different themes available. The space taken up by repeated components is not that heavy since the app itself is light.
//end::theme[]
