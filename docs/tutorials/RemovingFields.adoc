...
In scenarios like this, you can consider refactoring the existing `Eatery` model to suit your use case.

In this tutorial, we'll do exactly just that and remove the `address` field from `Eatery`.
...
The `address` field in `Eatery` is actually an instance of the `seedu.address.model.person.Address` class.
...
Other usages like in `EditEateryDescriptor` may require more careful inspection.

Let's try removing references to `Address` in `EditEateryDescriptor`.

. Safe delete the field `address` in `EditEateryDescriptor`
...
For example, removing the `address` field from the `Eatery` class will require you to modify its constructor.
...
For example, there is a `$address` in each `EateryCard` that has not been removed nor identified.

image::$address.png[width=1090px, height=890px]

A quick look at the `EateryCard` class and its `fxml` file quickly reveals why it slipped past the automated refactoring.
...
.invalidEateryAddressBook.json
[source, json]
```
{
  "eateries": [ {
    "name": "Eatery with invalid name field: Ha!ns Mu@ster",
    "phone": "9482424",
