...
image::AddBookActivityDiagram.png[width=400]
...
We can see that the book holds a `Optional<Loan>` in order to have either *0 or 1 `Loan` objects*.
This makes it consistent with the class diagram of `Book` above.
We can also observe that the `LoanHistory` object of the `Book` is empty since the book is new and is not loaned out before.
...
outweigh the additional memory costs incurred to maintain the HashMap.
...
*We have decided to go with Alternative 2* and keep it simple. This is because there are some edge cases which will lead to the implementation in
Alternative 1 not behaving as expected. Furthermore, Alternative 2 is also in line with the *KISS* (Keep it Simple, Stupid) principle of programming.
//end::addBook[]
...
. `Storage` component creates and save a new PDF `LoanSlipDocument` in a saved folder
. `Logic` component opens the newly generated `LoanSlipDocument` for the librarian to print it immediately
. `Logic` component clears the session in `LoanSlipUtil` at the end of the process
****
