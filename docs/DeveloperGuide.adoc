...
altenatives: placement of commandHistory attribute
...
*Problem with previous AB3 implementation:* Non-persistent list display on edits - any changes to an item within the displayed list requires
all items to be listed out again to properly "update". This forms a jarring UX experience, as every small change
(eg. increase student workload) would suddenly trigger the equivalent of a `list` command.

* *Explanation:* Using the AddressBook-Level3 implementation for MAMS, the GUI (MainWindow) would initialize each of the 3 List Panels
by assigning each of them an `ObservableList` reference obtained from `Model`. This reference will never be re-assigned again during
operation time. Any changes to the `ObservableList` raises an event to the List Panel, which prompts the UI to reconstuct the Cards
currently on display. However, if the internal parameters within an object in the list is changed (without changing the actual
object reference itself), no GUI updates will be made as the event will not be raised. In AddressBook-Level3, the attributes for each item in the underlying list was made final, and no setter methods were used.
Editing an object hence requires it to first be removed from the underlying list in `Mams`, then creating a new instance with
the edited field, and finally adding the new object with the edited field back into the list. The `ObservableList` passed to the GUI will then however lose the reference
to that object, and AB3 mitigates this by forcing the `ObservableList` to list all items again, regardless of the previous state of the list.

*Design Considerations*

* *Alternative 1:* Instead of implementing `Appeal`, `Student`, and `Module` with `final` attributes, let their internal details
be mutable. Whenever any of them is edited, object reference remains the same. `Model` will then raise an event for any changed item,
and the GUI upon catching the event will update the corresponding card appropriately.
** Pros: Has the potential to be more efficient. Only the edited item's card has to be updated.
** Cons: Difficult to implement without massive changes to the backend logic code. Depending on the implementation, carelessly making
the attributes of an item eg. `Student` mutable with setter methods may expose it to unwanted modifications.

* *Alternative 2 (Current Choice, on-going implementation):* Keep track of the previous list state using a `HashSet` of `String` IDs uniquely associated
with each MAMS item (`MatricId`, `ModuleCode`, and `AppealId`). Every time a predicate is applied to the underlying `FilteredList`
in the `ObservableList` passed to the GUI, the set of String IDs are copied over accordingly. To maintain the state of the
previous list even after object references have been changed (eg. when items are edited), `Logic#refreshFilteredList()` will be called.
This will cause the `FilteredList` to reconstruct its previous state using the IDs contained in the `HashSet`. In short, `FilteredList`
is made to list out all items, then filter them using the IDs stored in the `HashSet` to restore the previous state.
** Pros: Easy to implement. Changes to the code are kept within `mams.Logic`
** Cons: May arguably be more computationally inefficient due to the extra filtering step involved for reconstructing the previously displayed list.
