...
The sequence diagrams for interactions between the Logic and Model components when a user executes the `glist use` command is shown below. Their relations to the `Amount` class is shown in the subsequent diagram.
