...
image::StatisticsCalculationAcitvityDiagram.png[width="400", align="center"]
...
Note that in date input mode, the methods that return the `String` and
`XYChart.Series<String, Number>` is called as both are displayed in the popup window

Step 5: with this output from logic, `Statistic Window` or `DefaultStatisticWindow` will
then handle the displaying of the statistic, be it in chart
form or string form. With this the feature has finished executing! Attached below is the summary for step 3-5:

image::StatisticsCalculationUI.png[width="700", align="center"]
