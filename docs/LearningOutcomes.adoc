...
* Notice how having a separate `ParserUtil` class to handle user input validation, space trimming etc. of addressModel data (an application of the Single Responsibility Principle) improves the _cohesion_ of the addressModel component (since it does not need to be concerned with handling user input) as well as the `ParserUtil` class.
