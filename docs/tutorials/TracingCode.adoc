...
public CommandResult execute(Model model) throws CommandException {
    ...
    Person editedAccommodation = createEditedPerson(contactToEdit, editPersonDescriptor);
    if (!contactToEdit.isSamePerson(editedAccommodation) && model.hasPerson(editedAccommodation)) {
        throw new CommandException(MESSAGE_DUPLICATE_PERSON);
    }
    model.setPerson(contactToEdit, editedAccommodation);
    model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);
    return new CommandResult(String.format(MESSAGE_EDIT_PERSON_SUCCESS, editedAccommodation));
}
----
