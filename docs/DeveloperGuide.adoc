...
If the start date and end date is both specified by the user, the end date will be checked to not be before the start date.
...
After which, `StatsCommand` will call the `PieChartStatistics` implementation of `populateData()` to fill up its other fields
...
.. Test case: `statsbasic` +
Expected: A success message of "Pie Chart calculated!" appears with the title of the statistics
"Statistics Summary from DATE to DATE", where DATE is the current date, with $0.00 amount
but no pie chart being shown
.. Test case: Add an expense to the empty budget `addexpense d/ chicken rice p/2.50 c/food` and type `statsbasic` +
Expected: A success message of "Pie Chart calculated!" appears with the same title, amount to $2.50 and a pie chart being shown with only food
   category showing 100.00%.
.. Test case: In any budget, type `statsbasic sd/12-12 ed/11-11` +
Expected : An error message of "End date cannot be before start date."

=== Testing statscompare
.. Prerequisites: Create a new budget, one possible way is `addbudget d/school p/3.00 sd/28-10 pr/day`.
.. Test case: `statscompare sd1/12-12 sd2/ 1 month ago` +
Expected: A success message of "Statistics Comparison Calculated!" appears with the
the title "Statistics Summary: Comparing 12-12-2019 to 12-12-2019 with 12-10-2019 to 12-10-2019"
but all entries reflect 0 for frequency and 0.00 for amount.
...
Expected: A success message of "Statistics Comparison Calculated!" appears with
the title "Statistics Summary: Comparing 28-10-2019 to 28-10-2019 with 29-10-2019 to 29-10-2019" with the first entry of `Difference Amount`
having a value of -2.50 and having the first entry of `Difference Frequency` having a value of -1.

=== Testing statstrend
.. Prerequisites: Create a new budget, one possible way is `addbudget d/school p/3.00 sd/28-10 pr/day`.
.. Test case: `statstrend mode/category sd/14-01 ed/31-01` +
Expected: A success message of "Statistics Trend Calculated!" appears with
the title "Periodic trendline from 14-01-2019 to 31-01-2019 in the unit of days"
unit of time equal to the period of the budget, but no trend lines are seen
...
Expected: A success message of "Statistics Trend Calculated!" appears with the
title "Periodic trendline from 28-10-2019 to 29-11-2019 in the unit of days",
...
Expected: A success message of "Statistics Trend Calculated!" appears with the same title,
