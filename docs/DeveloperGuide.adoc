...
<<addSeqDiagram>> is a sequence of steps that illustrates the interaction between various classes when the `check 1`
command is entered.
[[addSeqDiagram]]
...
* The code snippet below shows the `CheckCommand::execute` method +
...
public CommandResult execute(Model model) throws CommandException {
    ...
    if (UiManager.getState().equals("claims")) {
        ...
        return new CommandException(MESSAGE_FAILURE);
    }
}
