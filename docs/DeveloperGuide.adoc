...
individual, or the entire team. The following operations provides this feature:
...
Reason of choice: `[green]#Training#` and `[green]#TrainingManager#` are created as standalone classes to
contain training information. We intend to introduce new features (e.g. training category) in the future and this
...
Reason of choice: A hash table is used as the `select` and `attendance` commands require the attendance rate of only
one person. A hash table provides us with the fastest access time to access attendance information of one person.
...
Reason of choice: Editing training information would require long commands which defeats the purpose of having a command
line interface. Editing training by replacing an old one with the `training` command makes it quicker.
...
. `calendar MMYYYY`: Displays calendar for the month specified by the user (e.g. `calendar 012019` where `012019` corresponds to January 2019).

The following sequence diagram shows you how the `calendar MMYYYY` (e.g. `calendar 012019`) operation works.
...
The `view calendar` command is similar except that `[green]#displayCalendar(CommandResult)#` is called instead of `[green]#displayCalendarWithDate(CommandResult)#` and the `[green]#CalendarPanel#` class is constructed _without_ an `[green]#AthletickDate#` specified by the user. The calendar displayed to the user will reflect the current month and year.
...
image::Function1ActivityDiagram.png[width=300]
...
* `[green]#CalendarPanel#initialiseSelectedDate()#` -- Fills up all the 42 cells of the `GridPane` with their respective days based on the selected month by the user. Days of the previous and next month used to fill up the remaining cells are marked in a lighter colour.
...
These operations are performed when an instance of `[green]#CalendarPanel#` is created in the `[green]#MainWindow#` class. An instance of `[green]#CalendarPanel#` is created when the `[green]#CommandResult#` obtained in `[green]#MainWindow#` after the execution of the user's command contains a `[green]#Feature#` corresponding to a calendar and an _optional_ `[green]#AthletickDate#`.

The sequence diagram below shows you what happens when the CalendarPanel is instantiated.

.Interactions Inside the UI and Model Components when CalendarPanel is Instantiated
image::CalendarConstructorSequenceDiagram.png[width="700"]

Upon completion of the above execution, all 42 cells `[green]#GridPane#` will be filled with a number corresponding to the day of the month. Days with training or performance records will be marked with a small green or purple dot indicator respectively. The following diagram depicts how each individual cell will look like.

image::CellResult.png[width="400"]
...
.Interactions Inside the UI and Logic Components for the `calendar 01012019` Command
image::CalendarCommandSequenceDiagram2.png[]

Upon completion of the above execution, the attendance and performance records of 1st January 2019 will be displayed to the user.
...
* `[green]#CalendarDetailPanel#initialiseAttendanceData#` -- Retrieves and displays the attendance records on the specified date.
...
The sequence diagram below shows you what happens when the CalendarDetailPanel is instantiated.
