...
The second diagram (Figure 10) here describes the process of undoing the `EditCommand` executed above using the `UndoCommand`. The
...
The following Activity Diagram (Figure 12) summarizes what happens when a user enter undoable commands,
...
* **Choice 3 (current choice):** Saves the entire model consisting of the exercise and regime books.
** Pros:
*** Easy to implement.
** Cons:
...
* **Choice 2:** Use a list to store the history of resource book states.
** Pros:
*** Very trivial
** Cons:
