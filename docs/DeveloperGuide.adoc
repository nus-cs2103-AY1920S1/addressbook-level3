...
suggestion is contained in the input, it will be further compared with other
...
For example, <insert picture>

The lower the index, the more similar the suggestion is.
...
of suggestions. The query process will be elaborated on below. This sequence diagram
is a high level example of the logic.
...
In order to achieve this, the first data structure used is a directed `Graph`
with every `Node` storing a reference to a `List` in a `DataBook`.
