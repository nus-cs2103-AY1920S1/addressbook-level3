...
.  `ParserManager` instance has reference to two `SpecificModeParser` 
.  When user enters a keystroke, the `SpecificModeParser` which hold switch Command or `SpecificModeParser` which hold current mode commands are accessed based on internal state.
...
.   Boolean algebra is used to derive the four overall states in the following way:
*   SwitchCommands are those that change mode and work as a WordBank is selected and Game is not running
*   ModeCommands belong to a specific mode and only work when application is in the specific mode.
