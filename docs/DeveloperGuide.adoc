...
=== Implementation
The autocomplete feature provides a list of suggestions to the user as the user types the command. The `AutoCompleteParser` starts by parsing the user input into the command word, the preamble and a list of prefixes used in the program.

After the input is parsed, the `AutoCompleteSuggester` gathers insight based on the parsed information and generate suggestions. If the command word is still being typed by the user, the `AutoCompleteSuggester` will suggest command words that partially matches the input. If the command word is in the input and that command word requires a preamble, `AutoCompleteSuggester` will suggest the preamble if the preamble has not been typed. Finally, the `AutoCompleteSuggester` will check the possible prefix for the command. The required prefix will appear at the top of the suggestions while optionals will be at the bottom.

Given below is a sequence diagram showing the generation of suggestions in `AutoCompleteSuggester`:
...
Below shows the checks taken by `AutoScheduleCommand` before it proceeds to schedule:
...
*Step 4.* It will filter the list that has been filtered by address to get all the activities with the same tag or activity name as the first activity to schedule.
...
*Step 7.* If the chosen activity overlaps, it will traverse the filtered list, picking the next activity with the **next lowest count** in the itinerary and check if it overlaps.
The process will continue until it finds a suitable activity or it has finished traversing the whole filtered list. If the activity to be scheduled is the last activity, it will instead check to ensure that it does not overlap til the next day.

.Activity diagram showing how `AutoScheduleCommand` finds an activity to schedule:
image::AutoScheduleActivityDiagram3.png[]

Step 4 to Step 7 will repeat for the next activity to schedule for that day until it reaches the last activity to schedule for that day or it could not find a suitable activity to schedule.
If there other days to schedule, the steps taken will also be the same as Step 4 to Step 7.

==== Design Considerations

===== Aspect: Counting the number of times each activity of the same name or tag appears in the list
* **Current Choice:** Count the number of times all the similar activities appear in the itinerary every time we are planning to schedule an activity.
** Pros: It will ensure that the count number for the similar activities will always be right before they are chosen to be scheduled
** Cons: Slow down the program as we have to traverse through all the activities for each day.
* **Alternative:** Have a counter inside the activity itself to keep track
** Pros: Significantly improve the runtime speed and less complicated to implement
** Cons: Over time, the count might be inaccurate due to multiple scheduling and unscheduling which alter the count number and we might not have keep track of it at certain point of time.
...
===== Aspect: Account for travelling time
* **Current Choice:** Allows user to input the start time of activity
** Pros: Gives user the flexibility and they better able to gauge their own travelling time of each activity
** Cons: It restrict the number of possible activities that could be scheduled as we have to consider activity not overlapping with the start time of the next activty.
* **Alternative:** Fix a break of 30min between every activity to acount for travel time
** Pros: Activities that user prioritised are more likely to be scheduled as we do not have to consider overlap of the start time of next activity.
** Cons: Certain activity may require more travelling time. If we fix a longer travelling time, it might not be optimising the schedule plan as certain activity might have shorter travelling time.
