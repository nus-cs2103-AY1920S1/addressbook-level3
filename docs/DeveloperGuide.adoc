...
//tag::addtransaction[]
=== Adding transactions
We allow users to add `**Expense**`/`**Income**` transactions into `**THRIFT**` which enables record-keeping. This section show how we handle such
...
.. Test case: **tag i/1 t/EXPENSIVE** +
   Expected: Details of the tagged transaction and original transaction is shown in the Result Box. Listing the transaction reflects changes.
