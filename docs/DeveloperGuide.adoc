...
image::MakeSortActivityDiagram.png[]
...
image::SavingsAccountClassDiagram.png[width=500]
...
image::SaveSequenceDiagram.png[]

The following activity diagram below summarizes how the save command works

.Activity diagram for a sample save command
image::SaveActivityDiagram.png[]
...
****
`RemainingBudget` stores a user's remaining budget amount. It contains an `ObservableValue` which allows the UI to track of the amount of money left in the current budget.
The maximum budget amount has been set to 1 million dollars.

Also, in order to avoid possible loss of precision errors, calculations regarding the budget amount is done using Java's `BigDecimal` class instead of `Double`.
****

===== DaysToExpire

****
`DaysToExpire` stores a user's remaining budget duration. It contains an `ObservableValue` which allows the UI to track of the number of days before the current budget expires.
The maximum budget duration has been set to 1 year (365 days).

Due to development constraints, we have decided to only update the budget duration every time the application is started up instead of constantly monitoring the current time to update this property.
****

.Wallet-related class diagram in Model component.
image::WalletClassDiagram.png[width=400]
...
The following activity diagram below summarizes how `budget 100 10` commands works
...
****
`PurchaseHistoryList` contains an `ObservableList` of `Purchases` made by the user.
****
...
image::PurchaseHistoryClassDiagram.png[width=400]
...
The recommendations are tailored to each user based on their current likes, dislikes and purchase history.
...
With the recommendation feature, a new set of classes were added.
...
image::RecommendationClassDiagram.png[width=500]
...
.Activity diagram for `calculateRecommendation`
image::RecValueActivityDiagram.png[width=600]
...
Detailed below are the design considerations taken into account when engineering the `calculateRecommendation()` function.

.Design considerations of the calculateRecommendation() function
[cols="20,40,40"]
|===
...
We chose this design because of its simplicity of implementation. Adding a hidden `recommendationValue`
