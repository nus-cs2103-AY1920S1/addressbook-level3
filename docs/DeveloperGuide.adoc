...
|`* * *` |HR Admin |improving work productivity | automate my manpower allocation process

|`* * *` |HR Admin |remind the company staff to attend |view upcoming events

|`* * *` |HR director |decide whether to have more or less of events | view the number of events

|`* * *` |HR personnel |access the worker’s personal details whenever I need it |keep track of all the worker's details

|`* * *` |finance staff |financial accounting for the company is done properly |keep track of all the worker's details

|`* * *` |accountant |easily track expenses related to employee wages |see the breakdown for each individual worker

|`* * *` |accountant |access the worker’s personal details |keep track of all the worker's details

|`* * *` |Staff in a Company |ensure that my pay is correct |able to see payslip

|`* * *` |Admin staff |ensure the workers are not abusing the leave system |able to track the workers paid leave and medical leave

|`* * *` |Admin staff |decide whether to take the next event |view the number of available workers in the office

|`* *` |worker |keep upcoming events on calendar |view upcoming events

|`* *` |an admin |decide what needs to be done soon |find upcoming event

|`* *` |a supervisor |have better manpower allocation |write comments/remarks for workers 
...
3.  User manually allocates a specific employee ID to a specific event ID
4.  Employee added.
+
Use case ends.
...
* 2.The employee ID given is invalid.
+
[none]
** 2a.AddMin+ shows an error message.
+
...
2.  AddMin+ shows the matching result
