...
The undo/redo mechanism is inspired by the undo/redo implementation in AddressBook 3 and is facilitated by `VersionedModulePlanner`.
...
// tag::dataencryption[]
=== [Work in Progress] Version Tracking
==== Current Implementation
...
All information regarding our modules are initially stored in json format, within the file `data/modules_cs.json`.
...
The autocompletion of keywords is facilitated by `Autocomplete`, which inherits from `Textfield`.
When the `MainWindow` is created, a `CommandBox` is created and added to the GUI.
The `CommandBox` holds an `Autocomplete`.
`CommandBox` attaches a `keyEvent` listener to `Autocomplete` that listens for the `TAB` key press,
which starts the autocomplete process.

`Autocomplete` holds two sets of keywords, a set for commands and a set for arguments.
...
`Autocomplete` is constructed with a `ReadOnlyModulePlanner`.
...
`Autocomplete` holds the following public operations:

- `Autocomplete#handleAutocomplete` -- Handles the entire autocomplete process based on the current text.

- `Autocomplete#handleChangeOfActiveStudyPlan` -- Resets the argument keywords when there is a
change in the active study plan.

Given below is an example usage scenario of the `Autocomplete#handleAutocomplete()` operation
...
The `KeyEvent` listener calls the `Autocomplete#handleAutocomplete()` operation.

Step 2: The text in the text field is checked.
As it is the first word, the keywords set for commands is used.
`Autocomplete#completeInput(String input, SortedSet<String> keywords, String currentText)` is called to proceed with the
completion of input.
...
Step 4: If there is only one search result, the input text is changed to the search result
...
the input text is changed to the search result and the caret is positioned at the end of the line.
...
* **Alternative 1 (current choice)**: All arguments including tags and module codes are checked for
together.
