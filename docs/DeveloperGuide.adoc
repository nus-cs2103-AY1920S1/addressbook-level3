...
When creating an `EditRepeaterDescriptor`, `EditRepeaterCommandParser` fills in arguments unspecified by the user with default values, as shown in the activity diagram below:

.Activity Diagram for Optional Arguments in `EditRepeaterCommandParser`
...
.. Test case: `add a/420.42 n/Felix Kjellberg sd/12 sm/12 sy/2019 ed/23 em/2 ey/2020 de/Borrowed for apple earpods ` +
...
.. Prerequisites: List all loans. Multiple loans in the list.
.. TestCase: 'edit 2 a/220 n/NewName sd/12 ey/2012' +
    Expected: First loan on the list is changed to new assigned values. No other loans or entries are changed.
.. TestCase: 'edit 1 n/NewName sd/12 ey/2012 d/New Description for the Loan' +
...
.. TestCase: 'edit a/220 n/NewName sd/12 ey/2012' +
