...
. For each date, create a *VBox* (a container to to stack `Block` objects). Eventually, this *VBox* represents the graphic for one date.
.. Condition: If the first `PersonTimeslot` in the list starts after 8am (TimeBook's schedule start time), stack an empty `Block` in the *container* with the height the same
...
Step 3. After that, the user wants to organise a group meeting with some of his group members and still want to keep track of the entire group schedule's free time slots, he
