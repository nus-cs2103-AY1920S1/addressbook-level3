...
I used JavaFX's `scene.image.Image`, which reduced the additional code I had to write on top of the
`Title` and `Content` classes by avoiding an unnecessary wrapper.
