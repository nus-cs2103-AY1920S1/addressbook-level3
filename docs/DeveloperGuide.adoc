...
We define this location as Closest Common Location.
...
... `NUS_LT17` identified as the correct location and `LT17` is not.
...
... `S4` and `S6` is identifiable but `S5` is not.
...
.. Call NUSMods api with `Cache#loadVenues` to get an array of Venues(`String`) in NUS,
