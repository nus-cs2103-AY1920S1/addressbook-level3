...
A `CloneModel` is a cloned version of the `Model` class and contains UserPrefs and the the items in `Xpire` and `ReplenishList`.

A `State` represents the status of `Xpire` at that point in history and contains the corresponding `CloneModel`, `ListType` which is the current view of the application, a `XpireMethodOfSorting` which determines how the items in `Xpire` are sorted, as well as a `predicate` that filters items in the current view.
...
The Undo stack is a double-ended Queue (`ArrayDeque`) which simulates as a stack whilst the Redo stack is a `Stack`. Both classes are imported from java.util.
These stacks are initialised and cleared upon beginning/ending every session of the application.
...
Step 1. The user launches the application for the first time. The two internal stacks in `StackManager` will be initialised. Both stacks should be empty as there are no previous commands by the user. The current state is s0, the initial state of `Xpire`.
...
Step 3. The user executes `add|Apple|30/10/2019|3` to add a new item. Similar to Step 2, The `add` command will then save the previous state, s1 by pushing it into the Undo Stack. The current state will be the new state `s2` with the item Apple added.

[NOTE]
If a command fails its execution, it will not save the previous state, thus the state will not be pushed into the Undo Stack.
...
Within Stack Manager, the current state will be the popped state, s2, from the Redo Stack. The current state, s1, will then be pushed back into the Undo Stack. The current states and their locations should be the same as after the execution of the Add command in Step 3.

Step 5b. The user decides to further undo his actions, which now includes the first Delete command. The initial state, s0, will then be popped from the Undo Stack and set as the current state. The current state, s1, will then be pushed into the Redo Stack.
