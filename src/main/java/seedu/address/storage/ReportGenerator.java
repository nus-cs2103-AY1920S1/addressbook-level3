...
public class ReportGenerator {
    ...
    private static Font signFont = new Font(Font.FontFamily.TIMES_ROMAN, 12, Font.BOLDITALIC);
    private String sign = "";
    /**
     * Generates a PDF summary report for all bodies registered in Mortago.
     *
     */
    public boolean generateSummary(java.util.List<Body> bodyList, String newSignature) {
        sign = newSignature;
     ...
     *
     */
    public boolean generateAll(java.util.List<Body> bodyList, String newSignature) {
        sign = newSignature;
        if (bodyList == null || bodyList.isEmpty()) {
            return false;
        }
     ...
     * @param document which is the report.
     */
    private static void addFooter(Document document, String sign) throws DocumentException {
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss");
        LocalDateTime now = LocalDateTime.now();
        document.add(new Paragraph("\n"));
        String dash = "-".repeat(130);
        document.add(new Paragraph(dash));
        document.add(new Paragraph("Manager Signature: " + sign, signFont));
        //document.add(new Paragraph(sign, signFont));
        document.add(new Paragraph("\n"));
        document.add(new Paragraph("Date: " + dtf.format(now), signFont));
        //document.add(new Paragraph(dtf.format(now), signFont));
        document.add(new Paragraph("\n"));
     ...
     */
    public boolean generate(Body body, String newSignature) {
        sign = newSignature;
        if (body == null) {
            return false;
        }
