...
The topmost panel shows you that you the "redo" delimiter.
The second panel from the top shows you the "current" delimiter.
...
* Press up kbd:[&downarrow;] to navigate to the next command.
* This is not a command you type into Alfred, and is mainly for convenience as it allows you to quickly re-use previously executed commands.
****
...
* Alfred's UI will display a list of top `NUMBER` teams based on their current score.
* If you input `NUMBER` as a number more than the number of teams in the Hackathon, Alfred will simply display all the teams in the Hackathon.
