...
Type in the `training` command, date and the indexes of athletes who were present in the following format and press
`Enter` to execute it.
...
If successfully added, the result box will display the following result in the diagram below. A green dot will
...
Type in the `training` command, the absent flag `-a`, date and the indexes of athletes who were absent in the following
format and
...
(see <<Check attendance of an athlete : `attendance`>>) or check overall team attendance (see: <<Check overall attendance of your team : `view attendance`>>)
...
Let's say you would like to track your team's overall attendance rate but don't want to manually type the attendance
