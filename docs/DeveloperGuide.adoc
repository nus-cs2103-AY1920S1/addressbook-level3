...
The functionalities and commands associated with the book loaning feature are `loan`, `return`, `renew` and `pay`.
...
After `LoanCommand#execute(model)` is called, this series of checks shown in the above diagram are done to determine if
...
Similarly, the creation of new objects for replacement are also done to support immutability. The `return` command is supported by the
...
after reading in `borrowerrecords.json` and `catalog.json`, and also every time a `Loan` object is updated. Method used to retrieve a `Loan` object from its `LoanId` must
