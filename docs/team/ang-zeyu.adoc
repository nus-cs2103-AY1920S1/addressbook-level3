...
* *What*: I refactored all of the `logic` code component of the base _address book_ code for team use in _TravelPal_.
I also added and updated several `ui` components, which of particular note was `ui FormItems`, a convenient and reusable
code structure to build user input forms anywhere in _TravelPal_.
* *Justification*: Although the code base given to us was extensive, much of the common functionality we had in mind for
_TravelPal_, such as the ability to switch between pages of the application, was not immediately implementable from the given _address book_ code base.
+
Given the limited time frame of our project, it was also crucial that we utilised much of the existing code to
speed our development process, and develop code components that were easily reusable.

* *Highlights*: It was challenging to avoid making
intrusive changes to the code base whenever possible while achieving the desired functionality,
as it would introduce an additional burden for the team to understand the code.
...
Nonetheless, using my prior experience and knowledge in developing well organized code,
I was able to contribute and implement numerous suggestions
which resulted in a extensible code base, easing the team development process.

Overall, developing _TravelPal_ under the various constraints given to us proved to be both a challenging and fruitful expereince.
