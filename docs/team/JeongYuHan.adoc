= Jeong Yu Han - Project Portfolio
:site-section: AboutUs
:imagesDir: ../images
:stylesDir: ../stylesheets

{nbsp} +

== Introduction and Overview

The purpose of this document is to display the author's (Jeong Yu Han) contribution to the software engineering project and his technical skills.

My team and I were tasked with enhancing a command line interface application for our Software Engineering project.
We chose to morph it into a teaching assistant called the Njoy Teaching Assistant, targeted at tech-savvy teachers who wish to improve
the efficiency of their daily tasks such as creating quizzes, schedules and tracking student performances.

This is what our project looks like:

image::Ui.png[]
_Figure 1. The graphical user interface for Njoy._

My role was to design and write code for the notes and statistics features. The rest of the sections covers the
summary of my contributions to the codebase, user guide and developer guide.

The following are icons and symbols that I will be using for the Project Portfolio:

[NOTE]
This symbol indicates a note to the user.

`This indicates a component, class or object in code representation.`

*This indicates important text.*

== Summary of contributions

This section details a summary of my specific enhancements, code contributions and other tasks.

I implemented the notes commands which include: *Creating*, *Editing*, *Deleting*, *Listing*, *Sorting* notes.

=== Enhancements

==== Creating, Editing and Deleting Notes

* *What it does:* Users can add notes with a title, description and an optional priority.
The default priority is unmarked and different priorities are color coded. The features can be edited by specifying their index and the fields to edit.

* *Justification:* Teachers often have a dizzying array of tasks to handle. The notes feature allows teachers to deal with work and other worries and allows them to handle the most pressing issues. Editing and deleting
gives the flexibility of shifting priorities, typing errors and task completion.

* *Highlights:* This enhancement works with existing as well as future commands. The implementation for this
feature required contemplation in terms of UX. While additional features such as re-doing, un-doing was considered, I decided that the main principle of the notes was being
lightweight and easy to use. I decided to give the simplest set of commands while making prioritising on optional utility. Validation for the input command
was also challenging, in particular for the edit command.

==== Sorting Notes

* *What it does:* Sorts the notes in the list in descending order of priority.

* *Justification:* Users should be easily able to identify more pressing tasks.

==== Listing Notes

* *What it does:* Shows the list of notes in the command result box. This feature is an alternative solution to the issue of notes having
content length that exceed its allocated space in the User Interface.

=== Code contributed

Please click the following link to see my code contributions dashboard. https://nus-cs2103-ay1920s1.github.io/tp-dashboard/#search=jeongyh99&sort=groupTitle&sortWithin=title&since=2019-09-06&timeframe=commit&mergegroup=false&groupSelect=groupByRepos&breakdown=false[Code Report]

=== Other contributions

* Project management:
** Reviewed team's code coverage, resolved issues in the tracker.

* Enhancements to team documentation:
** Updated README and ContactUs with missing acknowledgement and developer contacts.
** Updated Glossary and Product Survey : https://github.com/AY1920S1-CS2103T-W13-2/main/pull/24[#24]

* Enhancements to team code:
** Wrote the skeleton for storage using the Student entity : https://github.com/AY1920S1-CS2103T-W13-2/main/pull/47[#47]
** Wrote the skeleton for the generic printable interface for print/export functions : https://github.com/AY1920S1-CS2103T-W13-2/main/pull/105[#105]
** Refactor poor practices to follow better scalability and readability : https://github.com/AY1920S1-CS2103T-W13-2/main/pull/91[#91]
** Wrote additional tests and generic model stubs for testing : https://github.com/AY1920S1-CS2103T-W13-2/main/pull/123[#123]

* Documentation:
** Made additions to the appendix for Developer Guide and improve readability: https://github.com/AY1920S1-CS2103T-W13-2/main/pull/25[#25]
** Implemented features for User/Developer Guide: https://github.com/AY1920S1-CS2103T-W13-2/main/pull/77[#77],
https://github.com/AY1920S1-CS2103T-W13-2/main/pull/97[#97], https://github.com/AY1920S1-CS2103T-W13-2/main/pull/181[#181], https://github.com/AY1920S1-CS2103T-W13-2/main/pull/190[#190]

* Community:
** Reviewed Pull Requests of Other Teams: https://github.com/nus-cs2103-AY1920S1/addressbook-level3/pull/190[#190], review under username 'lumwb' : https://github.com/nus-cs2103-AY1920S1/addressbook-level3/pull/23[#23]

* Tools:
** Change team's dependencies to be compatible with both Mac and Windows.

== Contributions to the User Guide

|===
|_Given below are sections I contributed to the User Guide. They showcase my ability to write comprehensive documentation targeting end-users.
The following are some portions of the NJoy Assistant's User Guide that I have selected to display as a representation of my abilities._
|===

*The following is an example of Creating Notes*:

include::../UserGuide.adoc[lines=985..1014]

*The following is an example of Statistics Generation*:

include::../UserGuide.adoc[lines=1086..1155]

== Contributions to the Developer Guide

|===
|_Given below are sections I contributed to the Developer Guide. They showcase my ability to write technical documentation. As with the User Guide, I will only showcase
the portions that demonstrate the depth of my technical capacities as a representation of my contributions._
|===

*The following is an example of Adding Notes*:

include::../DeveloperGuide.adoc[lines=1110..1134]

{nbsp} +

*The following is an example of Statistics Generation*:

include::../DeveloperGuide.adoc[lines=1220..1261]