...
==== Implementations
Refer to sequence diagram above.

==== Design Considerations

===== Aspect: Subtracting the values

* **Alternative 1 (current choice):** Create a new grocery item and replace it with the old one.
** Pros: Easy to implement.
** Cons: May have performance issues in terms of memory usage.
* **Alternative 2:** Modify the Amount in the grocery item.
** Pros: Will be more efficient and use less memory.
** Cons: We must ensure that the implementation is correct.
...
|`* * *` |user |see the expiry dates of my groceries |know when they expire.

|`* * *` |user |update my grocery list |keep track of the amount of groceries I have

|`* *` |user |find a specific grocery |know more information about that grocery
...
|`* * *` |user |get reminders on expiring food items |prevent food wastage
...
|`* * *` |user |sort food items |view grocery list with the specified sorting method

|`* * *` |user |set default sort settings |view grocery list with the specified sorting method every time I display the grocery list

|`* * *` |user |merge food items with same name and expiry date when viewing grocery list |view grocery list in a more organized manner
|=======================================================================
...
3.  User requests to edit a specific grocery item's name/amount/expiry date
