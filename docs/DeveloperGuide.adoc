...
image::DashboardObjectDiagram.png[width=800]
...
image::SettingsActivityDiagram.png[width=400]
...
.. In the command prompt, enter "fire ti/1 mi/INITIALS" +
Expected: Shows that the task with index 1 has been removed from member with member id 'INITIALS' (under his 'list of tasks assigned'). Entering "list-tasks" in command prompt displays the list of tasks in +Work, with the involved member removed from
the task at index 1.
...
Expected: +Work recognises this command as member-stats.
...
.. Test case: "testing member-stats" +
Expected: +Work is unable to recognise this as a valid command.
.. Test case: Use command "add-task tn/name of task ts/unbegun" to add a task to +Work. Then, use commands "doing-task" and "done-task" to the task at a few minutes interval, before calling "task-stats" again.+
