...
image::QuizSessionActivityDiagram.png[]
...
=== Quiz Commands
During a quiz session the `QuizCommandParser` instead of `RevisionToolParser` is used.

.Interactions Inside the Logic Component during a quiz session
image::QuizSequenceDiagram.png[]


Sequence of steps after a user keys in their input.

. `Answerable#isCorrect(answer)` is called from the `McqInputCommand` to check whether the user's answer is correct.
...
[NOTE]
`CommandResult` is built using a factory design to allow flexibility of values returned to the `LogicManager`.
