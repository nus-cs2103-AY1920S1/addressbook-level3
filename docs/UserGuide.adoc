...
.  Double-click the file to start the application. The application should appear in a few seconds.
The following screenshot is a sample of how the application looks like:
+
...
This section describes the list of features and commands
in this application. Each feature is triggered by a command with a specific format. +
...
=== Viewing help : `help`
This command allows the user to access the user guide to ORGANice.
...
Currently, ORGANice only supports kidney matching. Refer to <<FAQ>> for a better understanding of the
...
There are two types of match commands. The following list explains each type of match command and describes the result
of executing each type of command:
...
For example, a user wants to match a specific patient called "John Doe" and his NRIC is "S1234568R".

. Type `match ic/S1234568R` in the command box and press Enter.
...
==== Matching all patients to all donors
For example, a user wants to find how many donors matches each patient in ORGANice.

. Type `match ic/all` in the command box and press Enter.
