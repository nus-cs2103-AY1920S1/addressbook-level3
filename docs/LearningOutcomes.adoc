...
* `Override` the `getPrintableString` in classes `Name`, `Credits`, `PrevMods`, and `Address` so that each produces a printable string representation of the object. e.g. `Name: John Smith`, `Credits: 12349862`
...
return getPrintableString(p.getCredits(), p.getPrevMods(), p.getAddress());
----
