...
[NOTE]
We differentiate transactions into income and expenditure items using the `TransactionType` field, which accepts
...
*Step 8*: `AddRepeaterCommand` fetches the running `RepeaterUniqueId` counter tracked by the `Model`, and sets it as the created ``Repeater``'s unique id.

*Step 9*: `AddRepeaterCommand` adds the newly created `Repeater` to the `Model`.

*Step 10*: `AddRepeaterCommand` creates the associated ``Transaction``s, setting their repeater unique ids to that of the newly created `Repeater`.

*Step 11*: `AddRepeaterCommand` adds the newly created ``Transaction``s to the `Model`.

*Step 12*: `AddRepeaterCommand` increments the current `RepeaterUniqueId` counter in the `Model`.
...
*Step 11*: `EditRepeaterCommand` deletes all associated ``Transaction``s in the `Model` using the edited repeater's unique id.

*Step 10*: `EditRepeaterCommand` creates the ``Transaction``s associated with the updated `Repeater`, setting their repeater unique ids to that of the updated `Repeater`.

*Step 11*: `EditRepeaterCommand` adds the newly created ``Transaction``s to the `Model`.

*Step 12*: `EditRepeaterCommand` returns a `CommandResult` to the `LogicManager` which is returned back to the user.
// end::repeatereidt[]
...
*Step 10*: `DeleteRepeaterCommand` deletes all associated ``Transaction``s in the `Model` using the deleted repeater's unique id.
...
*Step 1*: In the repeaters tab, the user executes `find phone husband` to find all repeaters whose descriptions contain both the keywords "phone" and "husband".
