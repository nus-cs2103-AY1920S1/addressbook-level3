...
* `editperson ed/Betsy n/Betsy Crower tag/` +
...
[IMPORTANT]
====
include::UserGuide.adoc[tag=alice-brief]

To personalize TimeBook and make it yours, use this `edituser` command as described below.
====

Edits your details in the TimeBook. +
...
* `edituser n/Frank` +
Renames your profile to the new name `Frank`.
* `edituser p/81234567 em/frank@example.com` +
Edits your phone number and email address to be `81234567` and `frank@example.com` respectively.
...
image:./icons/lightbulb.png[] You can use the command `toggle` see next week's schedule! You may see this person's schedule up to 4 weeks in advance.
After you've reached the fourth week's schedule, the next `toggle` command will reset the schedule view back to the first week's schedule.
...
. `NAME`
.. *Description:* The name of the person you want to add the event to. If the parameter is empty, the event
will be added to your own schedule.
.. *Prefix:* `n/`
.. *Requirements:* Optional
.. *Data Type:* String
. `EVENT_NAME`
.. *Description:* The name of the event you want to add to your schedule
.. *Prefix:* `e/`
.. *Requirements:* Required
.. *Data Type:* String
. `DATETIME`
.. *Description:* The date, time and location of the event. You can specify multiple `DATETIME` if you want the
event to repeat in different `DATETIME`. Note that the location of the event cannot contain `e/`, `n/` or `t/`.
.. *Prefix:* `t/`
.. *Requirements:* Require at least one `DATETIME` per event. In the `DATETIME` parameter it is optional for you to
specify the `LOCATION`
.. *Data Type:* String
.. *Format: * `e/DATE - START_TIME - END_TIME - [LOCATION]
... `DATE` : ddMMyyyy
... `TIME` : HHmm(24H)
... `LOCATION`: String without `e/`, `n/`, `t/` (Optional)
*****
image:./icons/lightbulb.png[] By not specifying `NAME`, the event will be added to your schedule.
*****
...
* `addevent n/John e/Math Lecture t/26092019-1300-1400-LT25 t/03102019-1300-1400-LT25` +
Adds an event `Math Lecture` that occurs on multiple dates to ``John``'s schedule.
...
Deletes an event specified by `EVENT_NAME` from your schedule.
...
* A group can be optionally created with a description, a remark, and your role in the Group.
****
...
Creates a group called `CS2103` with a description of `Project Group` and your role in the Group as the `Leader`.
...
image:./icons/lightbulb.png[] You can use the command `toggle` to help you show next week's schedule!
...
Adds a person specified by `NAME` with a role specified by `ROLE` into a group specified by `GROUP_NAME` in the
TimeBook.
...
* `addtogroup n/Bobby g/CS3203 role/Leader` +
...
| No schedule to XXX | This error occurs when you use `scroll` and `toggle` command when there is no schedule shown.
