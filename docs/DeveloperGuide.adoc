...
2. The add command creates a new `Notif` using this `Body` as the parameter.
3. The add command instantiates a new `NotifCommand` with `Notif`, `period`, `unit` as parameters.
`period` is a static final long equals to 10 and `unit` is static final TimeUnit set at TimeUnit.SECONDS.
...
a| No! If the status of the `Body` is changed to `ARRIVED` using the `UpdateCommand`, `NotifCommand` will still
...
`notifCreationTime` is stored in a JSON file along with the other `Entities`. When the `MainApp` is initialized, all the
`Notif` stored will be added in the `model`. During this process, for each `Notif`, the difference of the current system
