...
public class CheckTaskRunnable implements Runnable {
    ...
    public CheckTaskRunnable(ItemModel model) {
        ...
        while (futureReminders.size() > 0
                && futureReminders.get(0).getReminder().get().getDefaultDateTime().isBefore(LocalDateTime.now())) {
    ...
    public void run() {
        ...
        if (futureReminders.size() > 0) {
            ...
            while (reminder != null && reminder.getDefaultDateTime().isBefore(LocalDateTime.now())) {
                logger.info("----------------[LOGIC MANAGER]["
