...
`ackappt` Command - Acknowledges an appointment once the patient reaches.

`appointments` Command - Lists all upcoming appointment schedules or lists a specific patient's upcoming appointments.
...
`settleappt` Command - Settles a missed appointment when user contacts with patient.
...
Each Appointment object consists of a PatientId, Timing, Status. Timing class has 2 DateTime objects which indicates the start and end time of the appointment. The UniqueAppointmentList contains 0 or more Appointments.
...
==== `newappt` Command
The `newappt` command behaves similarly to the `add` command used for Patient and Doctor. The command takes in the parameters required to construct ReferenceId, DateTime and Status. The image below shows how the Appointment object is constructed.
...
The `ackAppointment` command will mark the patient's the most upcoming appointment as Acknowledge when the patient is reaching and it will update `UniqueAppointmentList` to display the rest appointments that are belong to the patient.
...
The `missAppointment` command displays the missed appointments. The missed appointments are found in `ModelManager`. The list is instantiated by filtering the `UniqueAppointmentList` using `EventsMissedPredicate` which checks all `APPROVED`-Status appointments ending times with current time.

==== `settleappt` Command

The `settleappt` command removes a missed appointment. It updates `UniqueAppointmentList` to display the rest of the missing appointments.
