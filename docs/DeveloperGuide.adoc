...
* can analyse the current list of `Password` object.
* can output a summary security report for the list of `Password` objects.
* can output a detailed security report for a specified `Password` object.

==== Proposed Implementation
The analyse password functionality is facilitated by the `AnalysePasswordCommand`, which makes use of various analysers.
Each type of analyser implements `Analyser`, and implements the following operations:

* `Analyser#analyse(List<Password> passwords)` -- Analyses a certain aspect for every password in the list.
* `Analyser#outputSummaryReport()` -- Outputs an overview summary report for the list of `Results` created from analysing each password.
* `Analyser#ouputDetailedReport(Index index)` -- Outputs a detailed report from the result of a specified Password.

Given below is the example usage of how the the analyse functionality, using the `DictionaryAnalyser`, behaves at each step.

Step 1. The user inputs `analyse` into the command line. The `Logic` uses the `PasswordBookParser` to parse the user command,
which results in a `AnalysePasswordCommand` which is executed by the `LogicManager`.

Step 2. The `AnalysePasswordCommand` calls the `Model#getFilteredPasswordList()` to obtain the current list of passwords,
as well as `#getRequiredAnalysers()` to get the default list of analysers.

Step 3. `AnalysePasswordCommand` will invoke `Analyser#analyse()` of each analyser on every password in the list to produce the respective result.
The `Result` object contains information of the password after analysis, such as the description of result.
For comparison-based analysers, `Match` objects are created to store information about matches found by the analyser.
Each `Match` contains details such as the match token, as well as the start and end index of the token with respect to the password.

Step 4. A report is compiled by stringify-ing the list of `Result` objects from each analyser and shown to the user.

Step 5. Should the user wish to view a more detailed report for a specific password, user may input `analyse strong/<index>`
to view a more detailed analysis of the specified password.

The following sequence diagram shows how the analyse functionality works:
