...
This statistic calculation and chart generation feature
extends `Seller Manager Lite` which allows seller to quickly
generate total statistics from their `DataBooks`, through the use
of a `statistic` module that handles this calculation.
...
* mode with date input from user: The statistic command takes in 2 `dates`, `starting date` and `ending date` and generates the `profit`
, `cost` or `revenue`  each `month` between this 2 dates.
...
`String` and the other and `XYChart.Data<String,Number>`
...
* Methods that return a `XYChart.Data<String,Number>`:
...
and then display the result of that call.
...
`UI` to use in its query to logic.

Attached below is a quick summary of what occurs when a statistic command is entered by the user:
...
The `Agenda` is made a class variable in `CalendarPanel`, which extends `UiPart`, making it one of the tabbed panels we offer on our application.

`CalendarPanel` keeps `UniqueList<Schedule>` and `UniqueList<Order>` objects as class variables. On instantiation, all the schedules will be converted to `Appointments` and then added to `Agenda`.
...
`CalendarPanel` also keeps `CalendarDate` as a variable to set the date `Agenda` shows.
...
`CalendarDate` is then placed in `Model` so that it can be edited right away while the Model executes any schedule related commands.
