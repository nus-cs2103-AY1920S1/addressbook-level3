...
==== Implementation
The `radd` feature comes with the support of `Task` and `TaskList` model. The `TaskList` is essentially a list of
`Task`. `AppData` keeps track of a `TaskList`. When `radd` command is executed, a `Task` will be added to the `TaskList`
in `AppData` and reflected on the GUI.
