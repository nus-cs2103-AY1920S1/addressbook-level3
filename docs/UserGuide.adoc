...
Format: `edit INDEX [n/NAME] [d/DESCRIPTION] [a/AUTHOR]  [t/TAG]...`
...
[NOTE]
If sorted by ascending order (i.e. the default order), empty fields will have the highest priority, which means if you execute `sort m/weblink`, the first few results would be (if any) without web links.
...
[NOTE]
When providing the search restrictions, you should ensure that keywords are non-blank and difficulty range is valid (as specified in `add` command). Also at least one search restriction should be provided.
...
Format: `add n/NAME [d/DESCRIPTION] [s/START_DATE] [e/END_DATE]...`

* Plans cannot have duplicate names.
* `START_DATE` and `END_DATE` should be in the format of `yyyy-MM-dd HH:mm:ss`

Examples:

* `addplan n/CS2040 d/past year questions in practical exams s/2019-01-01 00:00:00 e/3019-12-12 12:12:12`
...
Format: `editplan INDEX [n/NAME] [d/DESCRIPTION] [s/START_DATE] [e/END_DATE]...`
...
* `edit 2 s/2019-08-13 10:00:00` +
Edits the starting date of the 2nd plan to be `2019-08-13 10:00:00`.
...
Format: `findplan [n/NAME] [d/DESCRIPTION] [s/RANGE_START] [e/RANGE_END]...`

* Name
** is case-insensitive.
** is considered a match as long as there is the same sequence of characters. (e.g. `CS` matches both `CS2040` and `ICSD`)
* Description
** is case-insensitive.
** is considered a match as long as one word matches. (e.g. `Henz Ben` matches both `Martin Henz` and `Ben Leong`)
** is matched word by word. e.g. `hello` will match `hello world` but not `helloworld`
* Start and end range
** is considered a match when there is time overlap. (e.g. a plan with start date of `2019-01-01 00:00:00` and end date of `2019-03-03 00:00:00` matches a plan with start date of `2019-02-02 00:00:00` and end date of `2019-04-04 00:00:00`)
>>>>>>> Update UserGuide
...
* *Add Problem* `add n/NAME [d/DESCRIPTION] [a/AUTHOR] [t/TAG]...` +
...
* *List Problems* : `list` +
* *Edit Problems* : `edit INDEX [d/DESCRIPTION] [a/AUTHOR] [t/TAG]...` +
e.g. `edit 2 n/permutations t/`
* *Delete Problem* : `delete INDEX` +
e.g. `delete 3`

* *Sort Problems* : `sort m/METHOD [ord/ORDER]` +
e.g. `sort m/difficulty ord/ascend`
* *Find Problems* : `find [n/NAME] [a/AUTHOR] [d/DESCRIPTION] [src/SOURCE] [diff/LOWER_BOUND-UPPER_BOUND] [t/TAG]…` +
e.g. `find t/algorithm diff/2.0-4.0`
