...
* A Reference Id refers to a string of atleast 3 alphanumeric characters (case-insensitive) that can be used to uniquely identify a person, such as a membership number or NRIC.
...
Format: `edit -entry <ENTRY_ID> [-id <PATIENT_REFERENCE_ID>] [-name <NAME>] [-phone <PHONE_NUM>]
 [-email <EMAIL>] [-address <ADDRESS>] [-tag <Tags>]...`
...
Format: `updatedoctor -entry <ENTRY_ID> [-id <STAFF_REFERENCE_ID>] [-name <NAME>] [-phone <PHONE_NUM>]
[-email <EMAIL>] [-address <ADDRESS>] [-tag <Tags>]...`
...
Format: `changeappt -entry <ENTRY_ID> -start <START_TIMING> [-end <END_TIMING>]`
...
Format: `changeshift -entry <ENTRY_ID> -start <START_TIMING> -end <END_TIMING>`
...
** Search for doctors using reference Id, name or phone number: `doctors [<SEARCH_KEYWORD>]`
...
** Mark missed appointment as settled: `settle <ENTRY_ID>`
