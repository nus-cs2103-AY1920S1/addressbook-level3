...
*Step 2*. The method `Model#addQuestion(Question question)` will then be called to add the question and the question added,
...
*Step 3*. A success message with the edited question, will be appended with the `QuestionEditCommand#MESSAGE_SUCCESS` constant
...
_Sequence diagram illustrating the interactions happening_
