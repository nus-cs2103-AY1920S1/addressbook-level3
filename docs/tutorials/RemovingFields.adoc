...
In scenarios like this, you can consider refactoring the existing `Contact` model to suit your use case.
...
The `address` field in `Contact` is actually an instance of the `seedu.address.model.field.Address` class.
...
Other usages like in `EditContactDescriptor` may require more careful inspection.

Let's try removing references to `Address` in `EditContactDescriptor`.

. Safe delete the field `address` in `EditContactDescriptor`
...
For example, removing the `address` field from the `Contact` class will require you to modify its constructor.
...
For example, there is a `$address` in each `ContactCard` that has not been removed nor identified.
