...
The timed/untimed test is a exam mechanism whereby users can enter a test mode that allows users to test themselves using the various flashcards. Users will then be notified of their results at the end of the test and will also have the chance to review their answers and reattempt questions.
...
=====
Step 1. User keys in the command `test deck/DECK_NAME duratoin/DURATION_IN_SECONDS` (Eg. test deck/german duration/0)
