...
| ``MONTH``
| Uppercase letters that are highlighted in grey indicate that the word is a _parameter_.footnote:[Remember what this means? A parameter is merely like a field in your regular form. Just fill it up with anything appropriate. 😊]

| ``m\MONTH``
| The slash before the _parameter_ is a _prefix_. It is used to separate the current _parameter_ from other _parameters_.
...
We have all been there. It's the end of the semester but we still have orientation camps to plan, internships to complete, family commitments, etc. However, we have worked so hard and *really* want to go for a trip with our friends and unwind. Sounds like you this semester?
...
Let us find out what the main parameters (or fields, just like those which you fill up in a regular form📝) for calendar.
