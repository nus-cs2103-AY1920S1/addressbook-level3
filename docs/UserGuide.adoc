...
** Example: In `add-task tn/TASK_NAME s/TASK_STATUS`, the user inputs the name of the task after nt/ and inputs the status of the task after s/ , such as: +
 `add task t/Finish Project Draft s/undone`
...
Example: `add-member mn/Gabriel Seow mi/GS mt/Programmer` will results in the following: 
+
...
add-member mn/Gabriel Seow mi/GS mt/Programmer
