...
The _Command Word_ is ``edit`` with an parameter index. +
This command edits an expense in your Financial Tracker with the only parameter being the index of the expense you want to edit. The index must be an integer which is greater than 1 and corresponds to ID of the expense you want to edit.
...
The _Command Word_ is ``sort`` with parameters "date", "time", "amount" or "default". +
...
Format: ``sort date`` | ``sort time`` | ``sort amount`` | ``sort default``
