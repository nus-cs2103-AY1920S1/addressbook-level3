...
has been not been contactable for a given period of time from the point of admission of the `Body`. He / She then needs
to
contact
the police
to
...
`DuplicateNotifException` are directly thrown in the form of `CommandException` or they are logged in `Logger`. A
