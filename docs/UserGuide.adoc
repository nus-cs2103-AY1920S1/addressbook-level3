...
Format: `addpolicy n/NAME d/DESCRIPTION c/TIME_PERIOD p/PRICE sa/START_AGE ea/END_AGE [t/TAGS]... [cri/CRITERIA]...`

****
* A policy can have any number of tags (including 0).
* A policy can have any number of criteria (including 0).
* Coverage time period is specified in years.
* Price is specified in dollar ($) units.
****
Examples:

* `addpolicy n/SeniorCare d/Care for seniors c/5 p/50000 sa/50 ea/75]`
...
Edits an existing person in the address book. +
Format: `edit INDEX [n/NAME] [d/DESCRIPTION] [c/TIME_PERIOD] [p/PRICE] [sa/START_AGE] [ea/END_AGE] [t/TAG]... [cri/CRITERIA]...`
...
* `listpeople` +
`assignpolicy 2 pol/3` +
Assigns the policy at index 3 to the 2nd person in the list of people.
* `find Betsy` +
`assignpolicy 1 pol/4` +
Assigns the policy at index 4 to the 1st person in the results of the `find` command.
