...
** For the filter command: `filter -FLAG [/attributeName attributeValue ...]`::
*** `filter -w /sex male` and `filter -b /sex female /cod car accident` are valid commands.
*** `filter -b` is an invalid command.
...
You can delete a body, worker or fridge entry, using its Identification Number by entering a delete command
...
* Mortago automatically deletes its associated `Notif`. You can learn more
about `Notif` in <<notifs, Section 3.2.1>>.
...
| `delete -w 3`
| Output box shows: Deleted Entity: Jim Kerr Worker ID: W00003 Sex: MALE Phone: No input given.
Date of Birth: No input given. Date Joined: Tue Nov 21 00:00:00 SGT 1111 Designation: No input given. Employment Status: No input given. +

Dashboard no longer shows Jim Kerr.
|===
...
image::Notif2.png[width=550]

3) Type `showNotif` and press `ENTER` to execute it.

image::Notif3.png[width=500]
...
image::statsOverview.png[width="790"]
...
.Line chart displaying the last 10 days from the current date
...
25/10/2019. The line chart appears as below:
...
+
image::Appendix1.png[align="center", width=60%]
...
+
image::Appendix2.png[align="center", width=70%]
...
+
image::Appendix3.png[align="center", width=60%]
+
