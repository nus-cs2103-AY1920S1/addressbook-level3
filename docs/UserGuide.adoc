...
Similar to other CLI programs such as Windows' Command Prompt and MacOS' Terminal, pressing the "TAB" key will autofill the current field with data from the application. Pressing "TAB" repeatedly cycles through the different possible values.

E.g.: with `addTutorial m/GE` in the input box, pressing "TAB" changes the value to `addTutorial m/GER1000`, `addTutorial m/GEQ1000`, etc., depending on which modules beginning with "GE" were previously added to the application.
