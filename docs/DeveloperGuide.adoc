...
image::SlideshowFeature.png[]

The following is a detailed explanation as to how the questions are fetched and displayed on the slideshow. +
...
This will add questions based on the `Index` specified to the `slideshowQuestions` list under the `SavedQuestions` class. The usage of
`Index` here is such that it follows the same convention of when the user edits or deletes a question.
[NOTE]
...
questions will have 2 different answer formats and may cause confusion to the users.
