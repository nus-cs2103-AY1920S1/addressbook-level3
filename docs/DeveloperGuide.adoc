...
The `glist use` command is also able to support unit conversion. Currently, the implementation of the unit conversion are calculated manually inside the `Amount` class. No external library is used. (Refer to <<amount-parameter, About the amount parameter>>)

The following activity diagram summarises how the unit conversion is done.

image::UseGroceryActivityDiagram.png[align="center", width="400"]
...
Any value below the restriction would result in an error.
====
...
* Cons:
** Mutable amount field may result in unforeseen changes.
|===
...
* Pros:
** Will be more intuitive to the user. e.g. After using `650ml` of a `1L` milk, it is more intuitive to show `350ml` instead of `0.35L`.
