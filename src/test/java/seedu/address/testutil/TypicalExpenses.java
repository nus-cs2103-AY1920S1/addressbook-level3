...
public class TypicalExpenses {
    ...
    public static final Expense SHOPPING = new ExpenseBuilder().withName("Adidas shoe").withCurrency("SGD").withAmount("$120.00")
            .withDate("24/12/2019").build();
    public static final Expense GROCERIES = new ExpenseBuilder().withName("Groceries").withCurrency("SGD").withAmount("$17.80")
            .withDate("1/2/2019").withTags("groceries").build();
    public static final Expense VALENTINES = new ExpenseBuilder().withName("Chocolates").withCurrency("SGD").withAmount("$12.30")
            .withDate("13/2/2019").build();
    public static final Expense CHRISTMAS = new ExpenseBuilder().withName("Socks").withCurrency("SGD").withAmount("$5.00")
            .withDate("24/12/2019").build();
    public static final Expense NEWYEAR = new ExpenseBuilder().withName("Cheese").withCurrency("SGD").withAmount("$10.00")
            .withDate("31/12/2019").build();

    // Manually added
    public static final Expense FURNITURE = new ExpenseBuilder().withName("Sofa").withCurrency("SGD").withAmount("$700.00")
            .withDate("6/9/2019").build();
    public static final Expense SNACKS = new ExpenseBuilder().withName("Chips").withCurrency("SGD").withAmount("$1.20")
            .withDate("2/3/2019").build();

    // Manually added - Expense's details found in {@code CommandTestUtil}
    public static final Expense VODKA = new ExpenseBuilder().withName(VALID_NAME_VODKA).withCurrency(VALID_CURRENCY_VODKA).withAmount(VALID_AMOUNT_VODKA)
            .withDate(VALID_DATE_VODKA).withTags(VALID_TAG_ALCOHOL).build();
    public static final Expense RUM = new ExpenseBuilder().withName(VALID_NAME_RUM).withCurrency(VALID_CURRENCY_VODKA).withAmount(VALID_AMOUNT_RUM)
