...
* `addbio dp//Users/whneo97/Desktop/doge.png desc/hello world n/testName nric/testNric gender/testGender dob/12/12/2019 p/12343567 p/91234567 e/81234567 m/testMedicalCondition a/example address 123 goal/testGoal o/testOtherInfo`
...
fields will be removed. If a biography does not exist, this command can still be executed, although the result will be
no different than before. In either case, after clearing the biography, the biography display pane with an empty
biography table is shown to the user. A user cannot execute `editbio` after clearing the biography until another
biography has been added using `addbio`.
