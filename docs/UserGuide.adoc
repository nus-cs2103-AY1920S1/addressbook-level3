...
==== Recommend
Users can get a recommendation from the application, based on several factors, summarized
in the tables below.

.Bonuses applied by the recommendation system
|===
|Condition 1 |Condition 2 |Bonus/Penalty

.3+|Food tag matches the user's liked tags
|1 or more tags
|+0.05 plus 0.03 per matching tag
|3 or more tags
|+0.10 plus 0.03 per matching tag
|5 or more tags
|+0.25 plus 0.03 per matching tag
...
|N/A
|+0.10
|Food tag matches the user's tags in purchase history
|N/A
...
.3+|Food tag matches the user's disliked tags
|1 or more tags
|-0.10 minus 0.10 per matching tag
|2 or more tags
|-0.30 minus 0.10 per matching tag
|3 or more tags
|-0.50 minus 0.10 per matching tag
...
|Applies a decreasing penalty starting from -10 (just purchased) which diminishes to 0 after 2 days
|===
...
recommendation system will then be able to generate more accurate recommendations with your preferences.
