...
In this tutorial, we'll do exactly just that and remove the `employeeAddress` field from `Person`.
...
The `employeeAddress` field in `Person` is actually an instance of the `seedu.employeeAddress.model.employee.Address` class.
...
. Safe delete the field `employeeAddress` in `EditPersonDescriptor`
...
. Remove the usages of `employeeAddress` and select `Do refactor` when you are done.
+
TIP: Removing usages may result in errors.
Exercise discretion and fix them.
For example, removing the `employeeAddress` field from the `Person` class will require you to modify its constructor.
...
You can find them by searching for instances of the word `employeeAddress` in your code (`Edit` > `Find` > `Find in path`).
...
For example, there is a `$employeeAddress` in each `PersonCard` that has not been removed nor identified.

image::$employeeAddress.png[width=1090px, height=890px]
...
----
...
<Label fx:id="employeePhone" styleClass="cell_small_label" text="\$employeePhone" />
<Label fx:id="employeeAddress" styleClass="cell_small_label" text="\$employeeAddress" />
<Label fx:id="employeeEmail" styleClass="cell_small_label" text="\$employeeEmail" />
...
----
...
While keeping the `employeeAddress` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
{
  "employees": [ {
    "employeeName": "Person with invalid employeeName field: Ha!ns Mu@ster",
    "employeePhone": "9482424",
    "employeeEmail": "hans@example.com",
    "employeeAddress": "4th street"
  } ]
}
```
You can go through each individual `json` file and manually remove the `employeeAddress` field.
