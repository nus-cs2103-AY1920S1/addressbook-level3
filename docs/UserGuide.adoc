...
+
.  Click on the specific tab you want. For now, we have `Main` and `Schedule`.
...
For new users, do refer to the subsections under Section 3 for the specific command and how to use them. For more advanced
...
[NOTE]
Events are initialized without any schedule, other than their stated start and end date with a default time of 8-6pm. Hence, the mapping _declares_ the schedule for the dates that the event is being held.
