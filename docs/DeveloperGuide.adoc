...
* `AccountsManager#getActiveTransactionList()` -- Gets the `activeTransactionList`, representing the `transactionList` in the active account.
* `AccountsManager#getAccounts()` -- Gets `accounts`, representing the accounts in `accounts`.
* `AccountsManager#size()` -- Gets the length of the `accounts`, representing the number of accounts currently in `accounts`.
...
The following sequence diagram illustrates how the process of finding the accounts containing specified keyword works:

image::AccountSequenceDiagram.png[]

In general, the operations of account commands work using a similar sequence of steps.
Some commands might create a new `Account` object (as shown above) while others might just use the `Index` of an account.

Step 4. The user executes the command `account delete 2` to delete the second account in the `accounts`.
...
Step 5. The user executes the command `account edit 3 n/food` to edit the `name` of the first account in the list.
...
Step 6. The user executes the command `account report 2` to view the details of the second account in the list.
Firstly, `AccountsManager#resetFilteredAccountList` will be called to update the `filteredAccounts`,
so that all accounts present in `accounts` will be present in `filteredAccounts`,
then `AccountsManager#getAccount` and `Account#getAccountInfo` will be called to display the details of the second account.

Step 7. The user executes the command `account overview` to view the report of all accounts in an html export file.
Firstly, `AccountsManager#resetFilteredAccountList` will be called to update the `filteredAccounts`,
so that all accounts present in `accounts` will be present in `filteredAccounts`,
then `AccountsManager#exportReport` will be called to generate the overview of all accounts html file to the exports folder.

[NOTE]
For the commands `account edit`, `account delete` and `account report`,
